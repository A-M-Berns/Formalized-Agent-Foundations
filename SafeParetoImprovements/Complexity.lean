...
`n · ∏ᵢ mᵢ^{lᵢ} ≤ n · (∏ᵢ mᵢ) · m^{l−n} ≤ mⁿ · m^{l−n}`, using `lᵢ ≥ 1` (every reduced
action set is nonempty) and `card_mul_prod_le_pow`. -/
lemma card_unilateralCertificate_le' (Γ : Game N 𝒜) :
...
are at most `m ^ l` — the paper's `O(m^l)` on the nose, with the factor `n` from the
choice of player absorbed by the certificate count (`card_unilateralCertificate_le'`) —
for one passing the checks of Proposition 25.  Qualified node (`dd:complexity`) exactly as
`spiDecision_search`; the polynomial cost of the three checks (check 3 is a full
