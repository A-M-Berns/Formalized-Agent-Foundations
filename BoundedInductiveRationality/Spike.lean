...
what the paper says, and `covers_of_tendsto_atBot` together with the counterexample
following it fixes its relation to the global statement. -/
...
/-! ## D.4 Reactive environments — the paper's self-referential setting

§2 says "The sequence of decision problems `DP_t` may in turn be calculated depending on
the agent's choices", and every motivating case does exactly that: the Simplified
Adversarial Offer's reward is a function of the agent's own choice (§3, §4.5), and in §7
each player's reward is a function of *both* agents' choices. `theorem1_extensional` takes
`r` exogenously, so on its own it says nothing about those cases — which are the cases the
paper exists for.

**This turns out not to be an obstruction, and that is a finding.** The round-`t` winner
depends only on wealth at `t`, which depends only on rounds `< t`, so the reward stream can
be produced by the *same* structural recursion that produces the wealth. The result is a
genuine causal fixed point: a BRIA whose reward stream is generated by an environment from
that agent's own choices, with the fixed-point equation proved rather than assumed.

The paper never argues this. It is the difference between "there is a BRIA for any given
`DP, r̄`" (which is what Theorem 1 literally says) and "there is a BRIA that can be dropped
into a self-referential environment" (which is what §3 and §7 need). -/

section Reactive

variable (env : ℕ → Action → ℝ)

/-- Wealth under a reactive environment: the winner is credited `env t` applied to the
action it recommended, rather than an exogenous `r t`. -/
noncomputable def reactiveWealth : ℕ → ℕ → ℝ
  | 0, _ => 0
  | t + 1, i =>
      reactiveWealth t i + Al.A t i +
        (if i = pickWinner (fun j => min ((H j).estimate t) (reactiveWealth t j))
          then env t ((H i).choice t) - min ((H i).estimate t) (reactiveWealth t i)
          else 0)

/-- The reward stream the environment generates against this very auction. -/
noncomputable def reactiveReward (t : ℕ) : ℝ :=
  env t ((H (pickWinner (fun j => min ((H j).estimate t)
    (reactiveWealth Al H env t j)))).choice t)

/-- The two recursions agree: running the exogenous auction on the generated reward
stream reproduces the reactive one. This is the fixed point. -/
theorem wealth_reactive (t : ℕ) :
    wealth Al H (reactiveReward Al H env) t = reactiveWealth Al H env t := by
  induction t with
  | zero => rfl
  | succ t ih =>
    funext i
    show wealth Al H (reactiveReward Al H env) t i + Al.A t i +
        (if i = pickWinner (fun j => min ((H j).estimate t)
            (wealth Al H (reactiveReward Al H env) t j))
          then reactiveReward Al H env t
            - min ((H i).estimate t) (wealth Al H (reactiveReward Al H env) t i)
          else 0)
      = reactiveWealth Al H env t i + Al.A t i +
        (if i = pickWinner (fun j => min ((H j).estimate t)
            (reactiveWealth Al H env t j))
          then env t ((H i).choice t)
            - min ((H i).estimate t) (reactiveWealth Al H env t i)
          else 0)
    rw [ih]
    split_ifs with h
    · rw [reactiveReward, ← h]
...
/-- **Theorem 1a for reactive environments.** For every environment assigning a
nonnegative reward to each round and action, there is a BRIA covering the family whose
reward stream is exactly what that environment produces from the agent's own choices.

Note the hypothesis is only `0 ≤ env t x`; the auction never needs `env t x ≤ 1`. The
paper's `[0,1]` bound on rewards is load-bearing elsewhere (the Cesàro transfer of §B),
not here. -/
theorem theorem1_reactive (henv : ∀ t x, 0 ≤ env t x) :
    ∃ (r : ℕ → ℝ) (hr : ∀ t, 0 ≤ r t),
      IsBRIA (auctionAgent Al H r hr) r H (winSet Al H r) ∧
      ∀ t, r t = env t ((auctionAgent Al H r hr).choice t) := by
  refine ⟨reactiveReward Al H env, fun t => henv _ _,
    theorem1_extensional Al H _ (fun t => henv _ _), fun t => ?_⟩
  have hb : bid Al H (reactiveReward Al H env) t
      = fun j => min ((H j).estimate t) (reactiveWealth Al H env t j) := by
    funext j; rw [bid, wealth_reactive]
  show reactiveReward Al H env t
      = env t ((H (pickWinner (bid Al H (reactiveReward Al H env) t))).choice t)
  rw [hb, reactiveReward]
...
/-! ## E.1 The self-referential existence theorem, on the paper's own example

Putting §D.4 and §E together: the Simplified Adversarial Offer is a reactive environment,
and a BRIA exists *inside* it, with the reward stream generated by the agent's own
choices rather than handed to it. This is the case §3 introduces to show that logical
omniscience is contradictory, so it is the right non-vacuity test for the whole
construction. -/

/-- SAO as a reactive environment: `a₀ = false` pays `1/2`, `a₁ = true` pays `0`
(the counterfactual "`a₁` pays 1 if the agent chooses `a₀`" is never realised, exactly as
§4.5 observes). -/
noncomputable def saoEnv : ℕ → Bool → ℝ := fun _ x => if x = false then 1 / 2 else 0

example (H : ℕ → Hypothesis boolDP) :
    ∃ (r : ℕ → ℝ) (hr : ∀ t, 0 ≤ r t),
      IsBRIA (auctionAgent harmonicSquare H r hr) r H (winSet harmonicSquare H r) ∧
      ∀ t, r t = saoEnv t ((auctionAgent harmonicSquare H r hr).choice t) :=
  theorem1_reactive harmonicSquare H saoEnv
    (fun _ x => by unfold saoEnv; cases x <;> norm_num)
...
/-- **Theorem 2 — extensional core.** If the agent leaves room infinitely often, then for
*no* test set does it cover the diagonal hypothesis. Hence no BRIA whose family contains
...
/-- **Definition 8 as Theorem 4 uses it** — relativised to the rounds in which the option
is actually taken. The printed hypothesis is "the payoffs `r_t` **in rounds with
`αᶜ_t = a_t`** are `O(h(t))`-boundedly vMWC random with means `μ̄`", so the selection sets
range over subsets of those rounds. `Sel` stands for the class of admissible selectors —
the paper's "infinite `S ⊆ ℕ` decidable in `O(h(t))` time from available information",
which is undefined in the source and is therefore left abstract here rather than guessed
at. Keeping the `S.Infinite` guard matters: without it, `0/0 = 0` makes the condition
vacuously true for an empty selection. -/
def VMWCAveragedOn (r μ : ℕ → ℝ) (A : Set ℕ) (Sel : Set (Set ℕ)) : Prop :=
  ∀ S ∈ Sel, S ⊆ A → S.Infinite →
    Tendsto (fun T => (∑ t ∈ sel S T, (r t - μ t)) / (sel S T).card) atTop (𝓝 0)
...
/-- **Definition 8, as printed, is unsatisfiable outright** for a binary reward stream
with means `1/2`, as soon as the selector class contains a single infinite set — which it
must, since the paper quantifies over *infinite* `S` and `S = ℕ` is decidable under any
reading of "decidable from available information". -/
theorem not_vmwcPrinted_of_bits {y : ℕ → ℝ} {Sel : Set (Set ℕ)}
    (hbits : ∀ t, y t = 0 ∨ y t = 1)
    {S : Set ℕ} (hS : S ∈ Sel) (hSinf : S.Infinite) :
    ¬ VMWCPrinted y (fun _ => 1 / 2) Sel :=
  fun h => vmwcPrinted_vacuous_on_bits hSinf hbits (h S hS hSinf)
...
/-- A record over a **finite** test set is bounded below for free: each summand is at
least `-1` and the index set is contained in a fixed `Finset`. This is the branch that
lets `theorem4_single` keep the `S.Infinite` guard on its randomness hypothesis instead
of quietly relying on `0/0 = 0`. -/
theorem record_bddBelow_of_finite {DP : ℕ → Finset Action} {r : ℕ → ℝ} {M : Set ℕ}
    {h : Hypothesis DP} (hr : ∀ t, 0 ≤ r t) (hM : M.Finite) :
    ∃ C, ∀ T, C ≤ record r M h T := by
  refine ⟨-(hM.toFinset.card : ℝ), fun T => ?_⟩
  have hsub : sel M T ⊆ hM.toFinset := fun t ht =>
    hM.mem_toFinset.2 (mem_sel.1 ht).2
  have hcard : ((sel M T).card : ℝ) ≤ (hM.toFinset.card : ℝ) := by
    exact_mod_cast Finset.card_le_card hsub
  have hlow : -((sel M T).card : ℝ) ≤ record r M h T := by
    unfold record
    calc -((sel M T).card : ℝ) = ∑ _t ∈ sel M T, (-1 : ℝ) := by simp
      _ ≤ _ := Finset.sum_le_sum fun t _ => by
          linarith [hr t, h.estimate_le_one t]
  linarith

/-- **Theorem 4's engine.** On a test set where the hypothesis promises at least `ε`
below the mean, an averaged-deviation hypothesis keeps the record bounded below — indeed
eventually nonnegative. This lemma is deliberately stated *without* an infinitude guard,
because it is the unguarded engine; the guard lives upstream in `VMWCAveragedOn`, and
`theorem4_single` supplies the finite branch separately via `record_bddBelow_of_finite`.
Do not apply this lemma with a hypothesis obtained from an unguarded quantifier: for
finite `M` its `hvm` degenerates (`0/0 = 0`). -/
...
/-- **Theorem 4 — the single-`ε` step.**

Faithful to the printed hypotheses: the recommended option `ā` appears (`hia`), the
randomness is asserted of the reward stream **on the rounds where the agent takes `a_t`**
(`hrand`), and the selector class `Sel` is abstract because the paper never defines
"decidable from available information".

The residue is `hsel`: *the agent's own test set for this hypothesis is an admissible
selector*. In the paper that is discharged by the sentence "`M` is by assumption
computable in `O(h(t))` given the information available at time `t`", which leans on §4.4's
convention that test sets are output alongside `ᾱ` plus the assumption that `ᾱ` is
`O(h(t))`-computable. Theorem 4 is the **only** theorem in the paper where the agent's own
computability is load-bearing, and `hsel` is exactly where it bites. It is a hypothesis
here, named, and not a predicate pretending to be a proof.

Lemma 6 does the paper's WLOG: the legal promise is `max(μ_t − ε, 0)`, and trimming the
rounds where that clips to `0` leaves a test set on which the promise is exactly
`μ_t − ε`. -/
theorem theorem4_single {DP : ℕ → Finset Action} {ι : Type*} {α : Agent DP} {r : ℕ → ℝ}
    {H : ι → Hypothesis DP} {M : ι → Set ℕ} (hBRIA : IsBRIA α r H M)
    (hr : ∀ t, 0 ≤ r t) (μ : ℕ → ℝ) (ε : ℝ) (hε : 0 < ε)
    (a : ℕ → Action) (i : ι) (hia : (H i).choice = a)
    (hiE : (H i).estimate = fun t => max (μ t - ε) 0)
    (Sel : Set (Set ℕ))
    (hrand : VMWCAveragedOn r μ {t | α.choice t = a t} Sel)
    (hsel : (M i \ {t | μ t - ε ≤ 0}) ∈ Sel) :
  ...
  -- the trimmed test set really does consist of rounds where the agent takes `a t`
  have hsub : (M i \ N) ⊆ {t | α.choice t = a t} := by
    intro t ht
    have := hBRIA.testSet i t ht.1
    simpa [Set.mem_setOf_eq, hia] using this
  obtain ⟨C, hC⟩ : ∃ C, ∀ T, C ≤ record r (M i \ N) (H i) T := by
    by_cases hfin : (M i \ N).Finite
    · exact record_bddBelow_of_finite hr hfin
    · exact record_bddBelow_of_vmwc (h := H i) hε hpromise
        (hrand _ hsel hsub (Set.not_finite.1 hfin))
...
/-- **Theorem 4.** The reward stream on the rounds where the agent takes `a_t` is
averaged-vMWC random with means `μ` relative to the selector class `Sel`; for every
`ε > 0` the covered family contains the hypothesis `(a_t, max(μ_t − ε, 0))`, whose trimmed
test set is an admissible selector. Then the agent's average reward dominates the average
of `μ`.

Two residues, both explicit and both corresponding to sentences of the printed proof:
`(H i).choice = a` with the index `i` ("`h̄_{a,ε}` is e.c.", which Theorem 4 — unlike
Theorem 3 — does not even assume the option is efficiently identifiable), and membership
of the test set in `Sel` ("`M` is by assumption computable in `O(h(t))`"). -/
theorem theorem4 {DP : ℕ → Finset Action} {ι : Type*} {α : Agent DP} {r : ℕ → ℝ}
    {H : ι → Hypothesis DP} {M : ι → Set ℕ} (hBRIA : IsBRIA α r H M)
    (hr : ∀ t, 0 ≤ r t) (μ : ℕ → ℝ) (hμ1 : ∀ t, μ t ≤ 1)
    (a : ℕ → Action) (Sel : Set (Set ℕ))
    (hrand : VMWCAveragedOn r μ {t | α.choice t = a t} Sel)
    (hfam : ∀ ε > 0, ∃ i, (H i).choice = a ∧
      (H i).estimate = (fun t => max (μ t - ε) 0) ∧
      (M i \ {t | μ t - ε ≤ 0}) ∈ Sel) :
    AsympGE (avg r) (avg μ) := by
  intro δ hδ
  obtain ⟨i, hia, hiE, hsel⟩ := hfam (δ / 2) (by linarith)
  have hev := theorem4_single hBRIA hr μ (δ / 2) (by linarith) a i hia hiE Sel hrand hsel
...
asserts the diagonal hypothesis "is computable (in `O(g(t))`)" without argument; these are
the obligations that assertion hides. Each decision procedure carries its *specification*,
so the structure cannot be inhabited by garbage — the point is that it can still be
inhabited *classically* (`classicalDiagonalObligations` below), which is precisely the
obstruction. -/
structure DiagonalObligations {Action : Type*} {DP : ℕ → Finset Action} (α : Agent DP) where
  /-- (1) the agent's estimate must be compared *exactly* against `1`. Undecidable if
  estimates are computable reals — see `PromiseRepresentation`. -/
  decEstimateLtOne : ℕ → Bool
  decEstimateLtOne_spec : ∀ t, decEstimateLtOne t = true ↔ α.estimate t < 1
  /-- (2) the size test `|DP_t| ≥ 2` must be decidable within the bound. -/
  decTwoOptions : ℕ → Bool
  decTwoOptions_spec : ∀ t, decTwoOptions t = true ↔ 1 < (DP t).card
  /-- (3) an option other than the agent's must be *produced*, not merely shown to
  exist — a search over `DP_t`, whose cost is the size of `DP_t`, a quantity the paper's
  `O(g(t))` accounting (indexed by `t` alone) never mentions. -/
  altOption : ℕ → Action
  altOption_mem : ∀ t, altOption t ∈ DP t
  /-- (4) …and it must actually differ from the agent's choice, which is the whole
  content of the diagonal. -/
  altOption_ne : ∀ t, 1 < (DP t).card → altOption t ≠ α.choice t

/-- **The obstruction, compiled.** The obligations above are *classically* dischargeable
for every agent — `Classical.propDecidable` supplies both decision procedures and
`diagChoice` supplies the alternative. So no theorem stated over `DiagonalObligations`
alone can distinguish a computable diagonal hypothesis from an uncomputable one. That is
not a defect of this interface; it is the reason Theorem 2 cannot be given at paper
strength without a machine model in which programs, not functions, are the objects. -/
noncomputable def classicalDiagonalObligations {Action : Type*} {DP : ℕ → Finset Action}
    (α : Agent DP) : DiagonalObligations α where
  decEstimateLtOne t := decide (α.estimate t < 1)
  decEstimateLtOne_spec t := by simp
  decTwoOptions t := decide (1 < (DP t).card)
  decTwoOptions_spec t := by simp
  altOption t := diagChoice α t
  altOption_mem t := diagChoice_mem α t
  altOption_ne t ht := diagChoice_ne α t ht
