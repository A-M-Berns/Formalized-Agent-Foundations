...
/-- **Conditional independence transfers along a pushforward.**  `X ⊥ Y | Z` under the
image distribution `Q ∘ f⁻¹` on `T` is `X∘f ⊥ Y∘f | Z∘f` under `Q` on `S`: the preimage
under `f` of the event `{X = x}` is the event `{X∘f = x}`, so the two sides are the same
identity between probabilities of the same events. -/
lemma condIndepVar_map {S : Type w} {T : Type w'} [Fintype S] [Fintype T] {α β γ : Type*}
    (f : S → T) (Q : Distr S) (X : T → α) (Y : T → β) (Z : T → γ) :
    CondIndepVar (Q.map f) X Y Z ↔ CondIndepVar Q (X ∘ f) (Y ∘ f) (Z ∘ f) := by
  refine forall_congr' fun x => forall_congr' fun y => forall_congr' fun z => ?_
  simp only [CondIndep, Distr.map_prob, Set.preimage_inter]
  -- the preimage of `{t | X t = x}` under `f` is `{s | (X ∘ f) s = x}`, definitionally
  exact Iff.rfl
...
/-! ## Conditional independence of families of coordinates

Deciding `CondIndepVar P (proj J₁) (proj J₂) (proj J₃)` for *arbitrary, possibly
overlapping* `J₁, J₂, J₃` — which is what Definition 5.7's perfect maps quantify over
(`PerfectMap.lean`; errata E17) — comes down to three facts: a family already determined
by the conditioning family is independent of everything
(`condIndepVar_proj_of_subset_left`), a conditional independence of families restricts to
subfamilies (`CondIndepVar.of_proj_subset`), and a non-degenerate coordinate is never
independent of *itself* given a family that omits it (`not_condIndepVar_proj_self`), which
is what makes the overlapping quantification carry content.
-/
