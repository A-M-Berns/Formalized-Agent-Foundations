...
* **Distributions** (`dd:dist`, namespace `Distr`): `Distr S` (`mass`, `nonneg`,
  ...
  (`uniform_strictlyPositive`), `mix`, `euclDist` (`euclDist_self`, `euclDist_comm`,
  `euclDist_nonneg`, `abs_sub_le_euclDist` — the entry point to `dd:open-ball`),
  `nonempty_carrier`, `condDist`
...
* **Conditional independence** (Definition 6.1, product form, over an arbitrary finite
  sample space `S` — only `CondIndepAll` is `Pt Ω`-specific; the variable forms write their
  events as `{s | X s = x}`, i.e. `fiber X x` unfolded, so `rw` against a `fiber`-stated
  lemma needs a `show … (fiber X x) …` first): `CondIndep P A B C`
...
* **Soundness and completeness**: Theorem 6.2 `structIndepGiven_iff_forall_condIndepVar`
  ...
  `exists_polynomial_interp_prob`, Lemma C.10 `interp_mem_factorizingPos` with its halves
  `factorizes_interp` and `interp_prob_pos`), and the cohistory
...
* **Semigraphoids** (Definition 5.1): `IndepRel Ω` (no `[Nonempty]` on any slot; value
  spaces in the factors' universe `Type v`, a Lean structure-field limitation), `IsSemigraphoid`,
  `IsGraphoid`, `IsCompositionalSemigraphoid`; `structIndepRel Ω`, `condIndepRel P`;
  Proposition 5.2 `isCompositionalSemigraphoid_structIndepRel`, the Pearl half
  `isSemigraphoid_condIndepRel` (proved), and Table 1's negative claim in two forms:
  `not_intersection_structIndepRel` (the failing axiom written out, over one `Fin 3` factor
  with pairwise-distinct `X, Y, Z`) and `not_isGraphoid_structIndepRel`.
