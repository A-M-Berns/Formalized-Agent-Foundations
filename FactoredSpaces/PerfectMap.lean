...
Checking Definition 5.7 by hand over arbitrary, possibly overlapping triples runs on the
conditional-independence tools of `Probability.lean` (`condIndepVar_proj_of_subset_left` /
`_right`, `CondIndepVar.of_proj_subset`, `not_condIndepVar_proj_self`), which are §6.1
vocabulary; the transport along a family's joint variable is
`condIndepVar_map_famJoint` (`BayesNet.lean`).
-/
