...
theorem exists_polynomial_interp_prob (Q P : ∀ i, Dist (Ω i)) (A : Set (Pt Ω)) :
  ...
  · intro t
    rw [Dist.prob_eq_sum_filter, Polynomial.eval_finsetSum]
    refine Finset.sum_congr rfl fun ω _ => ?_
    rw [Polynomial.eval_prod]
    simp only [interp, Dist.prod_mass, Dist.mix, Polynomial.eval_add, Polynomial.eval_mul,
...
theorem interp_prob_pos (P P' : ∀ i, Dist (Ω i)) {C : Set (Pt Ω)}
  ...
  · have key : (t : ℝ) ^ Fintype.card I * (Dist.prod P').prob C ≤ (interp P P' t).prob C := by
      rw [Dist.prob_eq_sum_filter (Dist.prod P') C, Dist.prob_eq_sum_filter (interp P P' t) C,
        Finset.mul_sum]
      refine Finset.sum_le_sum fun ω _ => ?_
      have hrw : (t : ℝ) ^ Fintype.card I * (Dist.prod P').mass ω
          = ∏ i, (t : ℝ) * (P' i).mass (ω i) := by
        rw [Finset.prod_mul_distrib, Finset.prod_const, Finset.card_univ, Dist.prod_mass]
      rw [hrw]
      simp only [interp, Dist.prod_mass, Dist.mix]
    ...
    have hpos : 0 < (t : ℝ) ^ Fintype.card I * (Dist.prod P').prob C := by positivity
    linarith
...
private lemma exists_delta_euclDist_interp (Q P : ∀ i, Dist (Ω i)) {ε : ℝ} (hε : 0 < ε) :
    ∃ δ > 0, ∀ t : unitInterval, (t : ℝ) < δ →
      Dist.euclDist (Dist.prod Q) (interp Q P t) < ε := by
  classical
  have hcont : Continuous fun s : ℝ => ∑ ω : Pt Ω,
      ((Dist.prod Q).mass ω - ∏ i, ((1 - s) * (Q i).mass (ω i) + s * (P i).mass (ω i))) ^ 2 := by
    fun_prop
  obtain ⟨δ, hδ, hδ'⟩ :=
  ...
  have hzero : (∑ ω : Pt Ω,
      ((Dist.prod Q).mass ω - ∏ i, ((1 - (0 : ℝ)) * (Q i).mass (ω i)
        + (0 : ℝ) * (P i).mass (ω i))) ^ 2) = 0 := by
    simp [Dist.prod_mass]
  have hlt := hδ' hd
  rw [hzero, Real.dist_eq, sub_zero] at hlt
  rw [Dist.euclDist, Real.sqrt_lt' hε]
  refine lt_of_le_of_lt (le_abs_self _) ?_
  simpa only [interp, Dist.prod_mass, Dist.mix] using hlt
...
theorem condIndepVar_of_local {α β γ : Type*} {X : Pt Ω → α} {Y : Pt Ω → β} {Z : Pt Ω → γ}
  ...
  have e₁ : f₁.eval (1 : ℝ) = (Dist.prod p).prob (fiber X x ∩ fiber Z z) := by
    have := hf₁ 1; rwa [interp_one] at this
  have e₂ : f₂.eval (1 : ℝ) = (Dist.prod p).prob (fiber Y y ∩ fiber Z z) := by
    have := hf₂ 1; rwa [interp_one] at this
  have e₃ : f₃.eval (1 : ℝ) =
      (Dist.prod p).prob (fiber X x ∩ fiber Y y ∩ fiber Z z) := by
    have := hf₃ 1; rwa [interp_one] at this
  have e₄ : f₄.eval (1 : ℝ) = (Dist.prod p).prob (fiber Z z) := by
    have := hf₄ 1; rwa [interp_one] at this
  ...
  simp only [Polynomial.eval_sub, Polynomial.eval_mul, Polynomial.eval_zero,
    e₁, e₂, e₃, e₄] at hone
  show (Dist.prod p).prob (fiber X x ∩ fiber Z z) *
      (Dist.prod p).prob (fiber Y y ∩ fiber Z z) =
    (Dist.prod p).prob (fiber X x ∩ fiber Y y ∩ fiber Z z) * (Dist.prod p).prob (fiber Z z)
  linarith
