...
| `thm:affpolymax` (analytic + semantic construction hubs) | `NoPreemptiveUnderpricing`, `NoPreemptiveOverpricing`, `affineFutureHigh`, `affineFutureLow`, `BoundedAffinePrices`, `affpolymax_of_noPreemptiveGaps`; `gradualRemaining`, `gradualSellFraction`, `gradualTrader`, `gradualTrader_hasROI_of_price_gap` (`Properties/AffinePreemptiveLearning.lean`) | **done conditional on family-level e.c./maturity bridge** | **P** | The exact two paper equalities are kernel-checked from the operational threshold conditions produced by affine ROI. The concrete continuous component is also proved: it buys with a low-price ramp, sells fractions as the high-price ramp rises, telescopes to full liquidation, has exact total magnitude `2·entry·|A|`, stabilizes world-independently, and obtains positive semantic ROI from every protected low-to-later-high gap. Uniform affine price boundedness supplies every limit side condition. Still required for the unconditional `[IsLogicalInductor]` theorem: uniformly emit this family, construct its verified maturity checker from `ComputableMarket`/`ComputableDeductiveProcess`, and feed the resulting no-repeatable-ROI contradiction into `AffineNoPreemptiveGaps`. Axiom-clean. |
