...
| `lem:type3` (sparse ROI interface) | `allocationPrefix_not_bddAbove_of_frequently`, `sharedBudgetedTrader_exploits_of_frequently`, `repeatableROI_of_frequently`, `noRepeatableROI` (`ROI.lean`) | **done, conditional on polynomial maturity verifier** | **P+C** | Removes the provisional uniform-positive-magnitude restriction: zero-magnitude components between opportunities consume no budget, and magnitudes merely frequently bounded below force unbounded allocation. `noRepeatableROI` now proves the paper-facing conclusion `αₖ → 0` against `IsLogicalInductor`. Remaining computability gate is explicit: its selected maturity table must still be supplied as `PolyActiveSchedule`; deriving that table by bounded verification from `ComputableMarket`/`ComputableDeductiveProcess` is the next M4 task. Axiom-clean. |
