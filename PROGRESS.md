...
| M3 | Downstream property slice + LUV bridge + integration test | **implementation complete; pending Anson's statement read-through and fresh-context audit** — proved and axiom-clean: `thm:provind`, all three `thm:lc` bullets, both `thm:lex` forms, `thm:con`, both directions and limit forms of `thm:nd`, `lem:conluvapprox`, and `thm:ec` (including every exploiting trader's `EfficientlyComputableTok` certificate). The integration test discharges both the fixed-sentence provability hypothesis and concrete LUV expectation convergence. The seven statements moved to M4 have since split: `thm:ei`, `thm:loe`, and `thm:expprovind` are now proved; the four Self-Trust statements remain under interface audit. No M3 proof or e.c. certificate remains deferred. |
| M4 | Affine master + reusable LUV lift; close expectation/Self-Trust statements | **in progress (2026-07-13; expectation lift complete, Self-Trust interface blocked)** — the affine master and reusable fixed-LUV lift are complete and axiom-clean. `PolySequence.affine_provind` gives the semantic lower-bound bridge; certified expectation, indicator, and three-way linearity bundles discharge `thm:ei`, `thm:loe`, and `thm:expprovind`. Full build: 2,654 jobs; only the four Self-Trust declarations retain `sorry`. **Audit finding:** their current relational quote hypotheses do not connect the day-`n` threshold grid to the different day-`f n` grid. `thm:affpolymax` transports a fixed affine bundle to arbitrary later liquidation days, while `PolyThresholdCodeSeq` certifies emission only; neither supplies that cross-grid logical coherence. Imminent work is a non-oracular quote/coherence interface and statement repair before proofs of `thm:cee`/`ceu`/`ccee`/`st`. |
...
| `thm:cee` / `thm:ceu` / `thm:ccee` / `thm:st` | four declarations in `Properties/SelfTrust.lean` | **blocked statement audit — 4 `sorry`s** | **stmt** | Current revelation-schedule `ValuesAt` hypotheses constrain quoted values only after `D (r n)` and do not relate the day-`n` expectation grid to the day-`f n` grid. The affine hub transports fixed bundles but cannot manufacture that relation; codeability is not logical coherence. Repair requires a non-oracular quote/coherence interface before proof work resumes. |
...
`done` means kernel-checked without `sorryAx`; `blocked M4 statement` marks one of the four
Self-Trust types whose interface audit must be resolved before a sound proof can land.
...
- `lic_expected_future_expectations` — `Properties/SelfTrust.lean:80` — current and quoted future expectations agree asymptotically (`blocked M4 statement`, `thm:cee`).
- `lic_no_expected_net_update` — `Properties/SelfTrust.lean:94` — current prices agree with expectations of their quoted future prices (`blocked M4 statement`, `thm:ceu`).
- `lic_no_expected_net_update_conditional` — `Properties/SelfTrust.lean:114` — conditional/weighted expected future expectations agree (`blocked M4 statement`, `thm:ccee`).
- `lic_self_trust` — `Properties/SelfTrust.lean:140` — continuous-threshold self-trust inequality (`blocked M4 statement`, `thm:st`).
...
conclusions; (2) Self-Trust reflection hypotheses must preserve delayed revelation, must not
smuggle back an oracle-`DP` witness, and must explicitly bridge the `n`/`f n` expectation
grids; (3) Non-Dogmatism's persistent-world hypotheses are the
