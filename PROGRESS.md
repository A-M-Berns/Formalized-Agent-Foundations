...
| `dd:fuel` (dispatch) | `iterRight`, `sel`, `selFn`, `tupleEnc`, **`iterRight_evaln`** | done | Def+**P** | **runtime index selection** `sel ⟨T,i⟩ = left (right^i T) = tupleEnc⁻¹[i]`, via one genuine `Code.prec` recursion on `i` (the 2nd such in the file, after `predc`). Fuel bounded degree-2 in `pair T i` through the clocked interpreter. `selFn_tupleEnc`: selection correct on a right-nested tuple. Axiom-clean |
...
| `thm:con` (arbitrage) | `oscillation_exploitable` | **sorry (blocked)** | — | states: a rational oscillation (+ daily plausible world) admits an **e.c. trader that exploits**. **Blocked by OPEN RISK 4**, not by missing effort: convergence's exploiter needs a **hysteresis** rule (buy `<a`, hold until `>b`, sell) to bank `b−a` per swing on *smooth* oscillations; that "am I holding?" state is an unbounded look-back = a **linear-depth `EF`**, whose `toNat` code is `~4ⁿ` bits — unproducible in `evaln(poly n)`, so it is outside our e.c. class though inside the paper's (poly-*size*). Bounded-depth alternatives provably fail (memoryless → path-independent; mean-reversion → only quadratic variation, killed by smooth ramps). **Resolution is a trust-surface decision for Anson** (re-encode e.c. over poly-*size* structural descriptions), not a trader to construct. Statement stays honest & true (the exploiter exists in the paper's model); the `sorry` is the disclosed gap |
...
- **⚠️ OPEN RISK 4 — `EfficientlyComputable` excludes poly-*size*, linear-*depth* strategies
  (surfaced while attempting `thm:con`; needs Anson).** The `dd:fuel` model (OPEN RISK 3) is
