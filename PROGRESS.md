...
| M7 | budgeter, trading firm, LIA, existence, unconditionalization | **core construction complete; conditional+disclosed construction-scope endpoint closed 2026-07-18; stronger witness construction explicitly reopened 2026-07-18** — efficient-trader enumeration, the process-backed Budgeter, exact `TradingFirm`, dominance, recursive rational LIA, and the complete executable compiler are proved. `LIA_is_logical_inductor` and `exists_logical_inductor` are unconditional, use none of the fifteen post-M5 boundaries, and expose only `propext`, `Classical.choice`, and `Quot.sound`. Four realizability-critical boundaries are constructed; the other eleven are named, field-audited, cited, and disclosed in the README. Anson has reopened actual construction beyond that endpoint; the next bounded target is `M7-FEEDBACK-EMIT`, after which public-surface consolidation, deferred statement read-through, the M7 paper comparison, `M7-ERRATA-AUDIT`, and the final release gate still remain. |
...
via `evaln_mono`) drives `repeats`. Axiom-clean. These are the first two of the **4 of 15
constructed witnesses**; `M7-PATIENT-CLOCK` and `M7-PREFIX-PATCH` were subsequently closed.
...
1. The paper's `Budgeter`, Trading Firm, recursive rational `LIA`,
   `LIA_is_logical_inductor`, and `exists_logical_inductor` are concrete, executable where
   claimed, kernel-checked, and independent of every post-M5 representation witness.
2. Every construction whose realizability was genuinely in doubt is built. The four
   constructed witnesses are `M7-HIST-EVALN`, `M7-CE-REPETITION`,
   `M7-PATIENT-CLOCK`, and `M7-PREFIX-PATCH`.
3. Each standard-object or known-realizable boundary is named, mapped to its exact Lean
   fields and consumers, tied to the paper and a primary citation, and explicit about both
   what it assumes and what downstream conclusion it does not assume. These eleven
   boundaries remain hypotheses of the conditional property tail; they are not dependencies
   of the existence theorem.
4. The 15-row inventory below has no ambiguous cells; the construction/property/integration
   roll-up, full build, executable-hole scan, diff check, and public capstone axiom reports
   pass.

This closes the **construction-scope phase**, not the whole M7 release. Consolidation and
API cleanup, the frozen statement inventory, the line-by-line M7 paper comparison, Anson's
deferred statement/definition read-through, `M7-ERRATA-AUDIT`, correction recheck, and the
final release gate remain required in that order.

**Historical scope note.** The original contract set on 2026-07-14 required concrete Lean
inhabitants for all fifteen boundaries and fully instantiated M3–M5 corollaries. That
stronger literal-all-15 project is not complete and is not being silently redefined as
complete. It would additionally require a first-order Gödel/representability development,
concrete quotation syntax, a universal prefix machine and Kraft proof, a universal lower
semicomputable continuous semimeasure, and the recursively-inseparable separator-mass
theorem. Reopening any of those is a separate product-scope decision.

### M7 witness inventory — construction-scope endpoint

| # | Boundary | Disposition | Exact evidence or disclosed interface | Downstream scope |
|---:|---|---|---|---|
| 1 | `M7-HIST-EVALN` | **constructed** | `codeEvalnNat_polyFueled`, `boundedEvalnCompiler` (`Construction/M7Witnesses.lean`) | shared bounded interpreter infrastructure |
| 2 | `M7-CE-REPETITION` | **constructed** | `EfficientRepeatedEnumeration.ofCE` (`Construction/M7Witnesses.lean`) | `thm:obu` |
| 3 | `M7-PATIENT-CLOCK` | **constructed** | `SettlementChecker.ofComputations`, `PatientSettlementClock.ofComputations` (`Construction/M7Witnesses.lean`) | `thm:prandaff`, `thm:prandexp` |
| 4 | `M7-PREFIX-PATCH` | **constructed** | `liaEfficientPrefixPatch` (`Construction/M7Witnesses.lean`) | the LIA instance of `thm:ifp`'s honest conditional replacement |
| 5 | `M7-QUOTE-AFFINE` | **disclosed** | `CompletedAffineQuoteEq`, quote packages (`Properties/Introspection.lean`); `AffineQuotePortfolio`, `AffineQuoteEq/GE`, deferred quote packages (`Properties/SelfTrust.lean`) | `thm:ref`, `lp`, `epr`, `er`, `cee`, `ceu`, `ccee`, `st` |
| 6 | `M7-PREFIX-MACHINE` | **disclosed** | `PrefixMachinePresentation`, `OccamThresholdEmission`, `PrefixNegationCompiler` (`Properties/OccamBounds.lean`) | `thm:ob` |
| 7 | `M7-FEEDBACK-EMIT` | **disclosed** | `FeedbackTraderEmission`, `.Family`, `.Signs` (`Properties/Pseudorandomness.lean`) | `thm:wubaff`, `thm:wubexp` |
| 8 | `M7-FEEDBACK-TRUTH` | **disclosed** | `FeedbackTruthSequence` (`Properties/Pseudorandomness.lean`) | `thm:wubaff`, `thm:wubexp` |
| 9 | `M7-DUS-PREFIX-SYNTAX` | **disclosed** | `BitPrefixSentences` (`Properties/UniversalSemimeasure.lean`) | `thm:dus`, dependency of `thm:strict` |
| 10 | `M7-SCON-COMPILER` | **disclosed** | `GatedConditioningOperationalWitness` (`Properties/Conditioning.lean`) | `thm:scon` |
| 11 | `M7-SCON-PRESENTATION` | **disclosed** | `ConditioningPresentation` (`Properties/Conditioning.lean`) | `thm:scon` |
| 12 | `M7-LUV-SYNTAX` | **disclosed** | `LUV.PolyThresholdCodes/Seq`; `LUVCombination.PolySequence`, `WorldValued`, `ConvergencePresentation`, `ExactTheoryPresentation`, `MeshSoftmaxOperationalWitness` (`Expectations.lean`, `Properties/ExpectationProperties.lean`) | expectation-property tail |
| 13 | `M7-DUS-APPROX` | **disclosed** | `DUSApproximationPresentation`, `DUSThresholdEmission` (`Properties/UniversalSemimeasure.lean`) | `thm:dus` |
| 14 | `M7-STRICT-SEPARATORS` | **disclosed** | `StrictSeparatorPresentation` (`Properties/StrictSemimeasure.lean`) | `thm:strict` |
| 15 | `M7-COMP-SYNTAX` | **disclosed** | `RepresentedSemidecidableClaims`, `RepresentedDecidableClaims`, `InconsistentTheoryClaims` (`Properties/MetaLearning.lean`) | `thm:pac`, `pazfc`, `incons`, `halts`, `loops`, `dontwait` |

The README's “Axioms and disclosed M7 boundaries” section is the auditable disclosure
packet: it records the exact fields, paper nodes, concrete standard realizer and primary
citation, consumers, and excluded conclusions for every disclosed row.

**Field-audit findings (2026-07-18):** the packet was checked against every structure field,
not only its docstring. Five boundaries needed especially precise wording:
`M7-QUOTE-AFFINE` includes exact current-price identities and `AffineQuoteEq/GE`'s
asymptotic deferred-price coherence; `M7-FEEDBACK-TRUTH` includes the exact
`feedback_price` identity; `M7-SCON-COMPILER` includes a market-dependent denominator floor;
`M7-LUV-SYNTAX` includes market-indexed bounded-price/magnitude certificates; and
`M7-STRICT-SEPARATORS` includes the substantive `mass_tendsto_zero` theorem. These facts are
now stated affirmatively in the README. None of the eleven interfaces contains an
`IsLogicalInductor`, exploitation, or its advertised consumer theorem as a field; the
excluded-conclusion column records the narrower conclusion each interface also avoids.

**Construction-scope endpoint verification (2026-07-18):**
`lake build LogicalInduction.Construction LogicalInduction.Properties
LogicalInduction.IntegrationTest` completed 2,469 jobs and a fresh `lake build` completed
2,680. `git diff --check` is clean. The broad `sorry`/`admit`/`sorryAx` scan has four hits,
all in prose comments; there are no executable holes and no `axiom` declarations under
`LogicalInduction/`. Separate `#print axioms` checks for `LIA_is_logical_inductor`,
`exists_logical_inductor`, all four constructed witness capstones
(`codeEvalnNat_polyFueled`/`boundedEvalnCompiler`,
`EfficientRepeatedEnumeration.ofCE`, `PatientSettlementClock.ofComputations`, and
`liaEfficientPrefixPatch`), and representative
Occam/DUS/strict-domination/conditioning/Self-Trust property capstones report exactly
`propext`, `Classical.choice`, and `Quot.sound`. README, ledger, and handoff each contain the
same fifteen inventory rows, with no open disposition.
...
hypothesis is discharged by `liaEfficientPrefixPatch` (`M7-PREFIX-PATCH`) and the theorem is
**not vacuous**. But
...
The M7 patient-clock rows are a reverse-chronological construction record. Any “partial,”
“still owed,” or “done modulo” wording in an older step describes the state at that step;
steps 18–19 above it supersede those local handoffs and close the witness unconditionally.
...
| `M7-PATIENT-CLOCK` (step 19: checker extracted; witness closed) | `SettlementChecker.ofComputations`; `PatientSettlementClock.ofComputations` (`Construction/M7Witnesses.lean`) | **done (witness complete)** | **C/P** `(a,b)` | Extracts an actual `Nat.Partrec.Code` for the unbounded fuel search using `Partrec.rfindOpt` and `Nat.Partrec.Code.exists_code`. The checker specification is proved in both directions from `settlementCheckAtFuel_sound`/`settlementCheckAtFuel_complete`; it is not assumed. The constructor consumes `AffineCombination.PolySequence As` directly through step 18's primitive-recursive bridge, then `PatientSettlementClock.ofComputations` combines it with the already-proved semantic clock constructor. `M7-PATIENT-CLOCK` is therefore closed without a disclosed bridge or conclusion-in-hypothesis shortcut. Targeted 2,467-job build and full 2,680-job build green; printed axioms are exactly `propext`, `Classical.choice`, `Quot.sound`. |
| `M7-PATIENT-CLOCK` (step 18: polynomial affine families are primitive recursive) | `PolyFueled.primrec`, `PolySegStream.primrec` (`Computable.lean`); `deserializeTrades_prim` exposed (`LIACompiler.lean`); `efFromSerializedTokens` (+`_prim`, `_serialize`), `AffineCombination.PolySequence.primrec` (`M7Witnesses.lean`) | done | **P/C** `(a,b)` | Eliminates the disclosed type-`(c)` bridge `PolySequence As → Primrec As`. A `PolyFueled` code is run at the explicit polynomial majorant bundled by `IsPolyBounded`, using Mathlib's primitive-recursive `Code.evaln`; a `PolySegStream` is reconstructed as the map of its primitive-recursive token emitter over its primitive-recursive length range. Feature serializations are inverted by appending one dummy trade frame and reusing the canonical `deserializeTrades_prim` parser (exposed, not duplicated), with correctness from `deserializeTrades_serializeTrades`. The affine constant, coefficient family, sentence family, and ranged term list are then assembled through `PolySequence.terms_eq`. Full dependency rebuild and `M7Witnesses.lean` compile green. **Closed by step 19:** `Partrec.rfindOpt` + `Code.exists_code` → `SettlementChecker.ofComputations`. |
| `M7-PATIENT-CLOCK` (step 17: bounded settlement check compiled) | `AffineCombination.settlementCheckAtFuel_prim` (`Construction/M7Witnesses.lean`) | done (item 4 assembly, cont.) | **P/C** `(a,b)` | Compiles the complete conservative Boolean settlement test as a primitive-recursive function of `(A,j,fuel)`. The proof closes the nested `allBitLists`/`List.all` quantifiers with two `Primrec.list_foldr`s, compiles the process-stage `Option` match, and reuses `processStageAtFuel_prim`, `settlementAtomLimit_prim`, `allBitLists_prim`, `stageSatBits_prim`, and `valueRatCompAt_prim`. Optional rational equality is compiled as `v.isSome && decide (v = v')`, proved extensionally equal to the original two-option match via `valueRatCompAt_eq`; hence a timeout still reads `false` and the soundness-critical guard is preserved. Scoped `Nat.sqrt` irreducibility was again required for the deep product/fold input. `M7Witnesses.lean` compiles in-repo. **Closed by steps 18–19:** `PolySequence.primrec` supplies the polynomial bridge and `SettlementChecker.ofComputations` extracts the checker code. |
| `M7-PATIENT-CLOCK` (step 16: the computable value evaluator at a bit-list world) | `bitsPayoutRat_prim`; `affineTermsRatComp` (+`_eq`, `_prim`); `valueRatCompAt` (+`_eq`, `_prim`) (`Construction/M7Witnesses.lean`); `ratAdd_prim`, `ratMul_prim` un-`private`d (`LIACompiler.lean`) | done (item 4 assembly, cont.) | **P/C** `(a,b)` | Recasts `A.valueRatAtFuel market fuel (bitsPayoutRat l)` — the quantity `settlementCheckAtFuel` compares across worlds — through the computable `denoteRatComp` (step 15), so the whole check becomes primitive recursive. `valueRatCompAt_eq`/`affineTermsRatComp_eq` prove exact equality to the originals `(a)` (per-term `denoteRatComp_eq` rewrite; `map`/`bind` reconciled by casing on the `Option`s). `valueRatCompAt_prim : Primrec` via `list_foldr` over `A.terms` with `ratAdd_prim`/`ratMul_prim` (un-`private`d, **not** duplicated) + `bitsPayoutRat_prim` (from `evalBits_prim` `(b)`) + `denoteRatComp_prim`. **The `Nat.sqrt`/`whnf` GOTCHA fired here** — the first time across steps 5–16 — on the deep `list_foldr` step input `((AffineCombination × ℕ) × List Bool) × ((EF × Sentence) × Option ℚ)`; diagnosed (not heartbeat-bumped) and fixed with the documented scoped `attribute [local irreducible] Nat.sqrt`, vindicating the handoff's "diagnose first" guidance. M7Witnesses green at 2,467 jobs, capstones axiom-clean. **Closed by steps 17–19:** the Boolean check, polynomial-family bridge, and extracted checker code all landed. |
| `M7-PATIENT-CLOCK` (step 15: the computable bounded EF evaluator — item 4's guard, **partial**) | `MarketComputation.totalQuote`, `.readyAtFuel`, `.denoteRatComp`, `.denoteRatComp_eq`, `.readyAtFuel_prim`, `.denoteRatComp_prim`; `EF.denoteRatWith_congr`; `EF.denoteRatWithAtFuel_isSome_of_some` (`Construction/M7Witnesses.lean`) | done (blocks assembly; not the full checker) | **P/C** `(a,b)` | **The correctness-critical half of item 4** (see `notes/next-session.md`): making `valueRatAtFuel`'s inner `EF.denoteRatWithAtFuel` computable, which the whole settlement check rests on. Rather than compile a *fourth* `EF`-code recursion, reuses the total EF rational machine `efRatCompiledEval` (`LIACompiler.lean`, now public) with the **total** table `totalQuote fuel n φ := (market.quoteAtFuel fuel n φ).getD 0` — the exact reuse the note prescribed. `denoteRatComp := if readyAtFuel then some (efRatCompiledEval totalQuote fuel e) else none`, and `denoteRatComp_eq` proves it **equals** the partial `denoteRatWithAtFuel market fuel e []`. `denoteRatComp_prim : Primrec₂` closes computability via `efRatCompiledEval_prim` (`C := ℕ`) + `readyAtFuel_prim`. **Why the guard is load-bearing, proved not asserted:** the ready branch uses `denoteRatWithAtFuel_complete` `(a)` (hypothesis `∀ q ∈ priceQueries, …`) + a fresh `denoteRatWith_congr` (the table matters only at `priceQueries` leaves); the not-ready branch uses a fresh `denoteRatWithAtFuel_isSome_of_some` (success ⇒ every query terminated) so a timeout forces `none` — this is precisely what stops the total table's timeout/genuine-`0` conflation from certifying a false test. `efPriceQueries_prim` (step 14) is what makes `readyAtFuel` primrec. `list_all_eq_foldr` (private, same file) reused, not duplicated. M7Witnesses green at 2,467 jobs, capstones axiom-clean. **Still owed for the node (item 4 remainder):** fold `denoteRatComp` into `Computable valueRatAtFuel` (over the affine term list, `w := bitsPayoutRat l`), then `Computable settlementCheckAtFuel` (double `allBitLists` fold), then `Partrec.rfindOpt` + `Nat.Partrec.Code.exists_code` → `SettlementChecker.spec` → a `SettlementChecker.ofComputations` constructor. Plus the disclosed `PolySequence As → Primrec As` bridge on `ofComputations`. |
...
- **M7-PREFIX-PATCH (complete 2026-07-18):** `liaEfficientPrefixPatch` inhabits
  `EfficientPrefixPatch (liaHistory DP) cutoff`. `EF.freezeBefore` uses a dead
  administrative binding to retain each old price leaf, preserving rank and parser failure
  while increasing structural cost by at most a factor of three. The flat transducer is
  proved correct on canonical and arbitrary malformed raw streams; the generic
  `PrefixPatchCompile.freezeBefore_preserves_ec` compiler then consumes an exhaustive
  polynomial raw-code quote lookup. The LIA instantiation hardcodes the finite
  `liaStatePrefix`, matches every noncanonical sentence code accepted by Foundation's
  decoder, and emits the exact rational entry quote. This closes the concrete LIA gap while
  leaving the paper erratum for arbitrary computable markets explicit.
...
| `thm:ifp` | Closure under Finite Perturbations | `EF.freezeBefore`; `Strategy.freezeBefore`; `Trader.freezeBefore`; `freezeBefore_netWorth_difference_le`; `Trader.Exploits.of_boundedDifference`; `EfficientPrefixPatch`; `lic_iff_of_finitePerturbation`; `liaEfficientPrefixPatch` | **done as a conditional biconditional that is STRICTLY WEAKER than the paper's `thm:ifp`; concrete LIA compiler discharged** | **P/C** `(a)` + **paper erratum**; the semantic false-report transformation is represented by a parser-transparent administrative binding: it preserves rank and raw parser failure, has structural cost at most three times the source, has exact tail semantics, and differs in net worth by an explicit finite sum of magnitudes. Bounded error transports genuine bounded-downside/unbounded-upside exploitation in both directions. **Reclassified 2026-07-16 (was `(a,c)`, "exact conditional biconditional", "a real model mismatch"): this is not a `(c)` substitution — our `ComputableMarket` matches `def:marketprocess` exactly (computable sequence of pricings, no finite support: TeX line 681, and line 995 confirms the generalization is deliberate). The defect is in `app:ifp`, which asserts the freeze is e.c. because "only finitely many constants are needed" — false. See "Paper errata".** `EfficientPrefixPatch` is therefore not a routine universal obligation: it is **uninhabited for some legal markets**, so `lic_iff_of_finitePerturbation` does not cover every finite perturbation and must not be cited as `thm:ifp`. For `LIA`, `liaEfficientPrefixPatch` compiles the fixed finite state prefix using exhaustive noncanonical sentence-code matching and proves `preserves_ec`; it contains no exploitation or LIC conclusion. The 2,467-job affected roll-up and 2,680-job full build are green; source/diff checks pass and both new capstones print only the approved axioms. |
