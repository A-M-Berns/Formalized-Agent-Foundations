...
| M4 | Affine master + reusable LUV lift; close expectation/Self-Trust statements | **in progress (2026-07-13)** — trust-surface audit repaired the seven parked signatures. The affine core and both Boolean and continuous repeatable-ROI budgeters are proved. `EF.var`/`EF.letE` provide faithful shared straight-line syntax. Sparse/frequently-positive magnitudes are supported. The affine limit hub and continuous buy/gradual-sell component are kernel-checked. `AffineCombination.PolySequence` now exposes polynomial term boundaries, coefficient streams, sentence codes, rank legality, and closed denotation; it uniformly emits cross-time prices, magnitudes, launch risks, gradual occupancies, sale fractions, and the complete gradual trader family. `EfficientlyEmulatable.of_polySeg` closes the paired-family raw-program bridge, and `PolySequence.gradualRisk_converges` consumes `noFractionalRepeatableReturn` to prove the family-level continuous ROI consequence. **Audit correction:** polynomial maturity cannot be derived from merely partial-recursive market/process certificates; the continuous route avoids that false gate. Remaining affine gate: add a finite-prefix launch gate so eventual future-price hypotheses suffice, then derive `AffineNoPreemptiveGaps` and apply `affpolymax_of_noPreemptiveGaps`. After that, consume the hub in the seven expectation/Self-Trust statements. |
...
| `thm:affpolymax` (analytic + continuous family construction hubs) | `NoPreemptiveUnderpricing`, `NoPreemptiveOverpricing`, `affineFutureHigh`, `affineFutureLow`, `BoundedAffinePrices`, `affpolymax_of_noPreemptiveGaps`; `AffineCombination.PolySequence`; `gradualRemaining`, `gradualOccupancy`, `gradualFamily`, `gradualFamilyPolyTrade`, `gradualRisk_converges` (`Affine.lean`, `Properties/AffinePreemptiveLearning.lean`) | **done through family-level continuous ROI; operational no-gap wrapper pending** | **P** | Exact limit equalities, gradual-sale economics, continuous capital recycling, all uniform syntax emitters, structured family emulation, and the logical-inductor contradiction forcing launch risk to zero are kernel-checked. Remaining: gate the finitely many indices preceding an eventual future-price condition (zero entry/occupancy/trades there), derive both operational no-gap conditions, and invoke the analytic capstone. Axiom-clean. |
