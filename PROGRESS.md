...
| M4 | Affine master + reusable LUV lift; close expectation/Self-Trust statements | **in progress (2026-07-13)** — trust-surface audit repaired the seven parked signatures and restored computable rational-market/process certificates on `IsLogicalInductor`. The affine core and repeatable-ROI budgeter are proved. `EF.var`/`EF.letE` provide faithful shared straight-line syntax; the shared `β₀…βₙ` program is denotationally correct, rank-legal, non-duplicating, and uniformly token-emittable. Sparse/frequently-positive magnitudes are supported. `VerifiedMaturitySchedule` now turns a polynomial, sound, eventually successful checker into both the semantic maturity schedule and exact polynomial openness table by bounded prefix search; it never assumes the first maturity day is itself polynomially computable. The remaining domain-specific gate is to construct such a checker for the concrete affine traders from the computable market/process certificates, then consume it in `thm:affpolymax` and the seven expectation/Self-Trust statements. |
...
| `thm:affpolymax` (analytic hub) | `NoPreemptiveUnderpricing`, `NoPreemptiveOverpricing`, `affineFutureHigh`, `affineFutureLow`, `BoundedAffinePrices`, `affpolymax_of_noPreemptiveGaps` (`Properties/AffinePreemptiveLearning.lean`) | **done conditional on affine no-gap construction** | **P** | The exact two paper equalities are kernel-checked from the operational threshold conditions produced by affine ROI: eventual separated future highs rule out frequent current lows, and dually for future lows/current highs. Uniform affine price boundedness supplies every `liminf`/`limsup` boundedness side condition, and the diagonal-vs-tail `sSup`/`sInf` inequalities are proved. No limit algebra remains in the construction task. Still required for the unconditional `[IsLogicalInductor]` theorem: the uniformly polynomial continuous buy/gradual-sell family deriving `AffineNoPreemptiveGaps`. Axiom-clean. |
