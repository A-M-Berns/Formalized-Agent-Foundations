...
| M7 | budgeter, trading firm, LIA, existence, unconditionalization | **core construction complete; conditional+disclosed construction-scope endpoint closed 2026-07-18; consolidation/read-through/errata audit remain** — efficient-trader enumeration, the process-backed Budgeter, exact `TradingFirm`, dominance, recursive rational LIA, and the complete executable compiler are proved. `LIA_is_logical_inductor` and `exists_logical_inductor` are unconditional, use none of the fifteen post-M5 boundaries, and expose only `propext`, `Classical.choice`, and `Quot.sound`. Four realizability-critical boundaries are constructed; the other eleven are named, field-audited, cited, and disclosed in the README. M3–M5 remain explicitly conditional on `[IsLogicalInductor P DP]` and, where applicable, those representation interfaces. The public surface consolidation, deferred statement read-through, line-by-line M7 paper comparison, `M7-ERRATA-AUDIT`, and final release gate remain open. |
