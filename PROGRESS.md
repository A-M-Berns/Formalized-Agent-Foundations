...
| M4 | Affine master + reusable LUV lift; close expectation/Self-Trust statements | **in progress (2026-07-13)** — trust-surface audit repaired the seven parked signatures and restored computable rational-market/process certificates on `IsLogicalInductor`. The affine core and repeatable-ROI budgeter are proved. `EF.var`/`EF.letE` provide faithful shared straight-line syntax; the shared `β₀…βₙ` program is denotationally correct, rank-legal, non-duplicating, and uniformly token-emittable. Sparse/frequently-positive magnitudes are supported. `VerifiedMaturitySchedule` turns a polynomial, sound, eventually successful checker into the semantic maturity schedule and exact polynomial openness table by bounded prefix search. The affine preemptive-learning limit hub and its continuous buy/gradual-sell semantic component are now kernel-checked, including telescoping liquidation, exact magnitude, stabilized net worth, positive ROI from a protected price gap, and a partial-risk bound charging downside only to the remaining fraction. **Audit correction:** a polynomial maturity checker cannot in general be derived from the merely partial-recursive `ComputableMarket`/`ComputableDeductiveProcess` certificates. The affine family should instead use its polynomial, rank-legal `gradualRemaining` feature as continuous online capital occupancy. Remaining gates are to generalize the shared budget from Boolean openness to decreasing `[0,1]` occupancy features, uniformly emit the affine family, derive the operational no-gap conditions, then consume the hub in the seven expectation/Self-Trust statements. |
...
| `def:roi` / repeatable ROI | `HasROI`; `Trader.Matured`; `EfficientlyEmulatable`; `PolyTradeEmulatable`; `ROIBudget.weight`, `sharedFeatureWeight`, `sharedBudgetedTrader`, `repeatableROI`; `VerifiedMaturitySchedule`; `fractionalWeight`, `DecreasingOccupancy`, `fractionalWeight_nonneg_and_postAllocation_le` (`Engine.lean`, `ROI.lean`) | **Boolean hub done; continuous semantic budget started** | **Def+P** | Complete Appendix A.2-style Boolean closure. `sharedFeatureWeight` binds each `βᵢ` once and has semantic, rank, exact-cost, and uniform-emission proofs; the verifier bridge converts an independently supplied polynomial checker into the exact openness table. The new fractional analogue proves the same unit-capital invariant for any decreasing `[0,1]` occupancy and requires no maturity oracle. This is the correct affine specialization because `gradualRemaining` is decreasing, rank-legal price-feature syntax. Remaining continuous-budget work is its shared EF representation, uniform token emitter, recycling/exploitation theorem, and affine-family instantiation. |
...
| `thm:affpolymax` (analytic + semantic construction hubs) | `NoPreemptiveUnderpricing`, `NoPreemptiveOverpricing`, `affineFutureHigh`, `affineFutureLow`, `BoundedAffinePrices`, `affpolymax_of_noPreemptiveGaps`; `gradualRemaining`, `gradualSellFraction`, `gradualTrader`, `gradualTrader_netWorth_lower_partial`, `gradualTrader_hasROI_of_price_gap` (`Properties/AffinePreemptiveLearning.lean`) | **done conditional on family-level continuous-budget bridge** | **P** | The exact two paper equalities are kernel-checked from the operational threshold conditions produced by affine ROI. The concrete continuous component buys with a low-price ramp, sells fractions as the high-price ramp rises, telescopes to full liquidation, has exact total magnitude `2·entry·|A|`, stabilizes world-independently, and obtains positive semantic ROI from every protected gap. Its intermediate net worth is now bounded below by realized protected spread minus `remaining·|A|`; the affine constant is proved to cancel from risk. This supports a feature-driven budget with no external closure oracle. Still required for the unconditional `[IsLogicalInductor]` theorem: generalize the shared budget to continuous occupancy, uniformly emit the family, and feed its contradiction into `AffineNoPreemptiveGaps`. Axiom-clean. |
