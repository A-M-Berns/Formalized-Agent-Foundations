...
| `thm:ec` (D2) | feature-generic layer `buyIndF`/`sellIndF`/`hystChain` (+ facts 1–3, variation `hcDelta`/`hcBpos`/`hcBneg`/`hcBneg_unbounded`), `LUV.thresholdSumEF`/`expectEF`, gated signals `excPad`/`excBuy`/`excSell`, bundle trader `excTrader`, `excTrader_netWorth_ge`, `excBneg_unbounded`, `excTrader_exploits`, `excTrader_ecTok` (**sorry**), **`LUV.expect_converges`** (`Properties/ExpectationConvergence.lean`) | **exploitation done; e.c. cert `sorry`** | **C** | **Expectations Converge**: `𝔼ₙ(X)` converges for every `[0,1]`-LUV. `thm:con`'s hysteresis re-run on the expectation feature (`𝔼ₙ(X)` is an `EF` — rational multiple of a price sum), trading the **day-`n` threshold bundle** `{(1/n)·⌜X>i/n⌝}_{i<n}`. Bundle payout in a world valuing `X` at `x` is within `1/n` of `x` (`lem:conluvapprox`), so the C2 accounting gains an error `≤ (2B₋+1)/n₀`; the trader is **gated to start at `n₀` with `2/n₀ ≤ γ/2`** (B2's `δ=0` padding trick), keeping coefficient `γ/2 > 0` on `B₋`. **Statement gained two hypotheses vs the old sorried form** (disclosed): `hcons` (daily plausible worlds) and `hval` (worlds assign `X` values — the type-`(c)` import of "Θ represents computations"; its principled witness is M7). The old `Expectations.lean` sorry is deleted; `expectInf` re-homed. Feature-generic hysteresis layer (`hystChain` over abstract signal families) added *alongside* C's concrete file — `thm:con`'s certs untouched. Exploitation axiom-clean (`#print axioms excTrader_exploits`); `excTrader_ecTok` is the disclosed `sorry` (same three pieces as `thm:nd`'s ladders **plus** a new one: `⌜X.gt (i/n)⌝` sentence-code emission needs an encodability interface on the `LUV` threshold family — flagged in the docstring for the cert session) |
...
| `thm:ei` | `lic_expectation_indicator` | **stmt (sorry)** | — | `𝔼ₙ(1(φ)) ≈ₙ Pₙφ` over any `IsIndicator` family. Per-threshold `thm:lex` does **not** suffice (threshold set grows with `n` — needs a bundle trader, the D2 shape). Proof → M4 per G1 |
| `thm:loe` | `lic_linearity_of_expectation` | **stmt (sorry)** | — | fixed `X,Y,Z`; `Θ ⊢ Z = aX + bY` rendered as world-level `ValuesAt` linkage. `𝓔𝓒`-sequence form is the M4 target. Proof → M4 (affine hubs) per G1 |
| `thm:expprovind` | `lic_expectation_provind` | **stmt (sorry)** | — | single-LUV form: plausible worlds value `X ≥ c` ⇒ `𝔼(X) ≳ₙ c`. Proof → M4 per G1 |
...
| `thm:ccee` | `lic_no_expected_net_update_conditional` | **stmt (sorry)** | — | the `w`-weighted form; product LUVs `Z`/`Z'` linked pointwise to `X`'s world-values (`hX` non-triviality + `hZ`/`hZ'` product linkage). `w` carried as a `[0,1]` sequence — paper-side it is P-generable (`def:pgen`, an M4 object); **the P-generability hypothesis gets added at proof time** (stated form is the stronger claim; disclosed in the docstring). Proof → M4 |
...
| `thm:con` (arbitrage) | `oscillation_exploitable` | **done** | **C** | states: a rational oscillation (+ daily plausible world) admits an **e.c. trader that exploits**. **No longer blocked by a trust-surface limit** — OPEN RISK 4 is resolved, so the **hysteresis** exploiter (buy `<a`, hold until `>b`, sell; banks `b−a` per swing regardless of smoothness) is now admissible: its "am I holding?" state is a size-`Θ(n)` linear-depth `EF`, which `EfficientlyComputableTok` (poly-*size*) admits. Bounded-depth alternatives still provably fail. What remains is **the economic construction only** — the e.c. tooling is now complete (`ifzSel`/`subc`/`ecTok_of_tokenFn`, validated by `deepTrader_ecTok`): build the running-state hysteresis `EF`, prove it banks `≥ b−a` per swing, discharge e.c. mechanically via `ecTok_of_tokenFn`, feed to `exploits_of_ge_partialSums`. Per Rule 1 the `sorry` stays until that trader is built |
...
| `thm:nd` (positive direction, B2) | `armChain` (+`_mem/_denote_of_le/_shares_sum/_rank`), `ndThr`, `ndPadThr`, `ndBuySig`, `ndShares`, `ndCoef`, `ndLadderEF`, `ndLadderTrader`, `ndLadderTrader_exploits`, `ndLadderTrader_ecTok` (**sorry**), **`lic_nonDogmatism`** (`Properties/NonDogmatism.lean`) | **exploitation done; e.c. cert `sorry`** | **C** | **full Non-Dogmatism, `Θ ⊬ ¬φ` side**: G3's hypothesis ⇒ `∃ ε > 0, ∀ᶠ n, ε ≤ Pₙφ`. **No price-range hypotheses** (the ladder's economics localize to its trigger bands). Trader = the paper's `app:obu` **scale-ladder** (sketch at `main.tex:1533`), *not* the plan's §6 recursive budget trader — that trader's state update uses the state twice (bare + inside the clip) ⇒ its `EF` **tree is exponential**, and no single-occurrence chain expresses it (single-occurrence recursions are monotone-or-antitone in the state; the budget update isn't). **Disclosed rescaling (dd:fuel):** paper constants `2^{-j}`/`j2^j` have exponential-*value* encodings under the fuel clock; rung `j` here buys ≤ `j³` shares below `1/j³` at weight `1/j²` (coefficient constant `j`), spend ≤ `Σ1/j² ≤ 2`, fired rung banks `≥ j−1`. Rungs padded with degenerate `δ=0` ctsind factors (identically 0, `1/0=0` in ℚ) so all rung chains have uniform width. **`ndLadderTrader_ecTok` is the one `sorry`** — trader is genuinely poly-size (`Θ(n²)` tokens, poly-value constants) but the cert needs: runtime-divisor `divmod`, `PolySegStream.concat` (n-fold), and poly-fueled emission of rung-varying `ℚ`-constant tokens (`⌜ndThr j⌝` from `j`) — the plan's B2 growing-width decision point plus a new finding: the paper's "efficiently emulatable sequences" (`app:preliminaries`.3) certify by **dynamic programming**, which the `EF`-tree `dd:dsl` doesn't share. Exploitation itself axiom-clean (`#print axioms ndLadderTrader_exploits`); `lic_nonDogmatism` carries `sorryAx` through the cert, disclosed in its docstring |
| `thm:nd` (dual direction, B2) | `ndSellSig`, `ndSellShares`, `ndSellCoef`, `ndSellLadderEF`, `ndSellLadderTrader`, `ndSellLadderTrader_exploits`, `ndSellLadderTrader_ecTok` (**sorry**), **`lic_nonDogmatism_dual`** (`Properties/NonDogmatism.lean`) | **exploitation done; e.c. cert `sorry`** | **C** | **full Non-Dogmatism, `Θ ⊬ φ` side**: `φ`-falsifying plausible worlds keep existing (G3's rendering of `Θ ⊬ φ`) ⇒ `∃ ε > 0, ∀ᶠ n, Pₙφ ≤ 1 − ε`. Mirrored **sell** ladder over `sellIndEF` (spike band above `1 − 1/j³`) — *not* the positive direction applied to `∼φ` (prices of `φ`/`∼φ` are unlinked without coherence). Same `armChain` engine, same padding, same three-piece e.c. gap (`ndSellLadderTrader_ecTok` `sorry`). Exploitation axiom-clean; `lic_nonDogmatism_dual` carries `sorryAx` through the cert, disclosed |
| `thm:nd` (limit form) | **`lic_limit_pos`**, **`lic_limit_lt_one`** | done (carries the cert `sorryAx`) | **C** | the paper's stated form: with the price convergent (explicit hypothesis, supplied by `thm:con`'s `lic_price_convergesTo` in context), `P∞(φ) > 0` resp. `P∞(φ) < 1`. Pure limit-passage corollaries (`ge_of_tendsto`/`le_of_tendsto`) of the two eventual-bound theorems |
