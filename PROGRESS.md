...
| M3 | Downstream property slice + LUV bridge + integration test | **in progress** — proved: `thm:provind` (fixed φ, limiting `→1`), `thm:lc` bullet 2 (`→0`); LUV/expectation bridge (`E^H_n(X)` concrete); integration test (deference interface verified at price & expectation level). **`thm:con` reduction landed** (non-convergence ⇒ rational oscillation, axiom-clean via Mathlib `tendsto_of_no_upcrossings`); `thm:con` itself reduced to a single honest-`sorry` arbitrage lemma. **Responsive-trader e.c. infrastructure complete** (`Fueled` combinators → `PolyFueled` → `priceTrader_ec`). **`thm:lc` bullet 3 (finite additivity) proved** (world-neutral 3-sentence portfolio, e.c.-certified, axiom-clean) — all three coherence bullets now done in finite-stage/limit form. **Prec-fueled `pred` combinator landed** (`PolyEF.pricePred`), resolving the multi-day-trader e.c. gap. Remaining: the convergence arbitrage trader (needs *hysteresis* — the memoryless sketch was retracted; e.c. ingredient now in place), expectation family (`thm:ec/loe/ei/expprovind`), `thm:nd`, Self-Trust. Two disclosed `sorry`s (`thm:ec`, `oscillation_exploitable`). |
...
| `thm:con` (arbitrage) | `oscillation_exploitable` | **sorry** | — | **the genuine remaining work**, honestly ledgered. States: a rational price oscillation (+ daily plausible world) admits an **e.c. trader that exploits**. **Correction (this session):** the memoryless target-holding `T(Pₙφ)−T(P_{n-1}φ)` sketch is *retracted* — its net worth `≈ ∫T dP` is path-independent (a state function of price), hence **zero over a closed cycle**, so it does *not* exploit. The real arbitrage needs **hysteresis** (buy at `<a`, hold with memory until `>b`, sell) — the "discontinuous trader" subtlety the paper flags and sidesteps in `app:con` via `thm:tbo`. Ingredient status: e.c. **resolved** (day-`(n-1)` feature via `PolyEF.pricePred` — see `dd:fuel (pred)` row); the exploitation inequality (bounded-below ∧ unbounded-above for the hysteresis `EF`) is the remaining hard core |
