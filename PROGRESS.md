...
| `thm:con` (arbitrage) | `oscillation_exploitable` | **sorry** | — | **the genuine remaining work**, honestly ledgered. States: a rational price oscillation (+ daily plausible world) admits an **e.c. trader that exploits**. Intended construction fully specified in the docstring: continuous target-holding `T(x)=clamp₀₁((b−x)/(b−a))`, day-`n` coeff `c_n = T(Pₙφ)−T(P_{n-1}φ)` on `φ`; closed round trips net `≥ b−a` per swing in **every** world. **Two surfaced obstacles** (not routed around): (1) the exploitation inequality is a real discrete-Stieltjes/arbitrage lemma; (2) e.c. needs a **prec-free `pred`** — this is the *first* property trader referencing two consecutive days' prices (`price φ n` and `price φ (n-1)`), and the current `dd:fuel` combinators (const/id/pair/succ_comp) have no predecessor (`Nat.pred` via `Code.prec` decrements `evaln`, breaking poly-fuel composition) |
