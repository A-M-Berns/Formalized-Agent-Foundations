...
| `M7-PATIENT-CLOCK` (step 16: the computable value evaluator at a bit-list world) | `bitsPayoutRat_prim`; `affineTermsRatComp` (+`_eq`, `_prim`); `valueRatCompAt` (+`_eq`, `_prim`) (`Construction/M7Witnesses.lean`); `ratAdd_prim`, `ratMul_prim` un-`private`d (`LIACompiler.lean`) | done (item 4 assembly, cont.) | **P/C** `(a,b)` | Recasts `A.valueRatAtFuel market fuel (bitsPayoutRat l)` — the quantity `settlementCheckAtFuel` compares across worlds — through the computable `denoteRatComp` (step 15), so the whole check becomes primitive recursive. `valueRatCompAt_eq`/`affineTermsRatComp_eq` prove exact equality to the originals `(a)` (per-term `denoteRatComp_eq` rewrite; `map`/`bind` reconciled by casing on the `Option`s). `valueRatCompAt_prim : Primrec` via `list_foldr` over `A.terms` with `ratAdd_prim`/`ratMul_prim` (un-`private`d, **not** duplicated) + `bitsPayoutRat_prim` (from `evalBits_prim` `(b)`) + `denoteRatComp_prim`. **The `Nat.sqrt`/`whnf` GOTCHA fired here** — the first time across steps 5–16 — on the deep `list_foldr` step input `((AffineCombination × ℕ) × List Bool) × ((EF × Sentence) × Option ℚ)`; diagnosed (not heartbeat-bumped) and fixed with the documented scoped `attribute [local irreducible] Nat.sqrt`, vindicating the handoff's "diagnose first" guidance. M7Witnesses green at 2,467 jobs, capstones axiom-clean. **Still owed (item 4 remainder):** `settlementCheckAtFuel_prim` (double `allBitLists` fold + `stageAtFuel`/`stageSatBits`/`settlementAtomLimit` leaves, all in hand, over the `valueRatCompAt` comparison), then `Partrec.rfindOpt` + `Nat.Partrec.Code.exists_code` → `SettlementChecker.spec` → `SettlementChecker.ofComputations`; plus the disclosed `PolySequence As → Primrec As` bridge. |
