...
- **✅ OPEN RISK 4 — RESOLVED (2026-07-08) by the token-indexed `def:ec`.** The redefinition
  ...
  layer; the deep `exclTr`/`eqTr`/`impTr` via `serialize`-tree combinators), sorry count and
  axiom footprint unchanged. **Residual (small, disclosed, type-`(c)`):** each token's *value* is
