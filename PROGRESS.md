...
| `def:roi` / repeatable ROI | `HasROI`; `Trader.Matured`; `EfficientlyEmulatable`; `PolyTradeEmulatable`; `ROIBudget.weight`, `sharedFeatureWeight`, `sharedBudgetedTrader`, `repeatableROI`; `VerifiedMaturitySchedule` (`Engine.lean`, `ROI.lean`) | **done — semantic + faithful e.c. hub** | **Def+P** | Complete Appendix A.2-style closure. `sharedFeatureWeight` binds each `βᵢ` once and has semantic, rank, exact-cost, and uniform-emission proofs. `sharedBudgetedTrader_polySeg` performs nested variable-width trade/component concatenation and `sharedBudgetedTrader_ecTok` closes `def:ec`. Semantically, closed tails are charged to a summable tolerance and current downside is bounded by one unit. The verifier bridge uses a single primitive-recursive bounded scan (`polyFueled_boundedAny`/`boundedNone`) to convert a sound, eventually successful polynomial checker into the exact openness table. This avoids both noncomputable openness and the false stronger demand that the first verified day be polynomially computable. |
