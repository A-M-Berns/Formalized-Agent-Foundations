...
| `M7-PATIENT-CLOCK` (step 4/4: the clock, constructed) | `SettlementSemiDecider`; `PatientSettlementClock.ofSemiDecider`; `deadlineRun`(+`_eq`/`_mono`); `deadlineStep`; `deadlinePassed`(+`_eq_true_iff`/`_sound`/`_mono`/`_eventually`); `polyFueled_deadlinePassed`; `acceptsWithin_mono`; `dovetailFound_mono`; `deferralEnvelope_lt_of_forall` (`Construction/M7Witnesses.lean`) | **done modulo one named computability obligation** | **C/P** `(a)` | **`PatientSettlementClock` is now constructed, not assumed** — all six fields discharged, no `sorry`, axiom-clean. `active i n := ¬deadlinePassed ∨ ¬dovetailFound`. The one remaining input is `SettlementSemiDecider`: a code accepting `⟨i,j⟩` iff `As i` is settled at stage `j`. It is a **pure computability obligation** — no market, economic, limit or pseudorandomness content; soundness only reads off settlement. Its realizability is *proved* by step 2 (`agree_of_finiteWorlds_agree`: decidable on a rational market) and step 1 (`exists_settled_stage`: settlement occurs); what remains is only to exhibit the `Nat.Partrec.Code`, i.e. `Computable` plumbing through `MarketComputation.quoteAtFuel` / `DeductiveProcessComputation.stageAtFuel` / `PolySequence`. **Deadline subtlety** (real, and resolved): `DeferralFunction` gives fuel poly in `f n`, *not* `n`, so `deferralEnvelope f i` is not poly-computable and the clock cannot decide its deadline exactly; `active_through_envelope` only needs activity *true* before the deadline, so the sound under-approximation `deadlinePassed` suffices. |
