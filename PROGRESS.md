...
| `dd:fuel` (bridge) | `IsPolyBounded` (+`of_le`/`linear`/`max`), `EfficientlyComputable.of_fueled`, `pair_lt_sq` | done | **P** | turns a poly-bounded `Fueled` fact for a trader's encoding-function into `def:ec`. With the combinators, responsive-trader e.c. is now assemble-code → poly-bound → apply. Remaining per-family: `IsPolyBounded` pair/comp closure + specific codes (mechanical) |
