...
- **instantiated** — additionally instantiated over the constructed `LIA`, with the
  ...
  below; `[𝗣𝗔⁻ ⪯ T]` is a genuine small strengthening, disclosed globally with its charging
  question pending a user ruling. Read the status column, not the axis,
...
* **`dd:proofcode` on §4.10's finite proof search** *is* a genuine type-`(c)` substitution, and
  is disclosed here rather than charged per row: `Con(Θ′)(ν)` is read as “no `Θ′`-derivation of
  `⊥` has Gödel code below `ν`”, where the paper writes “≤ `ν` **symbols**”, because Foundation's
  internal derivations expose no size function (`Semiformula.bv` measures a formula, not a
  derivation). It changes *which* finite search each day names and nothing else — decidability,
  the `def:ec` emission and the truth of every instance are proved from consistency alone
  (`conWithin_of_consistent`), with no relation between the two measures assumed. **Retiring with
  tranche 9a**, the queued Foundation symbol-measure work, after which the same statements hold
  with `ν` a symbol bound. Charged once here, on the `dd:fuel` precedent; it is why `thm:pac`
  and `thm:pazfc` read `exact` rather than `qualified`. It does **not** reach `thm:incons`,
  whose sentence is the *unbounded* existential over proofs: nothing is metered there, so the
  substitution does not arise.
...
fields of the two structures since retired — to `BigDigits`, and, in tranche 9-pre, `thm:incons`'s
theory sequence `⟨Θ′⟩` to `PolyArithmeticSourceSeq` on the **written source** of the day's
adjoined axiom, the last hypothesis on paper-quantified data that was still metered by a
...
* the **closed / `_unconditional`** layer over the constructed `LIA` is stated over
  ...
  consistency itself; `thm:incons` adds a second theory `T'` with `[T'.Δ₁]` and, since
  T8/iii, nothing else (its redundant `[𝗥₀ ⪯ T]` was dropped); `thm:lp` adds `[𝗜𝚺₁ ⪯ T]`, one of the
...
**`[T.Δ₁]` (disclosed 2026-08-30, R5-F15).** The endpoints over a theory require a
`Δ₁`-definable axiom set, strictly stronger than the paper's "computably enumerable" as a
condition on the presented `T`. Craig's trick supplies a deductively equivalent `Δ₁`
axiomatization of any c.e. theory, and every row is a `T ⊢`-statement, so the theorems
transfer — but that transfer is not formalized. Charged once, globally, as representation
infrastructure (the enumeration of `T`'s theorems); it does not lower a row on its own.
Judgment call, recorded.

**`[𝗣𝗔⁻ ⪯ T]` — a genuine strengthening, disclosed 2026-08-30 (blind audit, round 6).**
`𝗣𝗔⁻` is **not** implied by "Θ represents computations", and an earlier edition of this
file asserted that it was. The paper's premise yields `Θ ⊬ n̄ = m̄` for `n ≠ m` but never
`Θ ⊢ n̄ ≠ m̄` — that is `𝗥₀`'s Ω₃ — and Robinson's **R** represents every computable
function while not containing `𝗣𝗔⁻`. So this binder, carried by **17** of the 105 canonical
endpoints (census below), is a finite set of ordered-semiring axioms strictly beyond the
paper's premises.

It is load-bearing for exactly two things, and both are consequences of choices made
elsewhere in this development:

* `provable_subst_iff_of_val` — the compact `binNumeral` spelling of the claim argument.
...
* `code_uniq`'s `rfind` case — object-level exclusivity of the two quotation fibers at
  `theoremDP_hworld`'s tag 7. This is the **price of avoiding Σ₁-soundness there**. The
  paper's own exclusivity argument is *metatheoretic*, running through the representability
  biconditional and needing no arithmetic inside Θ at all; ours is object-level, so that the
  stage-world proof stays constructive.

**Whether this is charged globally (like `[T.Δ₁]`) or per row is pending a user ruling.**
The rows below carry the clause in their residual text and are otherwise classified on
their own merits; no row was promoted or demoted on account of `𝗣𝗔⁻`.
...
footing: disclosed here, not levied against the row. The two literal-`PaperLUV` frontends
...
| `𝗣𝗔⁻ ⪯ T` | **17** — every closed arithmetic-theory endpoint except the two literal-`PaperLUV` frontends (which carry the stronger `𝗜𝚺₁ ⪯ T`) and `thm:scon`'s two, which carry `[T.Δ₁]` alone |
...
`theoremDP_hworld`'s **tag 7**, the quotation lane (migrated 2026-08-30). Every other
...
What made tag 7 need it was the *shape* of the quotation schemas, not a missing proof.
...
(`code_uniq`, revived from Foundation's commented-out block) plus Gödel completeness. Tag 7
...
`Entailment.Consistent Θ` (`paperTheoryDP_nonvacuous`). The unbounded lane needs even less:
...
`luv_wubexp_ofComputation_unconditional` — all of which reached the instance through tag 7,
...
1. The endpoint carries a hypothesis the paper does not take. Σ₁-soundness was the standing
   ...
   above: both are representation infrastructure disclosed globally. `[𝗣𝗔⁻ ⪯ T]` **does**
   trip it as stated — it is a real strengthening beyond the paper's premises — and whether
   it is charged globally or per row is **pending a user ruling**; the rows below carry the
   clause and their status is otherwise re-derived.
...
* **Still `qualified`** — `thm:incons` alone, on **one** charge, on **mathematical** grounds
  that have nothing to do with theory strength: its theory sequence is the *deduction family*
  `Θ′ₙ = Θ₀ ∪ {σₙ}` over a fixed Δ₁ base rather than an arbitrary e.c. sequence of
  recursively axiomatizable theories, because Foundation's derivability predicate takes its
  theory as a **meta** parameter and there is no uniform-in-theory-code derivability to
  represent. The day's theory is genuinely named in the day's sentence (by the *written name*
  of the day's adjoined axiom, `ArithSource.sourceNat sₙ`, which the deduction theorem makes
  sufficient), the premise `∀ n, ¬Consistent (σ n ∷ Θ₀)` is the paper's own, and the paper's
  subject matter is exhibited (`𝗜𝚺₁ ∪ {⊥}`, and beside it `deepInconsistentSource`, whose
  day-`n` axiom is `5n + 7` written tokens and `≥ 2^n` compiled nodes and which takes a
  different value on *every* day); the restriction on *which* theory sequences are covered is
  the disclosed part. That charge is backed by a **verified obstruction** (tranche 9b), not by
  a queued repair: Foundation's fixpoint blueprint for `Derivation` cannot host satisfaction
  over coded formulas, so full uniformity in the theory code is a truth-predicate project. A
  *middle* rendering — uniformity over a coded family of Δ₁ axiomatizations — is an optional
  upstream item, **pending the user's ruling**. The **second** charge this bullet used to
  carry is **retired (tranche 9-pre)**: the `def:ec` premise was
  `hσ : BigDigits (deductionFamilyArg σ)`, metering the day's theory name by the base-4 digit
  count of a formula's *Gödel code* rather than by its source length; it is now
  `hs : PolyArithmeticSourceSeq s` on the **written source** of the day's adjoined axiom, with
  the Gödel code recovered inside the represented predicate, where the paper asks only for
  recursive enumerability. Its row says exactly what is and is not formalized.
  ...
  consistency premise and a discharged `𝗜𝚺₁`/`𝗣𝗔` witness — so the only thing left standing
  between either and its printed statement is the globally disclosed `dd:proofcode` metering
  substitution. The `rfl` identity between the two nodes has been deleted and is false: the
...
| thm:epr | exact | instantiated | unconditional over `LIA` at `def:ec`'s own **write-out** class: the quote code is constructed from the market program (`theoremPriceQuoteCode`), leaving exactly `φ` and `BigSentenceCodes φ`. (`BigSentenceCodes` is the write-out class; `RpnSentenceCodes` is the per-token-value-metered one, and an earlier edition of this row called this hypothesis symbol-metered and then named the write-out class.) **Theory hypotheses, settled 2026-08-30.** The elaborated binder list is `[T.Δ₁] [𝗣𝗔⁻ ⪯ T] [Entailment.Consistent T]`. Consistency is the paper's own standing assumption on Θ (tex:600-606, tex:993-997) and `[T.Δ₁]` is representation infrastructure charged once under *Arithmetic-theory hypotheses* above. **`[𝗣𝗔⁻ ⪯ T]` is a genuine, if small, strengthening** — it is *not* implied by "Θ represents computations" (Robinson's **R** represents every computable function and does not contain `𝗣𝗔⁻`), and it is spent here on the compact-`binNumeral` value transfer `def:ec` forces and on the object-level fiber exclusivity that replaced Σ₁-soundness at tag 7. It is disclosed globally above; **whether it is charged globally or against this row is pending a user ruling**, and this row's status is otherwise re-derived without reference to it. Two hypotheses were shed to get here. `[T.SoundOnHierarchy 𝚺 1]`, which the paper explicitly declines (tex:2673), was load-bearing only through `theoremDP_hworld`'s tag-7 fiber-exclusivity step, and that step is now a theorem of Θ — the positive and negative quotation schemas are the value-`1`/value-`0` fibers of one Foundation `code` formula, so `Θ ⊢ ∼(pos ⋏ neg)` follows from `code_uniq` plus Gödel completeness. The residual `[𝗜𝚺₁ ⪯ Θ]` that then held this row at `qualified` was **deleted outright** in tranche 7, with `QuotationTheoryPresentation.theory_sigmaOne`: the endpoint had inherited it from that field and from no step of its own, and now spends only `𝗣𝗔⁻`. The `_closed` form is the only endpoint shown for this label, and it is now at once closed over `LIA` and at the paper's own theory hypotheses; nothing is charged against the row |
| thm:er | exact | instantiated | unconditional over `LIA` at `LUV.RpnThresholdCodeSeq`; the expectation quote code is constructed via `expectQuote_computable`, leaving exactly `X` and its threshold codes. **Theory hypotheses, settled 2026-08-30.** The elaborated binder list is `[T.Δ₁] [𝗣𝗔⁻ ⪯ T] [Entailment.Consistent T]`. Consistency is the paper's own standing assumption on Θ (tex:600-606, tex:993-997) and `[T.Δ₁]` is representation infrastructure charged once under *Arithmetic-theory hypotheses* above. **`[𝗣𝗔⁻ ⪯ T]` is a genuine, if small, strengthening** — it is *not* implied by "Θ represents computations" (Robinson's **R** represents every computable function and does not contain `𝗣𝗔⁻`), and it is spent here on the compact-`binNumeral` value transfer `def:ec` forces and on the object-level fiber exclusivity that replaced Σ₁-soundness at tag 7. It is disclosed globally above; **whether it is charged globally or against this row is pending a user ruling**, and this row's status is otherwise re-derived without reference to it. Two hypotheses were shed to get here. `[T.SoundOnHierarchy 𝚺 1]`, which the paper explicitly declines (tex:2673), was load-bearing only through `theoremDP_hworld`'s tag-7 fiber-exclusivity step, and that step is now a theorem of Θ — the positive and negative quotation schemas are the value-`1`/value-`0` fibers of one Foundation `code` formula, so `Θ ⊢ ∼(pos ⋏ neg)` follows from `code_uniq` plus Gödel completeness. The residual `[𝗜𝚺₁ ⪯ Θ]` that then held this row at `qualified` was **deleted outright** in tranche 7, with `QuotationTheoryPresentation.theory_sigmaOne`: the endpoint had inherited it from that field and from no step of its own, and now spends only `𝗣𝗔⁻`. The `_closed` form is the only endpoint shown for this label, and it is now at once closed over `LIA` and at the paper's own theory hypotheses; nothing is charged against the row |
...
| thm:incons | qualified | instantiated | **What the paper asserts** (tex:1893-1903): for an **e.c. sequence of recursively axiomatizable inconsistent theories** `⟨Θ′⟩`, `ℙₙ(⌜⌜Θ′ₙ⌝ is inconsistent⌝) ≈ₙ 1` and hence `ℙₙ(⌜⌜Θ′ₙ⌝ is consistent⌝) ≈ₙ 0`, those two sentences being the universal generalization of `Con(Θ′)(ν)` and *its negation* (tex:1855-1866). **What the Lean theorem now proves:** the endpoint's subject matter is a *theory sequence*, not an arbitrary semidecidable predicate, and its `def:ec` premise is metered on the **written source** of the day's adjoined axiom. Its signature is `lic_disbelief_inconsistent_theories_unconditional (T : ArithmeticTheory) [T.Δ₁] [𝗣𝗔⁻ ⪯ T] [Entailment.Consistent T] (T' : ArithmeticTheory) [T'.Δ₁] (σ : ℕ → ArithmeticSentence) (s : ℕ → ArithSource 0) (hs : PolyArithmeticSourceSeq s) (hcompile : ∀ n, ArithSource.compile (s n) = ↑(σ n)) (hinc : ∀ n, ¬Entailment.Consistent (σ n ∷ T')) : ((fun n => liaHistory (paperTheoryDP T) n ((representedInconsistentTheoryClaims T T' σ s hs hcompile).inconsistencySentence n)) ≈ₙ fun _ => 1) ∧ ((fun n => liaHistory (paperTheoryDP T) n ((representedInconsistentTheoryClaims T T' σ s hs hcompile).consistencySentence n)) ≈ₙ fun _ => 0)`, and both paper conjuncts are concluded. `hinc` is **the paper's own premise**, stated at the day's *theory* — `σ n ∷ T'`, Foundation's `adjoin` — and not at any provability surrogate; `hs` is `def:ec` on the writing and `hcompile` says that writing denotes `σₙ`, the same (source, compiles) pairing `PaperLUVSeq` uses for LUV defining formulas. **One charge. (i) The deduction-family paraphrase.** The theory sequence is `Θ′ₙ := Θ₀ ∪ {σₙ}` over a fixed Δ₁ base `Θ₀ = T'`, not an arbitrary e.c. sequence of recursively axiomatizable theories. This is forced by the substrate: Foundation's `Derivation T` is `(construction T).Fixpoint` with `T` a **meta** parameter, so there is no uniform-in-theory-code derivability predicate to represent, and a sequence of theories cannot enter one sentence as an argument. For a deduction family the deduction theorem collapses the day's theory to a single sentence code (`Θ′ₙ` inconsistent ⟺ `Θ₀ ⊢ ∼σₙ` ⟺ `⌜∼σₙ⌝` is internally `Θ₀`-provable), which *is* uniform, so the day's theory is genuinely named in the sentence and the restriction is only on *which* theory sequences are covered. Disclosed at the endpoint docstring, at the section header and here. **This charge is now backed by a verified obstruction (tranche 9b), not by a queued repair.** Foundation's fixpoint blueprint for `Derivation` cannot host satisfaction over coded formulas, so there is no uniform-in-theory-code derivability predicate to be had at this level: full uniformity is a truth-predicate project, not a tranche. A *middle* rendering — uniformity over a coded family of Δ₁ axiomatizations rather than over arbitrary theory codes — is an **optional upstream item, pending the user's ruling**, and is not assumed here. The earlier reading that tranche 9b would *retire* this charge is withdrawn. **Charge (ii) is RETIRED (tranche 9-pre).** The premise was `hσ : BigDigits (deductionFamilyArg σ)`, metering the day's theory name by the base-4 digit count of a formula's **Gödel code**; Foundation's encoding pairs at every node, so that count is `~2^depth` and the premise admitted only `O(log n)`-depth families, excluding paper-admissible short-source/deep-parse ones. It is now `hs : PolyArithmeticSourceSeq s` — one token per symbol the writer writes, `def:ec` verbatim (tex:753-755, tex:1931-1933). What the sentence emits is `ArithSource.sourceNat sₙ`, the run read as a base-`64` numeral with a sentinel (`tokenListNat`, `Framework/CodeSource.lean`), whose base-4 digit count is `3·(sourceTokens sₙ).length + 3`, *linear in the written text* — exactly the `Code.sourceNat` doctrine the halting lane applies to machines. The Gödel code is recovered from that numeral **inside** the represented predicate by `negSourceFormulaCode` (`Construction/Witnesses/SourceNumbering.lean`), which is computable and therefore all `codeOfREPred` consumes; the paper asks only for recursive enumerability there, so the code's size is free. The class is strictly wider for it, and the widening is *exhibited*, not asserted. **`dd:proofcode` is not charged here.** Unlike `thm:pac`/`thm:pazfc`, this node's sentence is the *unbounded* existential over proofs — the paper's `⌜Θ′⌝ is inconsistent` is the negation of the universal generalization of `Con(Θ′)(ν)` (tex:1863-1866). Nothing is metered, so the symbol-count-versus-Gödel-number substitution does not arise. **Anti-extensionality (R5-F08 discipline).** The represented object is the **universal** provability schema of the base theory read at written names, `inconsistencySchema T' := codeOfREPred (fun z => ProvableCode T' (negSourceFormulaCode z))` — one schema per base theory, argument-sensitive, with `inconsistencySchema_mentions_zero` *proved* from `Consistent T'` rather than assumed (the two separating arguments are now the names of the written formulas `⊥` and `⊤`) — and the day's data written into the sentence as the compact numeral `binNumeral (sₙ.sourceNat)`. Representing “`T'` is inconsistent” directly would have been the trap: under `hinc` that predicate is constantly `True`. `hs` is load-bearing, being the only route to `inconsistency_poly`. **Non-vacuity is discharged by the construction, at unbounded day-variation.** The in-file `example` runs the theorem at market `𝗜𝚺₁`, base theory `𝗜𝚺₁`, source `falsumSource` (the one-token writing of `⊥`), so every day's theory is `𝗜𝚺₁ ∪ {⊥}` — *actually inconsistent*, the paper's own subject matter. Beside it, and this is what the re-metering buys: `deepInconsistentSource n`, the writing of `(∀x. A(x) ⟺ ⋯ ⟺ A(x)) ∧ ⊥` with `n` biconditionals — `5n + 7` emitted tokens (`sourceTokens_deepInconsistentSource_length`), `≥ 2^n` nodes once compiled (`two_pow_le_encode_iffChain`), and a Gödel code doubly exponential in *that*. It is `def:ec`-admissible on the nose (`deepInconsistentSource_polyArithmeticSourceSeq`), every day's theory is genuinely inconsistent (`deepInconsistentAxiom_inconsistent`), the endpoint is fully applied at it with nothing left to the caller (`thm_incons_applied_deep`), and the days are pairwise distinct *as written* by a length count (`deepInconsistentSource_sourceNat_ne`), so `inconsistencyArgClaimSentence_deep_ne` separates the claim sentences of **every** pair of distinct days. No hypothesis on codes can admit this family; the previous rendering could not state it. The R7 two-valued witness (`alternatingInconsistentAxiom`, `thm_incons_applied_alternating`) is strictly subsumed and **retired**, as is `deductionFamilyArg_ne_of_ne`, whose quote-injectivity argument is replaced by injectivity of the naming map (`ArithSource.sourceNat_ne_of_sourceTokens_ne`). The "missing `BigDigits.precBig` combinator" signpost is **withdrawn**: the family it was blocking is now admitted by the source route, and no code-digit theory is needed. **The sentence families collapsed to one.** `InconsistentTheoryClaims` was `inconsistencySentence consistencySentence inconsistency_poly consistency_poly inconsistency_provable consistency_disprovable`; it is now `inconsistencySentence inconsistency_poly inconsistency_provable`, with `InconsistentTheoryClaims.consistencySentence R n := ∼R.inconsistencySentence n` — the paper's own shape (tex:1863-1866). **Theory hypotheses: nothing charged.** `[T.Δ₁] [𝗣𝗔⁻ ⪯ T] [Entailment.Consistent T]` and `[T'.Δ₁]`; `[T.SoundOnHierarchy 𝚺 1]`, a residual `[𝗜𝚺₁ ⪯ Θ]` and a redundant `[𝗥₀ ⪯ T]` were all dropped in earlier tranches. `[T.Δ₁]`/`[T'.Δ₁]` are the global Craig's-trick charge and `[𝗣𝗔⁻ ⪯ T]` the standing representation-infrastructure binder, **whose charging is pending the user's ruling**; this row's status is re-derived without reference to it. The `𝗣𝗔⁻`-level witness is likewise pending — the exhibited example is at `𝗜𝚺₁`, for the same reason as the other §4.10 witnesses. **What holds the row at `qualified` is therefore charge (i) alone: the deduction-family paraphrase.** No count changes — the row was and remains `qualified`. *Superseded 2026-08-30 (T9p): the previous rendering metered `def:ec` on `BigDigits (fun n => ⌜∼σₙ⌝)`, a Gödel-code digit count, and recorded charge (i) as due for retirement by tranche 9b; see charge (ii), retired, and the 9b obstruction above.* |
