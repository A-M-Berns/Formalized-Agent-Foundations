...
| thm:ref | `lic_introspection_closed` (quote constructed; adds `PolyRatCodes` on `ā`, `b̄`); `lic_introspection` (no code restriction; quote as interface) |
...
| thm:ref | qualified | instantiated | unconditional over `LIA` at `RpnSentenceCodes`, with the interval quote constructed from the market's exact rational quote. **The restriction, which the previous prose omitted:** over and above the paper's ℙ-generability of `ā` and `b̄` (tex:1969-1971), `lic_introspection_closed` requires `ha : PolyRatCodes a` and `hb : PolyRatCodes b`, consumed by `theoremIntervalQuoteCode`. A ℙ-generable sequence of probabilities is in general **not** an efficiently computable rational sequence — its values are market prices — so this is a genuine narrowing of the paper's class. It is, however, a narrowing the paper's *own proof* needs and does not state: `app:ref` forms `ψₙ := ⌜⌜aₙ⌝ < ⌜ℙ⌝_⌜n⌝(⌜φₙ⌝) < ⌜bₙ⌝⌝` and applies `thm:affprovind` to an affine combination over `⟨ψ⟩`, which requires `⟨ψ⟩` to be e.c., which requires the numerals `aₙ`, `bₙ` to be efficiently writable. We judge the printed hypothesis set incomplete rather than the Lean statement wrong, and disclose rather than assert an erratum. The universal `lic_introspection` carries no such restriction and is shown second; it takes the interval quote as a caller interface instead |
