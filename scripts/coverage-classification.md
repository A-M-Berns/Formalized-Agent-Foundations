...
census over the 105 canonical endpoints *as of the round-13 unification* (the surface has
...
is the only surviving canonical occurrence of that name.) **Superseded as the paper
rendering (post-merge strengthening, 2026-09-02):** `thm:ccee` now also has
`lic_no_expected_net_update_conditional_paperLUV_closed`, which is exact, zero-slack, and
priced on the single market `liaHistory (paperDP T)` — so every *paper-rendering* canonical
endpoint names the single market, and the `canonicalCCEEDP` form is retained as a
generalized semantic-extension result with a wider source class.
