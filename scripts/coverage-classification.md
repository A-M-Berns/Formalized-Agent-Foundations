...
| thm:ccee | qualified | unconditional over `LIA` with both quoted products constructed, a bare `DeferralFunction`, and — as of the mesh product — the paper's **arbitrary** e.c. source family (`lic_no_expected_net_update_conditional_closed` takes `X : ℕ → LUV` with `RpnThresholdCodeSeq` + `source_valued`, the same premises `thm:cee` carries). What keeps the row below `instantiated` is a declared type-`(c)` substitution rather than a class restriction: the left quoted product is reflected only to within `1/(n+1)` (`ConditionalExpectationQuote.slack`), because an exactly-reflecting product LUV built from the source's *own* threshold atoms would need either the value of the deferred weight or an infinite disjunction, neither available. Non-vacuity witnessed at both ends (`meshProductLUV_valuesAt`; `indicatorProductLUV_exact_left_reflected` at zero slack). **PROVISIONAL — not counted toward this tier.** An exact (`slack = 0`) closed endpoint now exists, `lic_no_expected_net_update_conditional_exact_closed`: it takes the same source and weight premises as the incumbent, discharges `hworld`/`source_valued`/`weight_valued`/`right_reflected` over the constructed `LIA`, and removes the slack outright — by extending the *deductive process* with fresh product atoms and their defining schema, which is the propositional counterpart of the product term the paper's `Θ` has natively. It does **not** take this row, because it rests on two decisions the human read-through has not ratified: **(C)** it is stated over `theoremDP T ∪ productDefDP` rather than over `theoremDP T`, so its conclusion is about a different market (Tier-1 precedent for a union-DP conclusion exists at `thm:scon`'s `lic_conditioned_gated_ofComputations`, but not for a *definitional-extension* union); and **(A2)** it carries a second `PGenerableRat` premise about the extended market that the paper does not state, which is the price of keeping `w` at `def:pgen` instead of narrowing it to `PolyRatCodes` and dropping the paper's own worked example at tex:2077. It additionally carries an atom-freshness premise (`ProductAtomFresh X`), the propositional substrate's residue of a guarantee first-order syntax supplies by the language; that premise is discharged by construction for every family this repo builds. The whole premise set — including both paper-absent premises — is exhibited jointly satisfiable by an index-varying construction, `lic_no_expected_net_update_conditional_exact_closed_nonvacuous` (harmonic weight, provably non-constant; source = that sequence's own quotation threshold family), and `hworld` is discharged by extending the base construction's own plausible world rather than by a stand-in. If both gates are ruled favourably this row becomes the exact endpoint's and moves up; if either is rejected the endpoint is deleted and this row is unchanged. Assessment: `notes/boundary-propositional-substrate.md` |
