...
| thm:ccee | qualified | unconditional over `LIA` with both quoted products constructed, a bare `DeferralFunction`, and — as of the mesh product — the paper's **arbitrary** e.c. source family (`lic_no_expected_net_update_conditional_closed` takes `X : ℕ → LUV` with `RpnThresholdCodeSeq` + `source_valued`, the same premises `thm:cee` carries). What keeps the row below `instantiated` is a declared type-`(c)` substitution rather than a class restriction: the left quoted product is reflected only to within `1/(n+1)` (`ConditionalExpectationQuote.slack`), because an exactly-reflecting product LUV built from the source's *own* threshold atoms would need either the value of the deferred weight or an infinite disjunction, neither available. Non-vacuity witnessed at both ends (`meshProductLUV_valuesAt`; `indicatorProductLUV_exact_left_reflected` at zero slack). **The slack is now known to be substrate-shaped, and that does not change this row.** `lic_no_expected_net_update_conditional_exact_closed` proves the same conclusion with `slack = 0` — same trader, same criterion — over the constructed `LIA` on `theoremDP T ∪ productDefDP`, i.e. once the quoted product is given a syntactic existence the propositional substrate otherwise denies it. That is evidence about *where the slack comes from*: it is an artifact of the substrate, not of logical induction. It is **not** a stronger `thm:ccee` and does not take the endpoint-of-record role, because it establishes nothing about the base inductor's conditional expectations: `LIA` over the extended process is a different inductor, and conservativity of completed-world truth does not carry prices across. It also pays two costs the mesh endpoint does not — `ProductAtomFresh X`, a genuine narrowing of the arbitrary e.c. source class in a flat atom space, and `def:pgen` at the extended market. Its premise set is exhibited jointly satisfiable by an index-varying witness (`..._nonvacuous`), and its `hworld` is discharged by extending the base construction's own plausible world. Cross-family adjudication (2026-08-11) rejected the instance reading and ranked the renderings mesh > extension > weight-narrowing. Whether the row moves to `instantiated` is deferred to Anson's read-through and turns on the slack alone, not on the demonstration. Assessment and adjudication outcome: `LogicalInduction/notes/boundary-propositional-substrate.md`. The one assessed route to `slack = 0` over the base inductor is a first-order substrate (deferred denotation via a first-order `LUV`); scoped and recommended against in `LogicalInduction/notes/boundary-substrate-fol-scope.md`. |
