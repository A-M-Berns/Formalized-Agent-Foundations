...
| thm:cee | qualified | unconditional over `LIA` with the deferred-expectation quote constructed and a bare `DeferralFunction`; the paper's own `source_valued` is the only semantic premise, but the unconditional endpoint takes the **whole-value** class `LUV.PolyThresholdCodeSeq`, which the repo proves is strictly narrower than `def:ec`'s symbol-metered rendering (see the metering note above); the hypothesis is also passed as data to the quote-code constructor, so it is not a coercion away from the faithful class.  The `[IsLogicalInductor]`-conditional endpoint (the `_ofRepresentation` layer) *is* at paper strength  **Route to paper strength is short:** the constructor takes only `Primrec fun n => encode (φ n)` from the hypothesis, which `RpnSentenceCodes.primrec` already supplies (verified by probe). |
| thm:ceu | qualified | unconditional over `LIA` with the quote code constructed and no reflection data or deferral narrowing, but the unconditional endpoint takes the **whole-value** class `PolySentenceCodes`, which the repo proves is strictly narrower than `def:ec`'s symbol-metered rendering (see the metering note above); the hypothesis is also passed as data to the quote-code constructor, so it is not a coercion away from the faithful class.  The `[IsLogicalInductor]`-conditional endpoint (the `_ofRepresentation` layer) *is* at paper strength  **Route to paper strength is short:** the constructor takes only `Primrec fun n => encode (φ n)` from the hypothesis, which `RpnSentenceCodes.primrec` already supplies (verified by probe). |
...
| thm:ec | qualified | `LUV.expect_converges` retains the per-grid `hval` **stage-quantified** over `v.ConsistentWith (DP.D n)`, which is stronger than the paper's `def:luv` world-value fact over `cworlds(Θ)` (rendered here as `WorldValued`, quantified over `v.ConsistentWithTheory DP`).  The row previously claimed the retained premise was entailed by `WorldValued`; **no such entailment lemma exists** — `daily_value` is only ever supplied by construction (`LUVSyntax.lean`, `LUVExpectationCertified.lean`).  Proving it (propositional compactness, as `Framework/Compactness.lean` does for `thm:scon`) would raise the row |
...
| thm:epr | qualified | unconditional over `LIA` with the quote code constructed from the market program (`theoremPriceQuoteCode`), but the unconditional endpoint takes the **whole-value** class `PolySentenceCodes`, which the repo proves is strictly narrower than `def:ec`'s symbol-metered rendering (see the metering note above); the hypothesis is also passed as data to the quote-code constructor, so it is not a coercion away from the faithful class.  The `[IsLogicalInductor]`-conditional endpoint (`Introspection.lean` `lic_expectations_of_probabilities`) *is* at paper strength  **Route to paper strength is short:** the constructor takes only `Primrec fun n => encode (φ n)` from the hypothesis, which `RpnSentenceCodes.primrec` already supplies (verified by probe). |
| thm:er | qualified | unconditional over `LIA` with the expectation quote code constructed via `expectQuote_computable`, but the unconditional endpoint takes the **whole-value** class `LUV.PolyThresholdCodeSeq`, which the repo proves is strictly narrower than `def:ec`'s symbol-metered rendering (see the metering note above); the hypothesis is also passed as data to the quote-code constructor, so it is not a coercion away from the faithful class.  The `[IsLogicalInductor]`-conditional endpoint (`Introspection.lean` `lic_iterated_expectations`) *is* at paper strength  **Route to paper strength is short:** the constructor takes only `Primrec fun n => encode (φ n)` from the hypothesis, which `RpnSentenceCodes.primrec` already supplies (verified by probe). |
| thm:expcoh | qualified | `expcoh_ofSyntax` retains `WorldValued` (the paper's own `def:luv` fact) **plus** `ConvergencePresentation`, whose `daily_value` field is stage-quantified over `v.ConsistentWith (DP.D n)`.  The row previously claimed `daily_value` is entailed by `WorldValued` via Cantor-space compactness; that entailment is **not proved anywhere** — `daily_value` is only ever supplied by construction.  Same defect as `thm:ec`; proving the compactness entailment would raise `thm:ec`, `thm:expcoh` and `thm:perexpkno` together |
...
| thm:perexpkno | qualified | `perexpkno_ofSyntax`, same retained `ConvergencePresentation` as `thm:expcoh` and the same unearned `daily_value` entailment claim |
...
| thm:ref | qualified | unconditional over `LIA` with the interval quote constructed from the market's exact rational quote, but the unconditional endpoint takes the **whole-value** class `PolySentenceCodes`, which the repo proves is strictly narrower than `def:ec`'s symbol-metered rendering (see the metering note above); the hypothesis is also passed as data to the quote-code constructor, so it is not a coercion away from the faithful class.  The `[IsLogicalInductor]`-conditional endpoint (`Introspection.lean` `lic_introspection`) *is* at paper strength  **Route to paper strength is short:** the constructor takes only `Primrec fun n => encode (φ n)` from the hypothesis, which `RpnSentenceCodes.primrec` already supplies (verified by probe). |
...
| thm:st | qualified | unconditional over `LIA` with `p` still P-generable — the quote code recovers `p`'s program from the feature presentation (`PGenerableRat.computable`) — and a bare deferral function, but the unconditional endpoint takes the **whole-value** class `PolySentenceCodes`, which the repo proves is strictly narrower than `def:ec`'s symbol-metered rendering (see the metering note above); the hypothesis is also passed as data to the quote-code constructor, so it is not a coercion away from the faithful class.  The `[IsLogicalInductor]`-conditional endpoint (`SelfTrust.lean` `lic_self_trust`) *is* at paper strength |
...
| thm:wub | qualified | unconditional over `LIA` with the emitter and truth bridge constructed, and the paper's own timed-feedback premise `FeedbackTruthComputation` rendered with a *polynomial* clock at `f(k+1)` (weaker than tex:1250 asks), but the unconditional endpoint takes the **whole-value** class `PolySentenceCodes`, which the repo proves is strictly narrower than `def:ec`'s symbol-metered rendering (see the metering note above); the hypothesis is also passed as data to the quote-code constructor, so it is not a coercion away from the faithful class.  The `[IsLogicalInductor]`-conditional endpoint (`Pseudorandomness.lean` `lic_wub`) *is* at paper strength  **Route to paper strength is short:** the constructor takes only `Primrec fun n => encode (φ n)` from the hypothesis, which `RpnSentenceCodes.primrec` already supplies (verified by probe). |
