...
| thm:ccee | qualified | unconditional over `LIA` with both quoted products constructed and no deferral narrowing (`lic_no_expected_net_update_conditional_closed` takes a bare `DeferralFunction`); the one retained restriction is the indicator-family source (general sources are impossible in the token model, disclosed) |
...
| thm:dus | interface | the analytic content is there — unconditional over `LIA` and over the semimeasure, the constructed dovetail dominating every lower-semicomputable continuous semimeasure with no approximation or emission input — but the retained prefix-sentence code emitter `BitPrefixCodeComputation` is **provably uninhabited** (`bitPrefixCodeComputation_isEmpty`: the literal conjunction's code grows as `2 ^ 4 ^ m` against an enumeration index `≤ 5 ^ 2 ^ m`), so the `_unconditional` endpoints are vacuous and no `BitPrefixSentences` witness exists; repair routes (symbol-metered `RpnSentenceCodes`, or a logarithmic-depth prefix syntax) are recorded at `Construction/Witnesses/BitPrefixSyntax.lean` |
...
| thm:strict | interface | the separator argument is fully constructed from Kleene inseparability (the stage classes are proved null) and the atom-code half is discharged (`ordinaryAtom_code_computable`), but the retained `BitPrefixSentences` presentation has no witness: the repo's only constructor for it is vacuous, its emission certificate being provably uninhabited (`bitPrefixCodeComputation_isEmpty`) — same repair as `thm:dus` |
