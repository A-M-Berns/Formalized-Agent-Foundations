...
* the **closed / `_unconditional`** layer over the constructed `LIA` is stated over
  ...
  consistency itself; `thm:incons` adds `[𝗥₀ ⪯ T]`; `thm:lp` adds `[𝗜𝚺₁ ⪯ T]`, one of the
...
| `𝗥₀ ⪯ T` | **1** — `lic_disbelief_inconsistent_theories_unconditional`, redundantly, beside the stronger `𝗣𝗔⁻ ⪯ T` it also carries (cleanup queued) |
| `T.Δ₁` | **23** |
...
* **Still `qualified`** — `thm:pac`, `thm:pazfc` and `thm:incons`, on **mathematical**
  grounds that have nothing to do with theory strength: the §4.10 subject matter. `thm:pac`
  and `thm:pazfc` price the represented claim of an arbitrary decidable `consistentWithin`,
  not `Con(Γ)`, and no proof-search-for-⊥ machine is constructed; `thm:incons` prices an
  arbitrary semidecidable predicate rather than a real sequence of inconsistent theories.
  Their rows say exactly what is and is not formalized. Note also that `thm:pac` and
  `thm:pazfc` are discharged by one and the same proposition — the kernel accepts
  `@lic_belief_finitistic_consistency = @lic_belief_stronger_theory_consistency := rfl` —
  and that closing the gap is a queued `Con`-schema project (~400–800 lines) plus a genuine
  inconsistent-theory sequence.
