...
| thm:expprovind | qualified | every combination-level endpoint (`lic_expect_combination_provind_ge/_le/_eq`) requires `DeterminedViaTheory` — all completed worlds give the *same* value — whereas tex:1753-1760 assumes only a one-sided bound `∀ W ∈ cworlds(Θ) ∀ n, W(Aₙ) ≥ b` and lets worlds disagree.  The restriction is avoidable in principle: the repo's own affine analogue `AffineCombination.PolySequence.affine_provind_theory_ge` takes exactly the paper's premise.  The fixed-LUV endpoints avoid `DeterminedViaTheory` but quantify over *stage*-consistent worlds instead, which the repo elsewhere shows excludes the paper's own `1(φ)`.  So no endpoint carries this node at the paper's hypothesis |
