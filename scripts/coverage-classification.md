...
| thm:ifp | qualified | Three carriers, no inhabitant. `lic_iff_of_finitePerturbation` keeps the paper's own tail-agreement hypothesis and takes `EfficientPrefixPatch` for both markets. `lic_iff_of_finiteSupportPerturbation` and `machine_lic_iff_of_finiteSupportPerturbation` are the **corrected** theorem: they quantify over perturbations moving only finitely many `(day, sentence)` price coordinates, which is strictly stronger than the paper's hypothesis (`FiniteSupportPerturbation.tail_agree` one way; the day-`0` huge-numeral market the other) and is exactly the case where the appendix's constant table really is finite. That repairs the *argument*, not the *coverage*: `EfficientPrefixPatch`, `FiniteSupportPatch` and `MachineFiniteSupportPatch` all have **zero inhabitants anywhere in the repo**, so every carrier here is vacuously instantiable only. The paper's unrestricted statement additionally has a recorded erratum (PE1) and remains unresolved. The fuel-class obstruction is the `dd:fuel` inverse-operation ceiling; the machine-class one is an unproved `Complexity.FP` transport result |
