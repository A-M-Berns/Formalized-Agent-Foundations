...
repository model**: sentences are propositional (`Formula ℕ`), and the *data* hypotheses of the
property tail are certified in the fuel-clocked `Nat.Partrec.Code` calculus (`dd:fuel`) rather than
the paper's first-order syntax.  The **trader** class is no longer part of that disclosure: it is
ordinary machine polynomial time (`MachineEfficientTrader`, through `Complexity.FP`), and `dd:fuel`
is a sufficient certification device for it.  A row marked *instantiated* claims the paper statement
is reached **within that model**, not that the model equivalence is proved.
...
| def:blcp | instantiated | direct rendering of the paper's two clauses (`LUVCombination.BoundedSequence`: an efficiency certificate on the compiled threshold mesh plus one uniform `L¹` bound), and now also stated over the paper's own LUVs rather than only over the abstract carrier: `PaperLUVCombination` carries its shares as literal `PaperLUV`s and reaches `LUV` only through `toLUV`, `boundedSequence` discharges the bounded-sequence interface from that data with the family's own structural threshold certificate, and `unitFracPaperLUVBoundedSequence` inhabits it with the genuinely varying `1/(n+1)` family. The carrier-level charge that used to sit here is therefore gone, on the same footing as `def:luv`'s |
...
| def:ec | qualified | **the trader half is closed.** `MachineEfficientTrader` is an honest complexity class — some `Complexity.FP` function of the *unary* day emits the day's strategy — and it is the class the construction actually dominates: the trader enumeration is sound and complete for exactly it (`enumeratedTrader_machineEfficient`, `exists_enumeratedTrader_eq`), and `IsMachineLogicalInductor` is what `LIA_isMachineLogicalInductor` proves. `dd:fuel` is a *certification device* for that class (`EfficientlyComputable.toMachine`), not a substitution for it. What still qualifies the row is the other half: the efficiently computable **sequence** classes the property tail takes as its own data (`RpnSentenceCodes`, `RpnThresholdCodes`, `PolySequence`, …) are still the symbol-metered fuel rendering, so those statements quantify over a possibly smaller set of admissible data than the paper's. The machine reading exists (`MachineSentenceCodes`, with the inclusion `RpnSentenceCodes.toMachine`) but is consumed only at `thm:scon`; the converse inclusion is open. This is the global fuel charge the preamble levies once, and it is levied here |
...
| thm:ccee | instantiated | `lic_no_expected_net_update_conditional_closed_exact` takes exactly the paper-facing source interface (`X : ℕ → LUV`, `RpnThresholdCodeSeq X`, and completed-world `source_valued`), a bare `DeferralFunction`, and a P-generable `[0,1]` weight. A fixed atom injection represents the caller's source in an old-language namespace, and a universal finite-entailment gate derives source admission from `source_valued`; no freshness or proof-carrying certificate is caller-visible. The sole market is `liaHistory (canonicalCCEEDP T)`, whose computable, explicitly non-vacuous process is fixed from `T` before `X`, `f`, or `w`. Deferred-weight and right-hand quotation LUVs are built internally, semantic multiplication is exact, and the generic CCEE theorem is instantiated with identically zero slack. This is one canonical enlarged language from the outset, not a source-dependent extension and not a market-correspondence claim about `liaHistory (theoremDP T)`. |
...
| thm:ifp | qualified | **The published unrestricted theorem is false**, and `not_overgeneral_ifp` (`Construction/Witnesses/FinitePerturbationWitness.lean`) proves it false — closed but for the deductive process, `sorry`-free, axiom-clean, at the paper's own quantifier, with the constructed `LIA` as the inductor and a day-`0` advice tape as the perturbation (PE1). Nothing on this node can rise above `qualified`, because the exact paper theorem is refuted rather than proved. The **corrected** theorem is `machine_lic_iff_of_recognizableSupport`: two computable markets differing on only finitely many `(day, sentence)` coordinates satisfy the criterion together — strictly stronger than the paper's tail agreement (`FiniteSupportPerturbation.tail_agree` one way; the day-`0` huge-numeral market the other), and exactly the case where the appendix's constant table really is finite. Its one residual hypothesis, `Recognizable`, is a condition on the syntax of the finitely many moved sentences rather than on any market: representation residue standing for two `Complexity.FP` primitives this toolkit lacks (integer square root, a structured-payload parser), both proved necessary rather than convenient. `machine_lic_iff_twoPoint` makes it non-vacuous, and `machineLogicalInductor_liaPerturbed` makes it informative: applied to the constructed `LIA` with one price moved, it derives a machine logical inductor that no construction here produces. `lic_iff_of_finitePerturbation` is retained as the compatibility carrier at the paper's own hypothesis shape; its fuel-class certificates remain uninhabited, at the `dd:fuel` inverse-operation ceiling |
...
| thm:scon | instantiated | growing-form `hjoint` deleted — derived by propositional compactness (`Framework/Compactness.lean`) or vacuous by the degenerate branch. Now also **complete at the paper's own quantifier**: `lic_conditioned_machine`, `lic_conditioned_gated_machine` and `lic_conditioned_eventual_machine` conclude `IsMachineLogicalInductor`, carried by the `Complexity.FP` transports `conditionedTranslation_preserves_machine` and `eventualConditionedTranslation_preserves_machine` under the same `RpnSentenceCodes` hypothesis on the condition as the fuel counterparts. The fuel endpoints and their inhabited witnesses are unchanged beside them, so this is a strengthening, not a replacement. The paper's two *printed* forms also stand at that quantifier with no consistency hypothesis: `lic_conditioned_fixed_machine_ofComputationAndMarket` and `lic_conditioned_growing_machine_ofComputationsAndMarket`, whose degenerate branch is `isMachineLogicalInductor_of_stage_unsatisfiable`, and their premise-free instances over the constructed `LIA` are `lic_conditioned_fixed_machine_unconditional` / `lic_conditioned_growing_machine_unconditional` |
...
| thm:wubexp | instantiated | unconditional over `LIA`; the normalized threshold mesh, its feedback traders, and its sparse delayed-truth affine family (the one the paper builds *inside* `app:wub`) are all constructed.  The remaining premises are exactly tex:1822-1832's — a bounded LUV-combination sequence determined via Θ at the *combination* level (`def:affthmval`), the `def:luv` representation premise `WorldValued`, a P-generable divergent weighting supported on the image of a strictly increasing deferral function, and timed feedback (`FeedbackTruthComputation`, polynomial clock at `f(k+1)`, as for `thm:wub`/`thm:wubaff`).  No endpoint uses `LUVCombination.ExactTheoryPresentation`: meshing is nonlinear, so the mesh of a combination-determined sequence is genuinely *not* determined, and the bridge is instead built at `ApproxDeterminedViaTheory` with the vanishing `meshErrorBound` (`lem:conluvapprox`) — all `affine_provind_theory_tendsto_zero` requires |
