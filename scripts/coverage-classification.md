...
| thm:expcoh | conditional | `expcoh_ofSyntax` retains `WorldValued` (the paper's `def:luv` fact) and a `ConvergencePresentation` now *derivable* from it (`WorldValued.convergencePresentation`), so the two world-value hypotheses coincide at `cworlds(Θ)`.  `ConvergencePresentation.daily_value` and the upstream `TheorySemantics.stage_values` field — the actual stage-quantified smuggling site — are deleted |
...
| thm:perexpkno | conditional | `perexpkno_ofSyntax`, same premises as `expcoh` and same repair: no stage-quantified world-value hypothesis remains anywhere in its transitive premise set |
...
| thm:st | qualified | the metering restriction is gone — `lic_self_trust_closed` now takes the symbol-metered `RpnSentenceCodes` (`indicatorProductLUV_rpnThresholdCodeSeq` emits the `⋏`-shell as tokens rather than as a `Nat.pair` on Gödel values), and the endpoint is unconditional over `LIA` with `SelfTrustQuote` discharged, `p` still P-generable (`def:ece`) and a bare deferral function.  What now keeps the row below `complete` is a *different* and much smaller gap, found by re-deriving the signature: the endpoint additionally requires `hδinv : PolyRatCodes (fun n ↦ 1 / δ n)`, whereas tex:2093 asks only that `δ` be an e.c. sequence of **positive** rationals.  The extra premise is very likely redundant — `Encodable.encode` of a rational is essentially `pair num den`, so a poly-valued code should give a poly-valued reciprocal — but no such lemma exists (`exact?` fails), so the row does not claim it.  Proving `PolyRatCodes δ → (∀ n, 0 < δ n) → PolyRatCodes (1/δ)` moves this row to `complete` and cleans ~8 threaded occurrences of `hδinv` repo-wide |
