...
`BigSentenceCodes`, `⟨m⟩` to `DigitMachineCodes`, and `⟨x⟩`/`⟨y⟩` — including the `input_poly`
fields of the two structures since retired — to `BigDigits`. What remains in the list above is repo-side
...
* **Still `qualified`** — `thm:incons` alone, on **mathematical** grounds that have nothing
  to do with theory strength: its theory sequence is the *deduction family*
  ...
  represent. The day's theory is genuinely named in the day's sentence (by the code
  `⌜∼σₙ⌝`, which the deduction theorem makes sufficient), the premise `∀ n,
  ¬Consistent (σ n ∷ Θ₀)` is the paper's own, and the paper's subject matter is exhibited
  (`𝗜𝚺₁ ∪ {⊥}`, and a day-varying `alternatingInconsistentAxiom` beside it); the restriction
  on *which* theory sequences are covered is the disclosed part. A **second** disclosed
  charge sits beside it: the `def:ec` premise `hσ : BigDigits (deductionFamilyArg σ)` meters
  the day's theory name by the base-4 digit count of a formula's *Gödel code* rather than by
  its source length — Foundation's encoding pairs at every node, so that count is `~2^depth`
  and only `O(log n)`-depth families qualify — and the faithful repair, restating the premise
  on `PolyArithmeticSourceSeq`, is queued with the 9-series. Its row says exactly what is and
  is not formalized, and the uniform-in-theory-code work of tranche 9b retires the first
  charge. `thm:pac` and `thm:pazfc` both **left this list in tranche
...
| thm:incons | qualified | instantiated | **What the paper asserts** (tex:1893-1903): for an **e.c. sequence of recursively axiomatizable inconsistent theories** `⟨Θ′⟩`, `ℙₙ(⌜⌜Θ′ₙ⌝ is inconsistent⌝) ≈ₙ 1` and hence `ℙₙ(⌜⌜Θ′ₙ⌝ is consistent⌝) ≈ₙ 0`, those two sentences being the universal generalization of `Con(Θ′)(ν)` and *its negation* (tex:1855-1866). **What the Lean theorem now proves:** the endpoint's subject matter is a *theory sequence*, not an arbitrary semidecidable predicate. Its signature is `lic_disbelief_inconsistent_theories_unconditional (T : ArithmeticTheory) [T.Δ₁] [𝗣𝗔⁻ ⪯ T] [Entailment.Consistent T] (T' : ArithmeticTheory) [T'.Δ₁] (σ : ℕ → ArithmeticSentence) (hσ : BigDigits (deductionFamilyArg σ)) (hinc : ∀ n, ¬Entailment.Consistent (σ n ∷ T')) : ((fun n => liaHistory (paperTheoryDP T) n ((representedInconsistentTheoryClaims T T' σ hσ).inconsistencySentence n)) ≈ₙ fun _ => 1) ∧ ((fun n => liaHistory (paperTheoryDP T) n ((representedInconsistentTheoryClaims T T' σ hσ).consistencySentence n)) ≈ₙ fun _ => 0)`, and both paper conjuncts are concluded. `hinc` is **the paper's own premise**, stated at the day's *theory* — `σ n ∷ T'`, Foundation's `adjoin` — and not at any provability surrogate. **Two charges. (i) The deduction-family paraphrase.** The theory sequence is `Θ′ₙ := Θ₀ ∪ {σₙ}` over a fixed Δ₁ base `Θ₀ = T'`, not an arbitrary e.c. sequence of recursively axiomatizable theories. This is forced by the substrate: Foundation's `Derivation T` is `(construction T).Fixpoint` with `T` a **meta** parameter, so there is no uniform-in-theory-code derivability predicate to represent, and a sequence of theories cannot enter one sentence as an argument. For a deduction family the deduction theorem collapses the day's theory to a single sentence code (`Θ′ₙ` inconsistent ⟺ `Θ₀ ⊢ ∼σₙ` ⟺ `⌜∼σₙ⌝` is internally `Θ₀`-provable), which *is* uniform, so the day's theory is genuinely named in the sentence and the restriction is only on *which* theory sequences are covered. Disclosed at the endpoint docstring, at the section header and here; retired by the uniform-in-theory-code work (tranche 9b). **(ii) The `def:ec` premise is metered on the wrong quantity.** `hσ : BigDigits (deductionFamilyArg σ)` meters the day's theory name by the base-4 digit count of a formula's **Gödel code**, not by its source length; Foundation's encoding pairs at every node, so that count is `~2^depth`, and the premise therefore admits only `O(log n)`-depth families, excluding paper-admissible short-source/deep-parse ones. It is neither the `dd:fuel` model choice nor the `dd:proofcode` measure choice, but the ordinary write-out class applied to the *wrong quantity* — the same failure that disqualified `Encodable.encode` as a machine-naming map and forced `Code.sourceNat` (`Framework/WriteOut.lean`, `DigitMachineCodes` doctrine). Being an over-strong *hypothesis* it narrows which theory sequences the endpoint covers; it does not weaken the conclusion where it applies. The faithful repair is to state the premise on `PolyArithmeticSourceSeq`, which is queued with the 9-series and not done. **`dd:proofcode` is not charged here.** Unlike `thm:pac`/`thm:pazfc`, this node's sentence is the *unbounded* existential over proofs — the paper's `⌜Θ′⌝ is inconsistent` is the negation of the universal generalization of `Con(Θ′)(ν)` (tex:1863-1866). Nothing is metered, so the symbol-count-versus-Gödel-number substitution does not arise: the existential ranges over all proofs either way. **Anti-extensionality (R5-F08 discipline).** The represented object is the **universal** provability schema of the base theory, `inconsistencySchema T' := codeOfREPred (ProvableCode T')` — one schema per base theory, argument-sensitive, with `inconsistencySchema_mentions_zero` *proved* from `Consistent T'` rather than assumed — and the day's data written into the sentence as the compact numeral `binNumeral ⌜∼σₙ⌝`. Representing “`T'` is inconsistent” directly would have been the trap: under `hinc` that predicate is constantly `True`. `hσ` is load-bearing, being the only route to `inconsistency_poly`. **Non-vacuity is discharged by the construction.** The in-file `example` runs the theorem at market `𝗜𝚺₁`, base theory `𝗜𝚺₁`, `σₙ := ⊥`, so every day's theory is `𝗜𝚺₁ ∪ {⊥}` — *actually inconsistent*, the paper's own subject matter. This replaces the superseded `ordinarySemidecidableComputation` “`0 < n`” stand-in, whose predicate was not a statement about a theory at all. Beside it sits a genuinely day-varying witness: `alternatingInconsistentAxiom n := if n % 2 = 0 then ⊥ else ⊥ ⋏ ⊥`, refutable on every day and with distinct Gödel codes on adjacent days, carried with its emission certificate `alternatingInconsistentAxiom_digits` (`BigDigits.ifZero` over a poly-fueled parity test between two constant codes) and the fully applied endpoint `thm_incons_applied_alternating`, plus an `example` exercising day separation at days `0` and `1` — which the constant witness cannot, its hypothesis being that the codes differ. Honestly not achieved: a family with *unboundedly many* distinct theories. This one takes two values; the blocker is not size but a missing combinator — a `BigDigits.precBig` giving poly-fueled digit access through an unboundedly iterated `Nat.pair` shell, since every closure in `Framework/DigitArith.lean` composes only a constant number of times and `PolyFueled.prec` cannot iterate a bignum-valued state. It is signposted at the witness, and the cheaper repair is the source-metered restatement of charge (ii). Day-variation of the *rendering* is a theorem regardless — `inconsistencyArgClaimSentence_ne_of_arg_ne` separates the sentences of any two distinct adjoined-axiom codes outright, with no behavioural hypothesis. **The sentence families collapsed to one.** `InconsistentTheoryClaims` was `inconsistencySentence consistencySentence inconsistency_poly consistency_poly inconsistency_provable consistency_disprovable`; it is now `inconsistencySentence inconsistency_poly inconsistency_provable`, with `InconsistentTheoryClaims.consistencySentence R n := ∼R.inconsistencySentence n`. The stated reason for two independent families — avoiding a syntactic-negation law the abstract `Sentence` representation does not provide — no longer held: `Sentence` is Foundation's propositional `Formula`, which has `∼`, and the paper itself *defines* the consistency statement and its negation as one sentence pair (tex:1863-1866), so this is the paper's own shape. **Theory hypotheses: nothing charged.** The endpoint carries `[T.Δ₁] [𝗣𝗔⁻ ⪯ T] [Entailment.Consistent T]` and `[T'.Δ₁]`. It used to carry `[T.SoundOnHierarchy 𝚺 1]` — gone when `theoremDP_hworld`'s tag-7 exclusivity became a theorem of Θ — then a residual `[𝗜𝚺₁ ⪯ Θ]`, deleted in tranche 7 with `QuotationTheoryPresentation.theory_sigmaOne`, and then a **redundant `[𝗥₀ ⪯ T]`** beside the stronger `𝗣𝗔⁻`, which was dropped in T8/iii (R6-F03): the proof elaborates unchanged, reaching `𝗥₀` through Foundation's `instance [𝗣𝗔⁻ ⪯ T] : 𝗥₀ ⪯ T`. `[T.Δ₁]`/`[T'.Δ₁]` are the global Craig's-trick charge and `[𝗣𝗔⁻ ⪯ T]` the standing representation-infrastructure binder, **whose charging is pending the user's ruling**; this row's status is otherwise re-derived without reference to it. The `𝗣𝗔⁻`-level witness is likewise pending — the exhibited example is at `𝗜𝚺₁`, for the same reason as the other §4.10 witnesses. **What holds the row at `qualified` is therefore the two charges above: the deduction-family paraphrase and the code-metered `def:ec` premise.** Neither is a new status: the row was and remains `qualified`, and no count changes. *Superseded 2026-08-30 (T8/iv): the previous rendering priced an arbitrary semidecidable `inconsistent : ℕ → Prop` presented by a `SemidecidableComputation`, with `ordinarySemidecidableComputation` (“`0 < n`”) as its only witness. That structure, its witness, `inconsistentTheoryClaimsOfComputation` and `lic_disbelief_inconsistent_theories_ofComputation` have been deleted; this node now has exactly one carrier.* |
