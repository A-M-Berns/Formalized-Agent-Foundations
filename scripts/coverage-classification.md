...
| thm:scon | qualified | the fixed-sentence form is unconditional over `LIA` and premise-free, exactly the paper's first statement; the growing-conjunction form retains a global joint-consistency hypothesis (disclosed type-`(c)`) that the paper does not take, because Foundation supplies no propositional compactness |
...
| thm:st | qualified | the abstract endpoint takes P-generable `p` (`def:ece`) with `SelfTrustQuote` reflection data; `lic_self_trust_closed` discharges that data over `LIA` with `p` still P-generable — the quote code recovers `p`'s program from the feature presentation (`PGenerableRat.computable`: parse the emitted serialization, evaluate against the certified market, minimize over the interpreter clock) — so the only retained narrowing is injective deferral |
