...
| thm:ifp | qualified | **The published unrestricted theorem is false, and `not_overgeneral_ifp` (`Construction/Witnesses/FinitePerturbationWitness.lean`) proves it false** — kernel-checked, axiom-clean, at the paper's own quantifier, with the constructed `LIA` as the inductor and a day-`0` advice tape as the perturbation (PE1). Nothing on this node can therefore rise above `qualified`. Three carriers remain, none of them the paper's statement: `lic_iff_of_finitePerturbation` keeps the paper's tail-agreement hypothesis and takes `EfficientPrefixPatch`; `lic_iff_of_finiteSupportPerturbation` and `machine_lic_iff_of_finiteSupportPerturbation` are the **corrected** theorem, over perturbations moving only finitely many `(day, sentence)` coordinates — strictly stronger than the paper's hypothesis (`FiniteSupportPerturbation.tail_agree` one way; the day-`0` huge-numeral market the other), and exactly the case where the appendix's constant table really is finite. `MachineFiniteSupportPatch` is now **inhabited** (`FreezeOracle.machineFiniteSupportPatch_ofTable`), for a market whose frozen coordinates come from a finite entry table satisfying `Recognizable` per sentence plus a `TablePresentation`, witnessed at a table with a real row rather than the degenerate empty one. The fuel-class certificates `EfficientPrefixPatch` and `FiniteSupportPatch` remain **uninhabited** — the fuel calculus does not close over the escape-leaf decode, which is the `dd:fuel` inverse-operation ceiling. A concrete computable market pair discharges every hypothesis at once (`machine_lic_iff_twoPoint`), so the corrected theorem's antecedent is **satisfiable** — non-vacuous, though not informative: neither side of the equivalence is known to hold at that pair. The node stays `qualified` regardless, because the published statement is false |
