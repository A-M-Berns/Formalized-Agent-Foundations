...
LI_PRIMARY = {
 ...
 'thm:ifp':['lic_iff_of_finiteSupportPerturbation','machine_lic_iff_of_finiteSupportPerturbation','lic_iff_of_finitePerturbation'],
...
LI_READING = {
 ...
 'thm:ifp': "Two carriers, and neither is the paper's unrestricted theorem. The corrected ones (`lic_iff_of_finiteSupportPerturbation`, and the same at the machine class) quantify over perturbations moving only finitely many (day, sentence) price coordinates \u2014 a hypothesis strictly stronger than the paper's tail agreement, and exactly the case in which the appendix's 'hard-code the constants' step is valid. `lic_iff_of_finitePerturbation` keeps the paper's own hypothesis shape and its `EfficientPrefixPatch` obligation. All three patch interfaces have no inhabitant anywhere in the repo, so no carrier on this card is yet shown to be about anything; the published proof is invalid and the unrestricted statement is unresolved (PE1).",
