...
LI_PRIMARY = {
 ...
 'thm:ifp':['not_overgeneral_ifp','lic_iff_of_finiteSupportPerturbation','machine_lic_iff_of_finiteSupportPerturbation','lic_iff_of_finitePerturbation'],
...
LI_READING = {
 ...
 'thm:ifp': "The published unrestricted theorem is FALSE, and `not_overgeneral_ifp` proves it false \u2014 kernel-checked and axiom-clean, at the paper's own quantifier: the constructed LIA, perturbed on day 0 only, publishes the diagonal sign bits as advice-atom prices, and an efficient trader reads them through historical price features without ever computing them. A single changed pricing day is an infinite computable function, which is enough to smuggle the gap between computable and efficiently computable. The remaining carriers are restricted: `lic_iff_of_finiteSupportPerturbation` (and its machine twin) quantify over perturbations moving only finitely many (day, sentence) coordinates \u2014 strictly stronger than the paper's tail agreement, and exactly the case where the appendix's constant table really is finite; `lic_iff_of_finitePerturbation` keeps the paper's own hypothesis shape. `MachineFiniteSupportPatch` is now inhabited \u2014 a finite entry table satisfying `Recognizable` per sentence plus a `TablePresentation`, witnessed at a table with a real row, not the degenerate empty one. The fuel-class certificates remain uninhabited, since the fuel calculus does not close over the escape-leaf decode. A concrete computable market pair now discharges every hypothesis at once, so the corrected theorem's antecedent is satisfiable \u2014 non-vacuous, though not yet informative, since neither side of the equivalence is known to hold at that pair.",
