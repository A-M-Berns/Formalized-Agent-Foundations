...
LI_READING = {
 ...
 'thm:ifp': "The published unrestricted theorem is FALSE, and `not_overgeneral_ifp` proves it false \u2014 closed but for the deductive process and axiom-clean, at the paper's own quantifier: the constructed LIA, perturbed on day 0 only, publishes diagonal sign bits as advice-atom prices, and an efficient trader reads them through historical price features without ever computing them. A single changed pricing day is an infinite computable function, which is enough to carry unbounded advice. The corrected theorem is `machine_lic_iff_of_recognizableSupport`: two computable markets differing on only finitely many (day, sentence) coordinates \u2014 strictly stronger than the paper's tail agreement, and exactly the case where the appendix's constant table really is finite. Its one residual hypothesis is a condition on the syntax of the moved sentences, not on any market, standing for two FP primitives this toolkit lacks. A concrete market pair makes it non-vacuous, though not yet informative. The fuel-class carrier is retained for compatibility only.",
