...
LI_READING = {
 ...
 'thm:incons': "The endpoint's subject matter is a *theory sequence*, not an arbitrary semidecidable predicate: `lic_disbelief_inconsistent_theories_unconditional` takes a base theory `Θ₀`, a sentence family `σ` and a **written source** family `s` with `hcompile : ∀ n, ArithSource.compile (s n) = ↑(σ n)`, and its premise `hinc : ∀ n, ¬Entailment.Consistent (σ n ∷ Θ₀)` is the paper's own — stated at the day's *theory* (Foundation's `adjoin`), not at a provability surrogate. Both conjuncts of the display are concluded. **The `def:ec` premise is metered on the written source.** `hs : PolyArithmeticSourceSeq s` charges one token per symbol the writer writes, which is tex:753-755 / tex:1931-1933 verbatim. What the sentence emits is `ArithSource.sourceNat sₙ` — the token run read as a base-64 numeral with a sentinel (`tokenListNat`, `Framework/CodeSource.lean`) — whose base-4 digit count is `3·(sourceTokens sₙ).length + 3`, linear in the written text, exactly the `Code.sourceNat` doctrine the halting lane applies to machines. The Gödel code is recovered from that numeral *inside* the represented predicate by `negSourceFormulaCode` (`Construction/Witnesses/SourceNumbering.lean`), which is computable and is all `codeOfREPred` consumes; the paper asks only for recursive enumerability there, so the code's size is free. **The sentence names the day's theory.** What is represented is the universal provability schema of the base theory read at written names, `inconsistencySchema Θ₀ = codeOfREPred (fun z => ProvableCode Θ₀ (negSourceFormulaCode z))` — one schema per base theory, argument-sensitive, with `inconsistencySchema_mentions_zero` *proved* from consistency rather than assumed — and the day's data enters as the compact numeral `binNumeral (sₙ.sourceNat)`. Representing '`Θ₀` is inconsistent' directly would have been the trap, since under `hinc` that predicate is constantly True. **Non-vacuity is the paper's own subject matter, at unbounded day-variation.** The in-file example runs at market 𝗜Σ₁, base theory 𝗜Σ₁ and source `falsumSource`, the one-token writing of ⊥, so every day's theory 𝗜Σ₁ ∪ {⊥} is actually inconsistent. Beside it is the payoff of the re-metering: `deepInconsistentSource n`, the writing of `(∀x. A(x) ⟺ ⋯ ⟺ A(x)) ∧ ⊥` with n biconditionals — `5n + 7` emitted tokens (`sourceTokens_deepInconsistentSource_length`), `≥ 2^n` nodes once compiled (`two_pow_le_encode_iffChain`), and a Gödel code doubly exponential in that. It is `def:ec`-admissible on the nose, inconsistent on every day (`deepInconsistentAxiom_inconsistent`), fully applied at `thm_incons_applied_deep` with nothing left to the caller, and distinct on *every* day as written (`deepInconsistentSource_sourceNat_ne`), so `inconsistencyArgClaimSentence_deep_ne` separates the claim sentences of **every** pair of distinct days. No code-metered premise could admit that family. The two-valued `alternatingInconsistentAxiom` witness of the previous edition is strictly subsumed and has been deleted, and the signpost for a missing `BigDigits.precBig` combinator is withdrawn — the family it was blocking is admitted by the source route, with no code-digit theory needed. The two sentence families are no longer independent: `consistencySentence` is now definitionally `∼inconsistencySentence`, which is the paper's own shape (tex:1863-1866). No symbol measure is in play here: this node's sentence is the unbounded existential over proofs, so nothing is metered and §4.10's counting convention `dd:symbolcount` does not arise. Theory hypotheses are `[T.Δ₁] [𝗣𝗔⁻ ⪯ T] [Entailment.Consistent T]` and `[Θ₀.Δ₁]` — no soundness (that went when `theoremDP_hworld`'s tag-7 exclusivity became a theorem of Θ), no 𝗜Σ₁, and no 𝗥₀, the redundant binder having been dropped in tranche 8; the standing `𝗣𝗔⁻` binder's charging is pending a user ruling. **One charge holds the row at `qualified`.** The theory sequence is the *deduction family* Θ′ₙ = Θ₀ ∪ {σₙ} rather than an arbitrary e.c. sequence of recursively axiomatizable theories. Foundation's `Derivation T` takes `T` as a **meta** parameter, so there is no uniform-in-theory-code derivability predicate to represent; for a deduction family the deduction theorem collapses the day's theory to the single written name `ArithSource.sourceNat sₙ`, which is uniform. This is backed by a **verified obstruction** (tranche 9b), not by a queued repair: Foundation's fixpoint blueprint for `Derivation` cannot host satisfaction over coded formulas, so full uniformity in the theory code is a truth-predicate project. A *middle* rendering — uniformity over a coded family of Δ₁ axiomatizations — is an optional upstream item, pending a user ruling. The earlier note that tranche 9b would retire this charge is withdrawn, as is the second charge it used to carry: the Gödel-code-metered `def:ec` premise `hσ : BigDigits (deductionFamilyArg σ)` was **retired in tranche 9-pre** by the source-metered restatement above.",
