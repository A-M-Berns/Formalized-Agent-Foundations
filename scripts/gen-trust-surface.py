...
# Curation note.  The definition carriers and the per-node primary endpoints are NOT kept
# here.  They are one machine-checked table in `scripts/coverage-classification.md`, beside
# the strength claim they have to agree with, and a curated name that does not resolve
# raises rather than falling back to an arbitrary endpoint.  A hand-kept list in this file
# keyed by `AxiomAudit.lean`'s exact spelling, with a silent fallback, is how the
# mis-selection of `thm:ifp`'s carriers — the one node whose printed statement is false —
# stayed invisible until an audit read the page against the ledger.
...
LI_READING = {
 'def:lic': "`IsLogicalInductor` is the criterion at the paper's own quantifier — no `Complexity.FP` trader exploits the market — it is the one the construction proves, and it is what the whole §4 tail is *conditioned* on. There is exactly one criterion class, and it is the paper's: no theorem here is stated over the certification engine's class instead. It bundles two facts the paper leaves ambient: the market and the process are computable. `P n φ` is the paper's ℙₙ(φ).",
 'def:ec': "The paper's “computable in O(poly(n))” is `EfficientlyComputable`: some `Complexity.FP` function of the unary day emits the day-n strategy's serialized symbol stream. It is shown first because it is the paper's own class and the one the trading firm dominates. The fuel-clocked `PolyFueledTrader` is the certification device that feeds it — `PolyFueledTrader.toEfficientlyComputable` compiles an `evaln` certificate into a real polynomial-time machine — and is not a substitution for the class. **Every canonical endpoint's emission premise is at the machine reading of the write-out ladder**: `MachineTokenStream`, `MachineSentenceCodes`, `MachineSpliceStream`, `MachineDigits`, `MachineMachineCodes`, `MachineRatCodes`, `LUV.MachineThresholdCodes(Seq)`, `MachineArithmeticSourceSeq` and the count class `UnaryRuler`, each of which bounds the number of symbols a poly-time writer must emit and leaves the written value unrestricted, which is `def:ec`'s own metering (tex:753-755, tex:1931-1933). Counted over the 107 canonical endpoints through the structures they bind and not only their printed binders: **22** print a `MachineSentenceCodes` binder outright, `LUV.MachineThresholdCodeSeq` 4, `MachineMachineCodes` 4, `MachineRatCodes` 3, `MachineDigits` 3, and no endpoint binds a fuel-, token- or value-metered *emission* class at all, printed or through a boundary structure. The `PolyFueled*`/`Big*`/`Digit*`/`Poly*`/`Rpn*` families are the certification calculus, reached by the forward bridges `BigTokenStream.toMachine`, `BigSentenceCodes.toMachine`, `RpnSentenceCodes.toMachine`, `BigSpliceStream.toMachine`, `BigDigits.toMachine`, `DigitMachineCodes.toMachine`, `DigitRatCodes.toMachine`, `PolyArithmeticSourceSeq.toMachine`, `LUV.BigThresholdCodes(Seq).toMachine` and `UnaryRuler.of_polyFueled`; the bridges run fuel → machine only, and no converse is provided. **The two clocks that are not emission premises are machine-metered too.** `def:deferralfunc`'s condition 2 is “`f(n)` computable in time polynomial in `f(n)`” (tex:1244) and `thm:wub`'s feedback premise is “`Th(φ_{f(n)})` computable in `O(f(n+1))` time” (tex:1251): both bound runtime in the **value returned**, which `Complexity.FP` cannot express of a machine handed the day alone — but it can of a machine handed the **unary pair**, whose length dominates that value. `DeferralFunction.graph_fp` decides the *graph* `f n = m` in `Complexity.FP` on the unary `⟨n, m⟩`, and `FeedbackTruth.FeedbackTruthComputation.computes` is `MachineDigits` on the value-code stream read at `⟨k, f (k+1)⟩`; the graph reading is equivalent to the paper's clause in both directions, and the argument is recorded at the structure. No `evaln` clock appears in either statement, so `dd:fuel` does no statement work anywhere on this surface. Twenty-three canonical endpoints bind a `DeferralFunction` (the `thm:prandaff`, `thm:prandexp`, `thm:benford`, `thm:prand`, `thm:wub`, `thm:wubaff`, `thm:wubexp`, `thm:cee`, `thm:ceu`, `thm:ccee` and `thm:st` carriers), six of them a `FeedbackTruthComputation` as well; the classification ledger's `def:ec` row lists them by name. **The converse of the fuel/machine inclusion splits.** At a value-metered target it is **false**, not open: `not_polyFueled_two_pow` refutes it at `2^n`, which is `Complexity.FP` once its output is written in binary. At a length-metered target it is open on a verified obstruction, and the obstruction is a workspace bound rather than a missing compiler. At a fixed code, `PolyFueled` is a poly-time, `O(log n)`-workspace device — `evaln`'s `n ≤ k` guard caps every value passed to a sub-code by the fuel, `IsPolyBounded` caps the fuel and the output, and the fixed code makes the `prec`/`rfind'` nest constant-depth — while `Complexity.FP` is poly-time with polynomial workspace. So the converse says every poly-time write-out over tally inputs is a logspace write-out: a P-versus-L flavoured containment, a complexity-theoretic conjecture rather than a lemma, and equally not refutable here. A TM → `Nat.Partrec.Code` compiler with fuel accounting would not close it, because the compiled code's `evaln` run still needs fuel as large as the configuration's numeric value. `PolyFueled.prec`'s `hst` bounds the iterated state's value where the machine-side `FPFold.foldlBits_mem_FP`'s `hbnd` bounds its length, and `Framework/Machine/WriteOutMachine.lean` exhibits the two signatures side by side. **No token-metered class survives as a binder.** The place they would live is the LUV *threshold* lane — `LUV.RpnThresholdCodes(Seq)` unfold to `RpnSentenceCodes` on the threshold family `⌜Xₙ > i/k⌝`, so a census grepping signatures for `RpnSentenceCodes` does not see them — and every one of those interfaces is stated at `LUV.MachineThresholdCodes(Seq)` instead: `LUVCombinationSyntax.threshold_poly` and the four canonical `_ofSyntax` endpoints, `ConvergencePresentation.threshold_code` and the single-LUV chain it feeds, `lic_expectation_indicator` (`thm:ei`), and the quote certificates of `thm:cee`, `thm:ceu`, `thm:ccee`, `thm:epr`, `thm:er` and `thm:st`. Why the threshold route is a rendering sensitivity rather than a narrowing of the paper's admissible LUVs — along it the metered object is a formula string over a fixed finite alphabet, so the per-token *value* clause is vacuous and the class is exactly polynomial length — is set out under *LUV-threshold metering* in the classification ledger.",
 ...
 'def:deferralfunc': "`lt` is f(n) > n; `graph_fp` renders \u201cf computable in time polynomial in f(n)\u201d at the machine model, by deciding the *graph* f n = m in `Complexity.FP` on the unary pair \u27e8n, m\u27e9, whose length dominates m \u2014 so the meter is polynomial in the *output*, exactly as the paper demands (and f may grow fast).",
 'def:ece': "`GeneratedRatFeature` is \u2018\u2119\u203e-generable\u2019: a rank-bounded, polynomially emitted expression whose denotation against the market's own prices is the sequence. Compare clause by clause \u2014 nothing about the values themselves is assumed. The emitter field `polyTok` is write-out metered at the machine class (`MachineSpliceStream`), so a constant leaf may name a rational whose G\u00f6del code is exponential in the day: `PGenerableRat.ofMachineRatCodes` is the general constructor, at `MachineRatCodes`, and `pGenerableRat_two_pow_inv` witnesses that the paper's own \u03b4\u2099 = 2\u207b\u207f is admissible.",
 ...
 'def:luv': "The paper's LUV is a first-order formula free in one variable, and `PaperLUV` is that object literally, so it leads: an `ArithmeticSemisentence 1` carrying object-level `T`-proofs of unique existence and `[0,1]` membership. `toLUV` compiles it into the abstract threshold carrier `LUV` (field `gt`) that downstream results consume — `LUV` is deliberately second, being the over-general one, admitting threshold families that are not literal paper LUVs. `PCWorld.ValuesAt` is *derived* through `paperTheoryDP` and the rational cut rather than assumed, and `PaperLUVSeq` compiles the literal threshold syntax to `LUV.MachineThresholdCodeSeq` under `structural : MachineArithmeticSourceSeq`, which is `def:ec`'s condition on the defining formula string and not a narrowing of the admissible LUVs: inhabited by the varying `1/(n+1)` family and, at a superpolynomially small value, by `dyadicPaperLUVSeq` at `2⁻ⁿ`, whose denominator is the compact `ℒₒᵣ` term `binNumeral (2 ^ n)`. The object-level value is named by a numerator/positive-denominator pair code.",
 ...
 'thm:lia': "One hypothesis — the deductive process is computable — and the conclusion instantiates the criterion at the constructed market `liaHistory DP`, **at the paper's own quantifier**: `LIA_is_logical_inductor` concludes `IsLogicalInductor`, `def:lic` over `EfficientlyComputable`. This is the paper's main theorem in its constructive form.",
 ...
 'thm:dus': "`B.prefixSentence \u03c3` is the paper's conjunction of fresh-symbol literals for the bit string \u03c3; M ranges over lower-semicomputable continuous semimeasures. The caller inputs shown are discharged by constructed witnesses, in two layers. `ordinaryBitPrefixSentences` over `emptyBitDeductiveProcess` is **inhabitation only** \u2014 every stage is \u2205 and `realizable` is vacuous. The substantive layer is `paperIndependentBitAtoms`/`paperBitPrefixSentences` over `paperDP T`, the market the whole \u00a74 tail runs on, where `realizable` is proved; through the constructed dovetail it yields `lic_domination_dovetailSemimeasure_paperDP` and `lic_domination_everyLowerSemicomputable_paperDP`, which close the node with no caller input and no vacuous premise. Cite that layer for non-vacuity; it is also why this row's axis reads `instantiated` while `thm:strict`'s stays `universal`.",
 ...
 'thm:scon': "Fixed form adjoins one ψ, growing form a whole computable process; the conclusion is the criterion for the *conditioned* history over the union process — the paper's ℙ‾|ψ. No joint-consistency premise: the degenerate branch covers unsatisfiable stages. All five shown endpoints conclude `IsLogicalInductor`, the paper's own quantifier; the closed pair takes exactly the hypotheses of the conditional forms and concludes the criterion over the union process. **Class note.** The condition's certificate is `MachineSentenceCodes`, the machine reading of `def:ec`'s write-out class: both `condition_codes` fields are stated at it (`ConditioningPresentation`, `CompactConditioningProcessComputation`), and `CondStep.machineSentenceBlocks_of_machine` reads the block word off the `Complexity.FP` witness directly, a machine-metered stream already carrying `TokenFold.BlockWF` and its decode law. The `Rpn` in the fuel transports' names is the RPN *symbol model*, not a sentence class. `RpnSentenceCodes` remains a sufficient subclass, reached by `RpnSentenceCodes.toMachine`; no sentence-level strictness between the two is proved here.",
 'thm:ifp': "**The published unrestricted theorem is FALSE, and the first endpoint below proves it false.** `not_overgeneral_ifp` is closed but for the deductive process and axiom-clean, at the paper's own quantifier: the constructed LIA, perturbed on day 0 only, publishes diagonal sign bits as advice-atom prices, and an efficient trader reads them through historical price features without ever computing them. A single changed pricing day is an infinite computable function, which is enough to carry unbounded advice; the published proof's invalid step is its “only finitely many constants” claim. The **corrected** theorem is `FreezeOracle.lic_iff_of_finiteSupport`: two computable markets differing on only finitely many (day, sentence) coordinates satisfy the criterion together — strictly stronger than the paper's tail agreement in the direction that survives, and exactly the case where the appendix's constant table really is finite. It takes no patch argument and no condition of any kind on the finitely many moved sentences: both halves of the former `Recognizable` side condition are discharged by building the `Complexity.FP` device each stood for (`FiberTest.fiberW_mem_FP` for the escape-leaf decode test, `CtrAuto.ctrMachine` and `PayAuto` for the structured payload recognizer). What is disclosed in their place is a property of the construction, not of the statement: the recognizer is compiled per frozen sentence, so its polynomial-time constants depend on that sentence — the paper's own hard-coded finite table, sound exactly because the support is finite. `lic_iff_twoPoint` makes it non-vacuous and `logicalInductor_liaPerturbed` — the third endpoint — makes it informative: applied to LIA with one price moved, it derives a machine logical inductor no construction here produces. The fuel-class carriers `lic_iff_of_finitePerturbation` / `lic_iff_of_finiteSupportPerturbation` are compatibility only and their patch hypotheses are *uninhabited*; they and `lic_iff_of_finiteSupportPerturbation_ofPatches`, which the corrected theorem supersedes, are deliberately not shown.",
 ...
 'thm:simcal': "The calibration indicator (price in [a,b]) is itself the weighting; conclusion pins limit points of the weighted truth-average to [a,b]. The endpoint takes the indicator's generability and the affine sequence's e.c. certificate as *arguments*, and tex:1188 merely asserts the first — but both are proved here from the paper's own hypotheses, by `calibrationIndicator_pgenerable` and `AffineCombination.sentenceAffine_polySequence`, shown beside it. The ⟨φ⟩ half reaches the paper's class exactly (`AffineCombination.PolySequence`, whose sentence and coefficient fields are the machine readings `MachineSentenceCodes`/`MachineSpliceStream`); the ⟨δ⟩ half is `PolyPositiveWidths`, whose `codes` field is `MachineRatCodes` and whose other field is just `∀ n, 0 < δ n`, so nothing bounds a width's magnitude and the paper's own 2⁻ⁿ is admitted (`polyPositiveWidths_two_pow_inv`).",
 'thm:wub': "The three operational premises are tex's own: generable divergent weighting supported on the deferral image (`hsupport`), strictly increasing f, and `FeedbackTruthComputation` — the delayed-truth program clocked polynomially at f(k+1), a *weaker* demand than the paper's O(f(n+1)). That premise is inhabited by more than a constant: `alternatingFeedbackTruthComputation_nonempty` is a genuine mixed-truth witness, with `exists_nonconstant_feedbackTruthComputation` exhibiting `truth (f 0) = 1`, `truth (f 1) = 0`. The universal form over any `[IsLogicalInductor]` leads; the `_unconditional` form discharges `hworld` but pays three arithmetic-theory class hypotheses the paper does not impose and is not about all inductors, so it comes second rather than alone.",
 ...
 'thm:recurringunbiasednessexp': "LUV-combination version: `WorldValued` is def:luv's world-value clause, `DeterminedViaTheory` def:affthmval — both the paper's own representation premises. **Erratum PE2:** the printed statement is garbled, carrying a spurious “support of ⟨w⟩ ⊆ image of f” clause that refers to an f it never introduces — a clause that belongs to `thm:wubexp` and is missing there. The affine twins `thm:wubaff`/`thm:prandexp` prove the intended placement. The Lean statement is the repair, and now says so at the declaration: no deferral function, no support clause, concluding `HasLimitPoint 0`.",
 'thm:prandexp': "Expectation pseudorandomness; same premise pair as `thm:recurringunbiasednessexp` plus the paper's pseudorandomness condition over a deferral function. The paper prints only the ≳ direction, so `prandexp_above` leads; `_below` and the two-sided `prandexp` are the other comparison directions.",
 ...
 'thm:loe': "The paper's \u0393 \u22a2 Z\u2099 = a\u2099X\u2099 + b\u2099Y\u2099 is encoded as: the combination a\u2099X\u2099+b\u2099Y\u2099\u2212Z\u2099 is determined with value 0 (`hdet0`). The conclusion is the paper's asymptotic linearity, unfolded. The coefficients are the paper's bounded \u2119-generable sequences carried by their *generating features* — `a b : \u2115 \u2192 EF`, valued `(a n).denote P` — so `BoundedSequence`'s emission field charges the feature's syntax and not the coefficient value's digits; a client with literal rationals passes `fun n => .const (a n)`.",
 ...
 'thm:wubexp': "The normalization bound b appears *inside* the feedback premise's type (`C` is about the normalized mesh) \u2014 that is the paper's own \u2018thmval of the combination computable by the deadline\u2019 premise, packaged operationally. Determination is at the paper's combination level (`def:affthmval`); the mesh bridge is built from the vanishing mesh residual, so no per-component-LUV determinedness is assumed. **Erratum PE2, declared not denied.** The Lean carries `hsupport` — the support of ⟨w⟩ lies in the image of the deferral function `f` — which this node does *not* print. It is printed instead on `thm:recurringunbiasednessexp`, a statement introducing no `f` at all; the affine twins `thm:wubaff`/`thm:prandexp` prove the intended placement. The repair is taken in both directions: this node carries the un-printed clause, and the mirror theorem drops the spurious printed one. Earlier notes here claimed the premises were exactly the printed ones; that was wrong in both halves.",
 'thm:epr': "Closed over the constructed inductor: the quoted-price LUV is built from the market program itself (`paperPriceQuoteCode`), so both sides of the paper's display are named objects; only \u03c6\u203e and its codes remain. The theory hypothesis here is `[Entailment.Consistent T]`, the paper's own premise. Σ₁-soundness would be stronger than tex:600-606 / tex:993-997's consistent, c.e., ‘represents computations’, and tex:2673 declines it. It is not needed: the two quotation schemas are the value-`1` and value-`0` fibers of ONE `code` formula, so `Θ ⊢ ∼(pos ⋏ neg)` is a theorem of Θ and the constructed stage world's plausibility proof (`theoremDP_hworld`) gets fiber exclusivity at tag 5 from consistency alone, without the provable⇒true direction of Foundation's `re_complete`; no endpoint anywhere carries a soundness instance.",
 'thm:er': "Same pattern one level up: the quoted LUV is the market's own day-n expectation of X\u2099; premises are the LUV sequence and its threshold codes. The theory hypothesis here is `[Entailment.Consistent T]`, the paper's own premise. Σ₁-soundness would be stronger than tex:600-606 / tex:993-997's consistent, c.e., ‘represents computations’, and tex:2673 declines it. It is not needed: the two quotation schemas are the value-`1` and value-`0` fibers of ONE `code` formula, so `Θ ⊢ ∼(pos ⋏ neg)` is a theorem of Θ and the constructed stage world's plausibility proof (`theoremDP_hworld`) gets fiber exclusivity at tag 5 from consistency alone, without the provable⇒true direction of Foundation's `re_complete`; no endpoint anywhere carries a soundness instance.",
 ...
 'thm:cee': "Deferred expectation version; `source_valued` is the paper's \u2018X\u2099 is an LUV of \u0393\u2019 (every completed world values it), the one semantic premise. The theory hypothesis here is `[Entailment.Consistent T]`, the paper's own premise. Σ₁-soundness would be stronger than tex:600-606 / tex:993-997's consistent, c.e., ‘represents computations’, and tex:2673 declines it. It is not needed: the two quotation schemas are the value-`1` and value-`0` fibers of ONE `code` formula, so `Θ ⊢ ∼(pos ⋏ neg)` is a theorem of Θ and the constructed stage world's plausibility proof (`theoremDP_hworld`) gets fiber exclusivity at tag 5 from consistency alone, without the provable⇒true direction of Foundation's `re_complete`; no endpoint anywhere carries a soundness instance.",
 'thm:ccee': "Exact zero-slack endpoint on the single market `liaHistory (paperDP T)`, for the paper's literal first-order source class `PaperLUVSeq`. A `PaperLUV` names its value by a numerator/positive-denominator pair code, so two of them multiply exactly inside arithmetic (`paperProductPaperLUV_valuesAt`), and the deferred weight is represented inside T by `RepresentsComputations` — the paper's own \u00a72 premise on \u0398 — rather than rendered as a numeral, which a merely-computable weight forbids. What is represented is the weight's pair *function*, i.e. its values day by day: `RepresentsComputations.repr` takes the function plus a `Computable` proof and the formula family it yields is determined by that extension, so two different generability certificates for the same weight give identical formulas. The deductive process is untouched: `paperTheoryDP T` already enumerates every T-theorem, so the representing formula may be chosen after `f` and `w`. Elaborated binders: Theory.Delta1 T, ISigma1 weaker-than T, and RepresentsComputations T — all three instantiated by PA; consistency comes from RepresentsComputations.consistent, so no separate consistency binder appears. Non-vacuity of the source side is witnessed in file by the paper's own 1/(n+1) family `unitFracPaperLUVSeq` going through the endpoint over PA. The general abstract-LUV form `lic_no_expected_net_update_conditional_closed` stays available on the same market as the more general-input, `dd:mesh`-approximate result; `lic_no_expected_net_update_conditional_exact_canonical` is retained as a generalized semantic-extension result rather than the paper rendering: exact zero-slack over one non-vacuous canonical process fixed from T. A fixed old-language lift and executable finite-entailment gate internally admit every arbitrary e.c. source satisfying the paper's completed-world [0,1]-valued premise; deferred weight, exact product, and right quotation are constructed internally. Non-vacuity is witnessed on both sides: the process side by `canonicalCCEEDP_computable` / `canonicalCCEEDP_hworld`, and the weight side by `canonicalCCEE_weight_nonvacuous`, which exhibits the non-constant harmonic weight `n \u21a6 1/(n+1)` as an admissible `weight_generable` over `liaHistory (canonicalCCEEDP T)`. The paper rendering additionally has a fully closed instance — PA, `unitFracPaperLUVSeq`, `succDeferral` and the harmonic weight discharge every binder — as an in-file client example, and `succDeferral` is the first constructed inhabitant of `DeferralFunction`. The theory hypothesis here is `[Entailment.Consistent T]`, the paper's own premise. Σ₁-soundness would be stronger than tex:600-606 / tex:993-997's consistent, c.e., ‘represents computations’, and tex:2673 declines it. It is not needed: the two quotation schemas are the value-`1` and value-`0` fibers of ONE `code` formula, so `Θ ⊢ ∼(pos ⋏ neg)` is a theorem of Θ and the constructed stage world's plausibility proof (`theoremDP_hworld`) gets fiber exclusivity at tag 5 from consistency alone, without the provable⇒true direction of Foundation's `re_complete`; no endpoint anywhere carries a soundness instance.",
 'thm:ref': "The interval sentence ⌜aₙ < ℙₙ(φₙ) < bₙ⌝ is constructed from the market's exact rational quote; a,b enter as generated features (the paper's ℙ‾-generable bounds), δ as the vanishing width; ε‾ is the paper's ‘accuracy’ sequence, existentially produced. The `PolyRatCodes a`/`PolyRatCodes b` hypotheses that once stood here are **gone** — they were consumed only as `.computable`, which `PGenerableRat.computable` supplies from the `MarketComputation` already in scope. Sentences enter as `hφ : MachineSentenceCodes φ` and the width as `MachineRatCodes δ`, both write-out metered: what is bounded is the time to write each code out, not its magnitude, so any e.c. ⟨δ⟩ → 0 qualifies, exponentially vanishing widths included. The universal `lic_introspection` carries the width hypothesis invisibly, inside `IntrospectionIntervalQuote.inverse_width_codes`, which is `MachineRatCodes` for the same reason. The theory hypothesis here is `[Entailment.Consistent T]`, the paper's own premise. Σ₁-soundness would be stronger than tex:600-606 / tex:993-997's consistent, c.e., ‘represents computations’, and tex:2673 declines it. It is not needed: the two quotation schemas are the value-`1` and value-`0` fibers of ONE `code` formula, so `Θ ⊢ ∼(pos ⋏ neg)` is a theorem of Θ and the constructed stage world's plausibility proof (`theoremDP_hworld`) gets fiber exclusivity at tag 5 from consistency alone, without the provable⇒true direction of Foundation's `re_complete`; no endpoint anywhere carries a soundness instance.",
 'thm:lp': "The self-referential \u03c7 \u2248 \u2018\u2119\u2099(\u03c7\u2099) < p\u2019 is the constructed public diagonal (`paperDiagonalQuoteCode` at parameter p); the conclusion drives its price to p. The statement is width-free: it binds only `(p : ℚ)` with `0 < p < 1`, and the paper's vanishing interval is discharged internally at `2⁻ⁿ` rather than asked of the caller. The statement carries no `width` binders: the eliminability argument is executed rather than merely made; the parametric twin `lic_paradox_resistance_ofDiagonal` keeps its width parameters, which its own proof consumes.",
 'thm:st': "A is the indicator product 1(φₙ)·Ind, B the confidence indicator Ind(ℙ_f(n)(φₙ) > pₙ) — both constructed from the market program. The four hypotheses correspond to tex:2093's four: deferral function, e.c. sentences, e.c. positive δ‾, generable p‾. ⟨φ⟩ enters as `hφ : MachineSentenceCodes φ`, the machine reading of `def:ec`'s write-out class, and `hδ` as `MachineRatCodes δ`; `SelfTrustQuote`'s product and confidence fields are `LUV.MachineThresholdCodeSeq`. `p` carries `PGenerableRat`, whose emitter field is `MachineSpliceStream`, so a value-exponential but polynomially writable probability sequence — 1 − 2⁻ⁿ, say — is admissible data. The deferral function `f` is machine-metered too, on a *pair* rather than on the day: `DeferralFunction.graph_fp` decides the graph `f n = m` in `Complexity.FP` on the unary `⟨n, m⟩`, whose length dominates `m`, which is the paper's own output-sensitive clause (tex:1244) read on the input that carries it. The theory hypothesis here is `[Entailment.Consistent T]`, the paper's own premise. Σ₁-soundness would be stronger than tex:600-606 / tex:993-997's consistent, c.e., ‘represents computations’, and tex:2673 declines it. It is not needed: the two quotation schemas are the value-`1` and value-`0` fibers of ONE `code` formula, so `Θ ⊢ ∼(pos ⋏ neg)` is a theorem of Θ and the constructed stage world's plausibility proof (`theoremDP_hworld`) gets fiber exclusivity at tag 5 from consistency alone, without the provable⇒true direction of Foundation's `re_complete`; no endpoint anywhere carries a soundness instance.",
 'thm:halts': "The market is the single `paperDP T` — the union of Θ's provability process `paperTheoryDP T` (every `T`-provable proposition, no fixed schema) with the literal stream `theoremDP T`, one market for the whole §4 tail as in the paper — and the endpoint is stated over `liaHistory (paperDP T)` under `[T.Δ₁] [𝗣𝗔⁻ ⪯ T] [Entailment.Consistent T]`, with **no** soundness instance and **no** `RepresentsComputations` instance: the claim's positive literal is Σ₁-completeness alone (`re_complete_mp`) and the stage world exists from consistency (`paperDP_nonvacuous`). That is exactly what the paper's own proof (tex:4495-4520) uses. **The sentence names the machine.** The day-n claim is the *fixed* universal r.e. schema `universalHaltingSchema = codeOfREPred UniversalCodeHalts` at the argument ⟨⌜mₙ⌝, xₙ⟩, written into the sentence as the compact Horner numeral `binNumeral (haltingClaimInput (machines n) (inputs n))`. The machine and its input are in the *argument*, not inside the schema, so the claim family genuinely depends on them — proved, not asserted: `haltingArgClaimSentence_ne_of_halts_ne` shows two machine/input pairs differing in halting behaviour get different claim sentences. `hm : MachineMachineCodes machines` names each machine by a linear, decodable in steps linear in the source length (`ofSource_peelSteps`, `sourceNat_peelSteps_le`) *source* encoding of `Nat.Partrec.Code` — Mathlib's `Encodable.encode` on that type squares per node and is deliberately not used for naming — and `hi : MachineDigits inputs` meters the inputs the same write-out way. (The fuel-metered `DigitMachineCodes`/`BigDigits` are producer routes into those two, by `.toMachine`.) Both halves therefore reach the paper's classes rather than narrowing them: tex:1931-1933 asks exactly that the source code be writable in time polynomial in n, and tex:1946-1952 that ⟨y⟩ range over an e.c. sequence of bitstrings, whose values are exponential in their length. They are load-bearing rather than decorative: the argument numeral's symbol run is emitted digit by digit from those two certificates, which is what discharges `def:ec` for the family. The public atom's vacuous ∃⁰ wrapper is proved invisible (`provable_schemaArgClaim_iff`). Foundation's *unary* numeral would have cost the argument its own value in symbols; the paper fixes no numeral notation, the compact term costs O(log v) nodes, and provability is insensitive to the choice (`provable_subst_iff_of_val`). Nothing is charged on theory strength: `QuotationTheoryPresentation` carries no `theory_sigmaOne` field for this row to inherit `[𝗜𝚺₁ ⪯ Θ]` from, and the census over the 107 canonical endpoints finds `𝗜𝚺₁ ⪯ ·` in exactly four signatures, none of them this one. The binders are now `[T.Δ₁] [𝗣𝗔⁻ ⪯ T] [Entailment.Consistent T]` — the paper's own premise on Θ plus the two globally charged binders `[T.Δ₁]` and `[𝗣𝗔⁻ ⪯ T]`, disclosed once in the classification ledger's *Arithmetic-theory hypotheses* section and levied against no row. The row is `exact`.",
 'thm:loops': "Dual of `thm:halts`, over the same single market `paperDP T`, the same instances `[T.Δ₁] [𝗣𝗔⁻ ⪯ T] [Entailment.Consistent T]` — no soundness, no `RepresentsComputations` — and the same machine-naming claim family, so the sentence account and the write-out accounting of `thm:halts` apply verbatim. `hloops` is the paper's own premise that T *refutes* each halting claim, spelled as the literal negation of the very sentence the conclusion is about: `∀ n, T ⊢ ∼(haltingArgClaimInstance machines inputs n)`, the fixed universal schema at the compact name of ⟨⌜mₙ⌝, xₙ⟩; the price → 0. It is object-level refutability, not an emission surrogate, and `provable_neg_schemaArgClaim_iff` proves it interchangeable with the public atom's vacuous ∃⁰ wrapper. `hloops` is inhabited by `loopsTheory` = 𝗜𝚺₁ plus one true Π₁ axiom — the negation of that same claim sentence at the constant never-halting family — which is Δ₁, consistent, Σ₁-sound and true in ℕ, but refutes **by axiom fiat**: `loopsTheory_refutes` is `by_axm`. The obstruction is representational and untouched by the machine-naming repair: `codeOfREPred` picks its formula by `Classical.epsilon`, so the schema's shape is unreachable and every bridge to `T ⊢ …` is positive. One sentence suffices where the previous rendering needed a ∀-closure, because the *witness machine family* is constant. Nothing is charged on theory strength: `QuotationTheoryPresentation` carries no `theory_sigmaOne` field for this row to inherit `[𝗜𝚺₁ ⪯ Θ]` from, and the census over the 107 canonical endpoints finds `𝗜𝚺₁ ⪯ ·` in exactly four signatures, none of them this one. The binders are now `[T.Δ₁] [𝗣𝗔⁻ ⪯ T] [Entailment.Consistent T]` — the paper's own premise on Θ plus the two globally charged binders `[T.Δ₁]` and `[𝗣𝗔⁻ ⪯ T]`, disclosed once in the classification ledger's *Arithmetic-theory hypotheses* section and levied against no row. The row is `exact`.",
 'thm:incons': "The endpoint is stated at the paper's own generality: an arbitrary **machine-enumerated** recursively axiomatized theory sequence. `lic_disbelief_inconsistent_theories_unconditional` takes a machine family `m : ℕ → Nat.Partrec.Code` with `hm : MachineMachineCodes m` and the paper's own inconsistency premise `hinc : ∀ n, ¬Entailment.Consistent (theoryOf (m n))`, stated at the day's *theory* itself rather than at any provability surrogate. `Θ′ₙ = theoryOf (mₙ)` is freestanding — no base theory, unrelated to the market's Θ, no Δ₁ hypothesis of its own, and possibly **infinitely axiomatized**. Both conjuncts of the display are concluded. **The `def:ec` premise is on the naming of the sequence, which is what the paper meters.** tex:1905 asks that the theories be “efficiently named” and tex:1931 that one can “write out the source code specifying `mₙ`”, the runtime of an individual `mₙ` being immaterial; `MachineMachineCodes` is the standing write-out class the halting lane already uses for machines, with the fuel-metered `DigitMachineCodes` as its producer route, and what the sentence emits is `binNumeral ((m n).sourceNat)`, the machine's own tag stream read base-16, whose base-4 digit count is linear in its written source. Nothing about the day's axioms is metered: they are produced *inside* the machine, and the spliced axiom window is parsed *inside* the represented predicate, where the paper asks only for recursive enumerability. **How uniformity is obtained without a uniform derivability predicate.** Foundation's `Derivation T` takes `T` as a **meta** parameter, so no uniform-in-theory-code derivability predicate can be formed at this level — that obstruction stands, and this rendering sidesteps it rather than contradicting it, never forming such a predicate. It quantifies over coded machines *externally*, at `V := ℕ`, and buys uniformity from **compactness**: an inconsistency is witnessed by finitely many axioms (Foundation's proof object carries its own axiom list), a finite list of written axioms splices into one written conjunction at token level (`combineSourceNats`) — **gated per entry**, a slot contributing only a number that is literally the name of its own decoded run whose run is the complete emitted run of one `ArithSource 0` compiling to a sentence (`AdmissibleName`, applied by `gateName`, the completeness test run by the recognizer `sourceRun`); anything else is replaced by the inert ⊤ — and refuting that conjunction is a question of pure logic. So the represented predicate `MachineTheoryInconsistent z := ∃ w, ProvableCode ∅ (negWindowCode z w)` runs over the **empty theory** and mentions no base theory at all; it is r.e. because its matrix is decidable, via `Partrec.rfind`/`Partrec.dom_re`. **The sentence names the day's theory, and the schema is non-constant unconditionally.** `inconsistencySchema = codeOfREPred MachineTheoryInconsistent` is one universal schema for the whole theorem, with the day's data written in as `binNumeral (machineArg m n)`; `inconsistencySchema_mentions_zero` is proved with **no hypothesis at all** — the machine that keeps writing ⊥ presents an inconsistent theory, the machine that never writes presents the empty one — where the previous deduction-family version needed `Consistent Θ₀`. Representing “the theory is inconsistent” directly would have been the trap, since under `hinc` that predicate is constantly True. **Non-vacuity is discharged by the construction, at two witnesses.** `thm_incons_applied_deep` runs at `mₙ = dayMachine (comp deepSourceCode left) n`, whose day-n theory is the single axiom `(∀x. A(x) ⟺ ⋯ ⟺ A(x)) ∧ ⊥` — `5n + 7` symbols as the paper writes it, `≥ 2^n` nodes in normal form (`two_pow_le_encode_iffChain`), Gödel code doubly exponential in that — while the machine's own source stays `O(n)`, so it is the witness that exercises the metering gap; `inconsistencyArgClaimSentence_deep_ne` separates the claim sentences of every pair of distinct days. `thm_incons_applied_infinite` runs at `mₙ = dayMachine deepSourceCode n`, which writes a different axiom on every input, so `theoryOf (mₙ)` is **infinite** (`infinite_theoryOf_infiniteDayMachine`) — genuinely recursively axiomatized rather than finitely axiomatized, which is what the paper's own examples 𝗣𝗔 (tex:1859) and ZFC (tex:1889) are, and which a deduction family adjoining one sentence can never be. Both are fully applied with nothing left to the caller. No symbol measure is in play: this node's sentence is the unbounded existential over proofs, so §4.10's counting convention `dd:symbolcount` does not arise. What reading a machine *as* a theory requires is a presentation convention, `dd:machinetheory`, disclosed as a convention rather than a substitution. **The gate is what makes the sentence say the convention's claim.** Ungated, `MachineTheoryInconsistent` was strictly broader than that claim — it held of machines presenting the **empty** theory, by prefix truncation, by splice-across-entries, and by a genuine source compiling to a formula with free variables, which ∅ refutes without its being a sentence of any theory. The per-entry gate (`AdmissibleName`/`gateName`, decided by the binder-depth-tracking recognizer `sourceRun`) closes all three, and `machineTheoryInconsistent_iff` proves the represented predicate **equivalent** to `¬Entailment.Consistent (theoryOf m)`, so the day-n sentence says exactly what `hinc` assumes, in both directions. **The surjectivity justification is scoped.** What is proved is the per-sentence half: `theoryOf_const_ofNNF` gives `theoryOf (Code.const ⌜written σ⌝) = {σ}`, every one-axiom theory presented exactly, `ArithSource.ofNNF` writing every sentence. The *uniform* half — one machine enumerating the names of any given r.e. set of sentences — is **not formalized**, would need `encodeArithmeticFormulaSymbols` certified primitive recursive, and is not consumed by the endpoint, `hinc` being stated at the caller's own machine. Theory hypotheses are only the market's: `[T.Δ₁] [𝗣𝗔⁻ ⪯ T] [Entailment.Consistent T]` — no soundness, no 𝗜Σ₁, no 𝗥₀, no second theory and no `[T'.Δ₁]`; `[T.Δ₁]` and `𝗣𝗔⁻ ⪯ T` are the two globally charged binders `[T.Δ₁]` and `[𝗣𝗔⁻ ⪯ T]`, disclosed once in the classification ledger's *Arithmetic-theory hypotheses* section and levied against no row. **The row is `exact`.** It carries no deduction-family paraphrase and no Gödel-code-metered `def:ec` premise; both would be charges, and neither is taken.",
 'thm:pac': "Read the two panes against each other. The paper (tex:1869-1875) prices Con(Γ)(⌜⌜f⌝(⌜n⌝)⌝), the formula ‘no proof of ⊥ from Γ in ≤ ν **symbols**’ (tex:1855-1866). **The Lean endpoint now prices exactly that family.** `lic_belief_finitistic_consistency_unconditional T horizons hh`'s day-n sentence is `conClaimSentence (conGamma T T hh) n` — the value-0 sentence ∀ν(γ(t,ν) ⟺ ν = 0̄) of the formula γ that `RepresentsComputations` returns for the **universal bounded-provability decider** `conRunValue T f` (`Framework/Theory/BoundedConsistency.lean`), at the compact argument `binNumeral ⟨⌜⊥⌝, n⟩`. So the sentence names ⊥ and the day, the γ names the theory — the decider's extension is T's bounded theorems — and one γ serves every day of a horizon, which is the paper's ⌜f⌝. The horizon is an arbitrary computable function named by its program (`ComputableHorizon`) and evaluated *inside* the represented decider, so diagonal Ackermann is admissible (`not_polyNatCodes_ack` proves that generalization strict) and the in-file example runs the theorem at Θ = 𝗜Σ₁ with horizon `fun n => ack n n`. **The truth premise is proved, not assumed:** the endpoint carries no `hconsistent` and no `[Entailment.Consistent T]` — consistency comes from `RepresentsComputations.consistent`, and `conWithin_of_consistent` derives the truth of every day's claim from it. **Non-collapse is a theorem, not a side condition.** The day sentences are pairwise distinct because γ genuinely mentions its argument: `mentions_zero_of_repr_ne` (`Framework/Theory/RepresentsComputations.lean`, from `Semiformula.rew_eq_of_not_mentions`) derives `γ.Mentions 0` from the representation spec alone whenever the represented decider is non-constant, and the Con lane discharges it at `conGamma_mentions_zero`, with `conGamma_mentions_zero_of_bProv` and `conGamma_mentions_zero_of_horizon_unbounded` as usable sufficient conditions and `conGamma_mentions_zero_ackermann` fully discharged at the paper's own illustration. The only case where collapse remains possible is degenerate and disclosed at the endpoint: at an eventually bounded horizon — in the limit, constantly 0 — the decider is constant, and there a γ ignoring its argument does represent it. The metering is the paper's own: the day-n search is `BProv T ⌜⊥⌝ k := ∃ d, Proof T d ⌜⊥⌝ ∧ dSize d ≤ k` — ‘ν or fewer symbols’, with the bound **inclusive**, as tex:1855-1866 writes it. `dSize` (`Framework/Theory/DerivationSize.lean`) is a total symbol count on Foundation's internal derivation codes, tied to Foundation's own constructors by equation (`dSize_axL`, `dSize_cutRule`, …), and the converse bound `le_G_dSize : d ≤ G (dSize d)` keeps the symbol-bounded search finite and so decidable in both polarities. Metering by the derivation's Gödel number instead would be a substitution — a code bound is a different predicate — and is not used here; what this row carries is a **convention** — the paper fixes neither encoding nor alphabet, so a counting convention is unavoidable, and ours charges each variable/function/relation index at its binary digit length plus one marker token (`idxLen n = Nat.size n + 1`), which is what makes the measure finite-fibred. It is recorded as `dd:symbolcount`, and the truth of every instance is independent of it (`conWithin_of_consistent`; the convention affects only which horizons discharge the non-degeneracy side conditions). This row reads *exact*. **This node is the diagonal of a two-theory family:** `conGamma`, `conGamma_spec` and `representedConClaims` all take a second theory parameter, and `thm:pac` is the instantiation Θ′ = Θ of the same parametric family that `thm:pazfc` runs at a genuinely stronger Θ′. The former carrier `lic_belief_finitistic_consistency_ofComputation` — an arbitrary decidable `consistentWithin` presented by a `BoundedComputation`, with consistency supplied by the caller — does not exist; this node has exactly one carrier.",
 'thm:pazfc': "The paper's node (tex:1881-1886) is about a *second*, stronger theory Θ′, and that parameter is carried in the Lean statement. `lic_belief_stronger_theory_consistency_unconditional T T' hcons horizons hh` prices the arithmetized finite-consistency family of Θ′ in Θ's own market: the inductor is `liaHistory (paperDP T)`, the single market, whose process unions the enumeration of what **Θ** proves with the literal stream — trained on Θ's own commitments and on nothing about Θ′ — while the day-n sentence is `conClaimSentence (conGamma T T' hh) n` — the value-0 sentence ∀ν(γ(t,ν) ⟺ ν = 0̄) of the **Θ-formula** γ that `RepresentsComputations T` returns for the universal bounded-provability decider of the **second** theory, `conRunValue T' f` (`Framework/Theory/BoundedConsistency.lean`), at the compact argument `binNumeral ⟨⌜⊥⌝, n⟩`. The two theories are genuinely distinct parameters: Θ′ supplies the derivations being metered, Θ the derivations being enumerated. One γ serves every day of a horizon — the paper's ⌜f⌝ — and the horizon is an arbitrary computable function named by its *program* (`ComputableHorizon`) and evaluated inside the represented decider, so diagonal Ackermann is admissible. **The paper's own illustration is the in-file example:** Θ = 𝗜Σ₁, Θ′ = 𝗣𝗔, horizon `fun n => ack n n` — a theory that does not prove Con(𝗣𝗔), whose inductor's belief in Con(𝗣𝗔)(⌜Ack(n,n)⌝) nevertheless converges to 1, with every instance discharged and the whole thing axiom-clean. **The truth premise is the paper's own:** `hcons : Entailment.Consistent T'` is exactly 'let Θ′ be consistent', and `conWithin_of_consistent` derives the truth of every day's claim from it alone; there is no soundness hypothesis, no `hworld` and no presentation argument anywhere on the endpoint. **No hypothesis relates Θ and Θ′ — and the paper states none either.** tex:1881-1886 assumes of Θ′ only that it is a stronger consistent recursively axiomatizable theory, so the Lean statement *matches* the paper's hypotheses rather than generalizing them; a containment would in any case be unusable, since `conRunValue T' f` is total computable for any Δ₁ Θ′, `RepresentsComputations T` supplies the representing formula, and the day's truth is a fact about Θ′. What makes the result *interesting* is the informal case where Θ cannot prove Con(Θ′), and the 𝗜Σ₁/𝗣𝗔 witness carries that concretely. (Do not call the statement 'more general than the paper' for omitting a premise the paper never had.) **Non-collapse is a theorem, not a side condition.** The day sentences are pairwise distinct because γ genuinely mentions its argument: `mentions_zero_of_repr_ne` (`Framework/Theory/RepresentsComputations.lean`, from `Semiformula.rew_eq_of_not_mentions`) derives `γ.Mentions 0` from the representation spec alone whenever the represented decider is non-constant, and the Con lane discharges it at `conGamma_mentions_zero`, with `conGamma_mentions_zero_of_bProv` and `conGamma_mentions_zero_of_horizon_unbounded` as usable sufficient conditions and `conGamma_mentions_zero_ackermann` fully discharged at the paper's own illustration. The only case where collapse remains possible is degenerate and disclosed at the endpoint: at an eventually bounded horizon — in the limit, constantly 0 — the decider is constant, and there a γ ignoring its argument does represent it.  The metering is the paper's own: the day-n search is `BProv T ⌜⊥⌝ k := ∃ d, Proof T d ⌜⊥⌝ ∧ dSize d ≤ k` — ‘ν or fewer symbols’, with the bound **inclusive**, as tex:1855-1866 writes it. `dSize` (`Framework/Theory/DerivationSize.lean`) is a total symbol count on Foundation's internal derivation codes, tied to Foundation's own constructors by equation (`dSize_axL`, `dSize_cutRule`, …), and the converse bound `le_G_dSize : d ≤ G (dSize d)` keeps the symbol-bounded search finite and so decidable in both polarities. Metering by the derivation's Gödel number instead would be a substitution — a code bound is a different predicate — and is not used here; what this row carries is a **convention** — the paper fixes neither encoding nor alphabet, so a counting convention is unavoidable, and ours charges each variable/function/relation index at its binary digit length plus one marker token (`idxLen n = Nat.size n + 1`), which is what makes the measure finite-fibred. It is recorded as `dd:symbolcount`, and the truth of every instance is independent of it (`conWithin_of_consistent`; the convention affects only which horizons discharge the non-degeneracy side conditions). This row reads *exact*. There is no `BoundedComputation` lane beside it — no abstract `lic_belief_stronger_theory_consistency`, no `_ofComputation` instance, no such structure — so this node has exactly one carrier.",
 'thm:dontwait': "The bounded-halting claim at horizon f(n) never fires (`hnever`), and the belief → 0. **What is represented is universal, and the sentence names the machine.** The decider is `universalRunValue steps`, which decodes a packed ⟨⟨source, input⟩, day⟩ argument and mentions no machine sequence, so `RepresentsComputations` supplies **one** γ per horizon program — the paper's ⌜f⌝ (tex:600-606) — and the day-n claim is ∀ν(γ(t,ν) ⟺ ν = 0̄) at t = `binNumeral (boundedArg machines inputs n)`, the compact name of ⟨⟨⌜qₙ⌝, yₙ⟩, n⟩. Both literals come from that one sentence (`represents_proves` / `represents_refutes_all`), carried to the compact spelling by `provable_subst_iff_of_val` — Gödel completeness both ways, adding no hypothesis on T beyond the `[𝗣𝗔⁻ ⪯ T]` already in the binder list. That the family separates its data is proved: `representedClaimSentence_ne_of_runValue_ne`. `hh : ComputableHorizon horizons` names ⌜f⌝ by a program with no growth bound, so the paper's arbitrary computable horizon is reached — that half is a proved strengthening (`not_polyNatCodes_ack`). The machine/input half matches as well: `hm : MachineMachineCodes machines` names each machine by a linear, decodable in steps linear in the source length (`ofSource_peelSteps`, `sourceNat_peelSteps_le`) source encoding of `Nat.Partrec.Code` — Mathlib's `Encodable.encode` there squares per node and is deliberately not used for naming — which is tex:1931-1933's ‘write out the source code in time polynomial in n’, while `hi : MachineDigits inputs` meters inputs by write-out, as tex:1946-1952's e.c. sequence of bitstrings requires. Both are load-bearing: `boundedArg_digits` is what emits the argument numeral's symbol run, discharging `def:ec`. On the theory side the endpoint asks for the paper's own premise and nothing semantic — `[T.Δ₁] [𝗣𝗔⁻ ⪯ T] [RepresentsComputations T]`, with **no** soundness instance — and the stage world is consistent from `Entailment.Consistent Θ` (tex:604). Nothing is charged on theory strength: no `[𝗜𝚺₁ ⪯ Θ]` residual reaches this row, and the endpoint's elaborated binders are `[T.Δ₁] [𝗣𝗔⁻ ⪯ T] [RepresentsComputations T]` — the last being the paper's own tex:600-606 premise, which supplies consistency itself. What is left is the two globally charged binders `[T.Δ₁]` and `[𝗣𝗔⁻ ⪯ T]`, disclosed once in the classification ledger's *Arithmetic-theory hypotheses* section and levied against no row. The row is `exact`.",
}
...
def build_logical_induction(paper, warnings):
    ...
    Three artifacts define this section: `Paper node:` docstrings (provenance), the curated
    per-node endpoint list (what is shown) and the strength table (what is claimed).  The
    last two live in one file, `scripts/coverage-classification.md`, read here through
