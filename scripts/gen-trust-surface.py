...
LI_READING = {
 ...
 'thm:ifp': "The published unrestricted theorem is FALSE, and `not_overgeneral_ifp` proves it false \u2014 kernel-checked and axiom-clean, at the paper's own quantifier: the constructed LIA, perturbed on day 0 only, publishes the diagonal sign bits as advice-atom prices, and an efficient trader reads them through historical price features without ever computing them. A single changed pricing day is an infinite computable function, which is enough to smuggle the gap between computable and efficiently computable. The remaining carriers are restricted: `lic_iff_of_finiteSupportPerturbation` (and its machine twin) quantify over perturbations moving only finitely many (day, sentence) coordinates \u2014 strictly stronger than the paper's tail agreement, and exactly the case where the appendix's constant table really is finite; `lic_iff_of_finitePerturbation` keeps the paper's own hypothesis shape. All three patch interfaces have no inhabitant anywhere in the repo, so no restricted carrier is yet shown to be about anything.",
