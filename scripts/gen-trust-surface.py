...
LI_READING = {
 ...
 'def:luv': "The paper's LUV is a first-order formula free in one variable; the abstract carrier here is its family of threshold atoms `\u231cX > r\u231d` (field `gt`), which is what downstream results consume. A literal frontend now produces that carrier: `PaperLUV` is an actual one-variable arithmetic formula with object-level unique/`[0,1]` proofs, and `PaperLUVSeq` derives both `PCWorld.ValuesAt` and `RpnThresholdCodeSeq` rather than assuming them, with a varying concrete inhabitant. Retained: the abstract carrier itself, and a fixed numerator/denominator pair-code value representation.",
