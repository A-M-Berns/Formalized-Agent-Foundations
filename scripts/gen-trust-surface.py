...
LI_READING = {
 ...
 'def:ec': "The paper's “computable in O(poly(n))” is `MachineEfficientTrader`: some `Complexity.FP` function of the unary day emits the day-n strategy's serialized symbol stream. It is shown first because it is the paper's own class and the one the trading firm dominates. The fuel-clocked `EfficientlyComputable` is the certification device that feeds it — `EfficientlyComputable.toMachine` compiles an `evaln` certificate into a real polynomial-time machine — and is not a substitution for the class. What still qualifies this node is the fuel rendering itself, on the *sequence* classes downstream statements take as their own data: the write-out ladder (`BigDigits`, `BigSentenceCodes`, `DigitRatCodes`, `BigSpliceStream`/`BigTokenStream`) meters the number of tokens a poly-time writer must emit and leaves values unrestricted, and formula families are metered on the paper's own source language (`PolyArithmeticSourceSeq`, one token per node of the formula as the paper writes it) rather than on a normal form — so neither a value bound nor a connective charge survives here. What survives is that those certificates are `evaln`-fuel certificates rather than machines. Two things about this charge are easy to get wrong. First, the class census: counted over the 107 canonical endpoints, through the structures they bind and not only their printed binders, the charge is carried mostly by the write-out classes — `BigSentenceCodes` reaches 51 endpoints and `BigSpliceStream` 52, 64 of the 107 in all — while on the *sentence* lane the token-metered `RpnSentenceCodes` binds **none** under its own name. It would bind one — `lic_self_trust_closed`, which has to discharge a `LUV.RpnThresholdCodeSeq` obligation — were there no write-out threshold class to reach for; `LUV.BigThresholdCodeSeq` is that class, `SelfTrustQuote`'s product and confidence fields sit at it, and that endpoint takes `hφ : BigSentenceCodes φ`. Second, `MachineSentenceCodes` — the machine reading, with the inclusion `RpnSentenceCodes.toMachine` — has **no consumer anywhere in the development**; the earlier claim that it was consumed at `thm:scon` is false, since that node's transports reach the separately defined `CondStep.MachineSentenceBlocks`, a different predicate that happens to read similarly, and reach it from `BigSentenceCodes`. And the converse inclusion does not merely stand open — it splits. At the length-metered target it is open on a **verified obstruction**: compiling a machine write-out certificate back to an `evaln`-fuel one needs a TM → `Nat.Partrec.Code` compiler carrying fuel accounting, because `evaln`'s fuel guard decrements per constructor while the machine's cost is counted in emitted symbols, and the `comp` case cannot be reconciled without one. At the value-metered target it is **false**, not open: `not_polyFueled_two_pow` refutes it at `2^n`, which is `Complexity.FP` once its output is written in binary. Both repair directions need that same missing compiler, so closing the gap would mean redefining `dd:fuel` — an index-oracle certification device rather than a complexity class — not proving one more lemma. The token-metered charge is **not** discharged, though, and a signature grep understates what survives. What survives is the LUV *threshold* lane: `LUV.RpnThresholdCodes` and `LUV.RpnThresholdCodeSeq` are defined as `RpnSentenceCodes` on the threshold family `⌜Xₙ > i/k⌝`, so a census grepping signatures for `RpnSentenceCodes` does not see them. Named: `LUVCombinationSyntax.threshold_poly`, a Tier-2 field-frozen structure field bound through the structure by the four canonical `_ofSyntax` endpoints — so this one **is** on the endpoint census; `ConvergencePresentation.threshold_code`; the direct threshold binders on `thm:expcoh`, `thm:perexpkno`, the `lic_expectation_provind*` family and the affine tail; the quote certificates of `thm:cee`, `thm:ceu`, `thm:ccee`, `thm:epr` and `thm:er`; and `LUV.RpnThresholdCodeSeq` on the quotation family's `_ofRepresentation` layer. The conditioning certificate at `thm:scon` is not among them: it sits at `BigSentenceCodes`, `def:ec`'s own class. So the honest statement is the narrow one: the retentions are the LUV-threshold lane's, one of them on the canonical census. Why that is a rendering sensitivity rather than a narrowing of the paper's admissible LUVs — along the threshold route the metered object is a formula string over a fixed finite alphabet (`0..19`), so the per-token *value* clause is vacuous and the class is exactly polynomial length — is set out under *LUV-threshold metering* in the classification ledger, which is also why no row is lowered for it.",
 ...
 'thm:scon': "Fixed form adjoins one ψ, growing form a whole computable process; the conclusion is the criterion for the *conditioned* history over the union process — the paper's ℙ‾|ψ. No joint-consistency premise: the degenerate branch covers unsatisfiable stages. All four shown endpoints conclude `IsMachineLogicalInductor`, the paper's own quantifier; the closed pair takes exactly the hypotheses of the fuel-class `lic_conditioned_{fixed,growing}_unconditional` and concludes the strictly stronger criterion, so it supersedes them. **Class note — no disclosure here any more.** The condition's certificate is `BigSentenceCodes`, `def:ec`'s own write-out class: both `condition_codes` fields and both the machine and the fuel transports take it (the `Rpn` in the fuel transports' names is the RPN *symbol model*, not the sentence class). It is carried by `CondStep.machineSentenceBlocks_of_big` running on `BigTokenStream.digitizeStream`, which digitizes a write-out token stream into a `PolySegStream` off its own digit stream. A token-metered retention here would be forced only by a line-level obstruction, and that diagnosis fails on both halves — the digit clamp is the identity because the clamped object is a list of base-4 digits, not because any token value is bounded, and the write-out certificate supplies its own clock — and the widening cost one short lemma rather than an FP re-blocking. `RpnSentenceCodes` remains a sufficient subclass (`BigSentenceCodes.ofRpnSentenceCodes`); no sentence-level strictness between the two is proved here.",
 'thm:ifp': "**The published unrestricted theorem is FALSE, and the first endpoint below proves it false.** `not_overgeneral_ifp` is closed but for the deductive process and axiom-clean, at the paper's own quantifier: the constructed LIA, perturbed on day 0 only, publishes diagonal sign bits as advice-atom prices, and an efficient trader reads them through historical price features without ever computing them. A single changed pricing day is an infinite computable function, which is enough to carry unbounded advice; the published proof's invalid step is its “only finitely many constants” claim. The **corrected** theorem is `FreezeOracle.machine_lic_iff_of_finiteSupport`: two computable markets differing on only finitely many (day, sentence) coordinates satisfy the criterion together — strictly stronger than the paper's tail agreement in the direction that survives, and exactly the case where the appendix's constant table really is finite. It takes no patch argument and no condition of any kind on the finitely many moved sentences: both halves of the former `Recognizable` side condition are discharged by building the `Complexity.FP` device each stood for (`FiberTest.fiberW_mem_FP` for the escape-leaf decode test, `CtrAuto.ctrMachine` and `PayAuto` for the structured payload recognizer). What is disclosed in their place is a property of the construction, not of the statement: the recognizer is compiled per frozen sentence, so its polynomial-time constants depend on that sentence — the paper's own hard-coded finite table, sound exactly because the support is finite. `machine_lic_iff_twoPoint` makes it non-vacuous and `machineLogicalInductor_liaPerturbed` — the third endpoint — makes it informative: applied to LIA with one price moved, it derives a machine logical inductor no construction here produces. The fuel-class carriers `lic_iff_of_finitePerturbation` / `lic_iff_of_finiteSupportPerturbation` are compatibility only and their patch hypotheses are *uninhabited*; they and `machine_lic_iff_of_finiteSupportPerturbation`, which the corrected theorem supersedes, are deliberately not shown.",
 ...
 'thm:simcal': "The calibration indicator (price in [a,b]) is itself the weighting; conclusion pins limit points of the weighted truth-average to [a,b]. The endpoint takes the indicator's generability and the affine sequence's e.c. certificate as *arguments*, and tex:1188 merely asserts the first — but both are proved here from the paper's own hypotheses, by `calibrationIndicator_pgenerable` and `AffineCombination.sentenceAffine_polySequence`, shown beside it. The ⟨φ⟩ half reaches the paper's class exactly (`AffineCombination.PolySequence`, whose sentence and coefficient fields are the write-out classes `BigSentenceCodes`/`BigSpliceStream`); the ⟨δ⟩ half carries no width class at all any more — the only premise on it is `∀ n, 0 < δ n` — so no narrowing is imposed there.",
