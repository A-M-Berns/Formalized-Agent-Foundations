...
LI_READING = {
 ...
 'thm:perkno': "`limitingBelief P (φ n)` is ℙ∞(φₙ). The conclusion is a three-way conjunction matching the paper's three displayed clauses (≈, ≲ and ≳ against the future sup/inf) clause for clause; the `_lower`/`_upper` variants split them. `φ` carries the symbol-metered `RpnSentenceCodes`. `p` carries `PolyRatCodes`, which is **not** the paper's e.c. class: it bounds the Gödel *value* of each pₙ, so numerator and denominator must be of polynomial magnitude and the probabilities may approach their limits only at a polynomial rate.",
 ...
 'thm:simcal': "The calibration indicator (price in [a,b]) is itself the weighting; conclusion pins limit points of the weighted truth-average to [a,b]. The endpoint takes the indicator's generability and the affine sequence's e.c. certificate as *arguments*, and tex:1188 merely asserts the first — but both are proved here from the paper's own hypotheses, by `calibrationIndicator_pgenerable` and `AffineCombination.sentenceAffine_polySequence`, shown beside it. The ⟨φ⟩ half reaches the paper's class exactly (symbol-metered `RpnSentenceCodes`); the ⟨δ⟩ half does not, since `PolyPositiveWidths` is built from the whole-value `PolyRatCodes` and so admits only polynomially shrinking widths.",
