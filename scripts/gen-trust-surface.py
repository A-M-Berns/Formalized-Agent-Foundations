...
LI_READING = {
 ...
 'thm:incons': "The endpoint's subject matter is a *theory sequence*, not an arbitrary semidecidable predicate: `lic_disbelief_inconsistent_theories_unconditional` takes a base theory `Θ₀` and a sentence family `σ`, and its premise `hinc : ∀ n, ¬Entailment.Consistent (σ n ∷ Θ₀)` is the paper's own — stated at the day's *theory* (Foundation's `adjoin`), not at a provability surrogate. Both conjuncts of the display are concluded. **The sentence names the day's theory.** What is represented is the universal provability schema of the base theory, `inconsistencySchema Θ₀ = codeOfREPred (ProvableCode Θ₀)` — one schema per base theory, argument-sensitive, with `inconsistencySchema_mentions_zero` *proved* from consistency rather than assumed — and the day's data enters as the compact numeral `binNumeral ⌜∼σₙ⌝`. Representing '`Θ₀` is inconsistent' directly would have been the trap, since under `hinc` that predicate is constantly True. Separation is a theorem: `inconsistencyArgClaimSentence_ne_of_arg_ne` distinguishes the sentences of any two distinct adjoined-axiom codes, with no behavioural hypothesis. **Non-vacuity is the paper's own subject matter**, discharged in the file: market 𝗜Σ₁, base theory 𝗜Σ₁, σₙ := ⊥, so every day's theory 𝗜Σ₁ ∪ {⊥} is actually inconsistent — replacing the superseded '0 < n' stand-in, whose predicate was not a statement about a theory at all. Beside it sits a genuinely day-varying witness, `alternatingInconsistentAxiom n := if n % 2 = 0 then ⊥ else ⊥ ⋏ ⊥` — refutable on every day, with distinct Gödel codes on adjacent days — carried with its emission certificate `alternatingInconsistentAxiom_digits`, the fully applied endpoint `thm_incons_applied_alternating`, and an example exercising day separation at days 0 and 1, which the constant witness cannot. It takes two values, not unboundedly many: an unbounded-length family is blocked on a missing `BigDigits` combinator (poly-fueled digit access through an unboundedly iterated `Nat.pair` shell), signposted at the witness. The two sentence families are no longer independent: `consistencySentence` is now definitionally `∼inconsistencySentence`, which is the paper's own shape (tex:1863-1866); `Sentence` is Foundation's propositional `Formula`, so the syntactic-negation worry that once motivated two families does not arise. `dd:proofcode` is **not** charged here: this node's sentence is the unbounded existential over proofs, so nothing is metered and the symbol-count-versus-Gödel-number substitution does not arise. Theory hypotheses are `[T.Δ₁] [𝗣𝗔⁻ ⪯ T] [Entailment.Consistent T]` and `[Θ₀.Δ₁]` — no soundness (that went when `theoremDP_hworld`'s tag-7 exclusivity became a theorem of Θ), no `𝗜Σ₁`, and no `𝗥₀`, the redundant binder having been dropped in tranche 8. **Two charges hold the row at `qualified`.** (i) The theory sequence is the *deduction family* Θ′ₙ = Θ₀ ∪ {σₙ} rather than an arbitrary e.c. sequence of recursively axiomatizable theories. Foundation's `Derivation T` takes `T` as a **meta** parameter, so there is no uniform-in-theory-code derivability predicate to represent; for a deduction family the deduction theorem collapses the day's theory to the single code ⌜∼σₙ⌝, which is uniform. Retired by the uniform-in-theory-code work in tranche 9b. (ii) The `def:ec` premise `hσ : BigDigits (deductionFamilyArg σ)` meters the day's theory name by the base-4 digit count of a formula's **Gödel code**, not by its source length: Foundation's encoding pairs at every node, so that count is ~2^depth and the premise admits only O(log n)-depth families, excluding paper-admissible short-source/deep-parse ones. This is neither the `dd:fuel` model choice nor the `dd:proofcode` measure choice but the ordinary write-out class applied to the wrong quantity — the failure that disqualified `Encodable.encode` as a machine-naming map and forced `Code.sourceNat`. Being an over-strong *hypothesis* it narrows which theory sequences the endpoint covers and does not weaken the conclusion where it applies; the faithful repair is to state the premise on `PolyArithmeticSourceSeq`, queued with the 9-series.",
