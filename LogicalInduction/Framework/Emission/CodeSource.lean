...
* `tokenListNat` — the name of a token run, one base-64 digit per token closed by the
  ...
  `tokenListNat_primrec` are what `Construction/Knowledge/SourceNumbering.lean` and
...
* `nest`, `bigDigits_sourceNat_nest` and `two_pow_le_sourceNat_nest` — the family witnessing
  ...
  (`Framework/Emission/WriteOut.lean`); the strictness statement it feeds is
...
`digitMachineCodes_nest_not_polyMachineCodes`
(`Construction/Knowledge/Syntax.lean`) separates the two machine-naming classes
with. -/
