...
(`PolySegStream.dayClampTokens`), which `Construction/Conditioning/Compiler.lean` and
`Construction/Conditioning/{PricePass,FramePass}.lean` consume for `thm:scon`'s
...
as `BigSentenceCodes` (`Framework/Emission/WriteOut.lean`).
...
elaboration safeguard `Framework/Emission/Emission.lean` explains.
...
@[simp] lemma digitVal_cons (d : ℕ) (ds : List ℕ) :
...
General facts about `digitVal` and `natDigits4`, used by the token tests that compare a
block's value against a fixed numeral.  A run's value does not determine the run — `[1]` and
`[1, 0]` are both the token `1`, which is why a client reads blocks rather than words.  It
determines it *up to trailing zeros* (`digitVal_eq_iff_zero_padded`), and that is what makes
a fixed-numeral test decidable by two constant-word checks: the run must begin with the
numeral's canonical digits and continue with nothing but zeros. -/
...
The conditioning transducer's parser control (`EF.freezeTokenNext`,
`Framework/Emission/FreezeTransducer.lean`) tracks a mode `≤ 5` plus the pending sentence
code.  In the digit model the pending
...
`Construction/Conditioning/Compiler.lean` consume the digit-metered form directly. -/
