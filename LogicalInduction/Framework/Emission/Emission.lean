...
* The **bounded dovetail** (`acceptsWithin`, `dovetailStep`, `dovetailFound`,
  `dovetailFound_eq_true_iff`, `polyFueled_dovetailFound`) — the generic form of the paper's
  `DefinitelySettled` clock (`app:prandaff`), consumed by the
  `PatientSettlementClock.active_codes` field (declared in
...
* `PrefixPatchCompile.clockedTokens_polySegStream` — the raw token stream a clocked
  ...
  `Construction/Freeze/Prefix.lean`) build their trader certificates on.
