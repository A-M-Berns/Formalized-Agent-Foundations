...
* **`BigTokenStream t`** — some `PolySegStream` of base-4 digits undigitizes to `t n`, block
  ...
  polynomially long digit run, and `BigDigits` (`Framework/Emission/DigitArith.lean`) is the
...
(`Framework/Emission/Computable.lean`) inherits the restriction by asking for a poly-fueled
...
lemma ofBigDigits {x : ℕ → ℕ} (h : BigDigits x) : BigTokenStream (fun n => [x n]) :=
  ⟨fun n => tokenBlock (x n), h.blockSeg,
    fun n => (TokenFold.undigitize_run_terminator (natDigits4 (x n))
      (natDigits4_lt (x n))).2,
    fun n => (TokenFold.undigitize_run_terminator (natDigits4 (x n))
      (natDigits4_lt (x n))).1.trans (by rw [digitVal_natDigits4])⟩
...
machine is *named*.  The name is `Code.sourceNat` (`Framework/Emission/CodeSource.lean`): the
postfix tag stream of the syntax tree read base-16, whose symbol count is linear in the tree
...
`Framework/Emission/CodeSource.lean`, and the strictness statement they feed is
`digitMachineCodes_nest_not_polyMachineCodes`
(`Construction/Knowledge/Syntax.lean`).
Paper node: `def:ec` -/
...
the family, in `Framework/Emission/Computable.lean`. -/
...
lemma bigSpliceStream_two_pow_inv_not_rpnSpliceStream :
  ...
  have hu : unRpn (s z) = [1, Encodable.encode ((((2 ^ z : ℕ) : ℚ))⁻¹)] := by
    simpa [EF.serialize] using (hc z).unRpn_eq
