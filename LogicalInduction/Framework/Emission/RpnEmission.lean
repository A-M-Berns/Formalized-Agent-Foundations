...
* **The escape-slot scan.** `PolySegStream.escModeScan` runs the escape-slot automaton over
  ...
  `PolySegStream.freezeModeScan` (`Framework/Emission/DigitArith.lean`).
...
first bound and drops the second is `BigTokenStream` (`Framework/Emission/WriteOut.lean`).

**Design.** `Nat.sqrt` is locally irreducible file-wide; see the note in
`Framework/Emission/Emission.lean`.
