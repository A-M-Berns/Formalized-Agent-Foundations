...
encoded day-`n` strategy.  With the `Nat.pair`-tagged `EF` encoding (`Framework/Criterion.lean`) the
...
* **The digit layer** — `PolySegStream.block`, `PolySegStream.digitizeStream`,
  ...
  `EfficientlyComputableTok.toDigit` lives in `Framework/Emission/Emission.lean`.
...
real size-`Θ(n)` traders; `Framework/Emission/RpnSplice.lean` and
`Framework/Emission/WriteOut.lean` build their sentence and splice classes on `PolySegStream`.
-/
...
(`Framework/Emission/Emission.lean`), whose explicit `codeEvalBound` carries the simulator's
...
/-- **Course-of-values primitive recursion, packaged.**  A function whose value at `n` is
computed by one primitive-recursive `step` from the table of its earlier values is itself
primitive recursive.  This is Mathlib's `Primrec.nat_strong_rec` with its dummy parameter
discharged; every syntactic decoder in `Construction/LIACompiler.lean` is of this shape. -/
lemma _root_.Primrec.of_courseOfValues {σ : Type*} [Primcodable σ]
    (f : ℕ → σ) {step : List σ → σ} (hstep : Primrec step)
    (hrec : ∀ n, step ((List.range n).map f) = f n) : Primrec f := by
  have h : Primrec₂ fun (_ : Unit) (n : ℕ) => f n :=
    Primrec.nat_strong_rec (fun (_ : Unit) n => f n)
      (g := fun (_ : Unit) (L : List σ) => some (step L))
      (Primrec₂.option_some_iff.mpr (hstep.comp Primrec₂.right))
      (fun _ n => congrArg some (hrec n))
  exact h.comp (Primrec.const ()) Primrec.id

/-- Monotonicity of the fuel-clocked interpreter in `isSome` form: once a semi-decider has
halted at some fuel it has halted at every larger fuel.  Every dovetailing deductive process
in `Construction/` uses this for the monotonicity of its stages. -/
...
/-! ### `sel` as a poly-fueled function, and the `IsPolyBounded` closure algebra. -/
...
/-- Polynomially bounded functions are closed under pointwise product.  Unrolling a
fuel-decrementing recursion multiplies a step count by the fuel, which is where the
machine layer needs it. -/
...
/-- Scaling a polynomially bounded function by a constant keeps it polynomially bounded. -/
...
/-! ### Rational quotients of naturals

Threshold and mesh emitters everywhere in `Construction/` name a rational as a quotient of
two runtime naturals, so they all need the same two facts: the closed form of
`Encodable.encode` on `(i : ℚ) / (k : ℚ)`, and one poly-fueled program computing it from
poly-fueled numerator and denominator.  Both live here, once, because their consumers sit in
four different lanes.  The reduction is by runtime `gcd`; the `k = 0` branch emits `⌜(0 : ℚ)⌝`,
matching `x / 0 = 0` in `ℚ`. -/
...
lemma encode_rat_natCast_div {i k : ℕ} (hk : k ≠ 0) :
  ...
  have hnum : ((i : ℚ) / (k : ℚ)).num = ((i / Nat.gcd i k : ℕ) : ℤ) := natCast_div_num hk
  have hden : ((i : ℚ) / (k : ℚ)).den = k / Nat.gcd i k := natCast_div_den hk
...
/-! #### The `List.range` / `Finset.range` sum bridge -/

/-- A `Finset.range` sum written as the sum of the corresponding mapped `List.range`.
Used wherever a bundle is built as a list but reasoned about as a `Finset` sum — payouts
(`Properties/UniversalSemimeasure.lean`), partial magnitudes and net worths
(`Framework/ROI.lean`), and dovetailer approximations
(`Construction/NonDogmatism/UniversalDovetailer.lean`). -/
...
lemma length_flatMap_eq_segPrefix {α : Type*} (seg : ℕ → List α) (lenFn : ℕ → ℕ)
...
lemma getD_flatMap_of_prefix {α : Type*} (seg : ℕ → List α) (lenFn : ℕ → ℕ) (d : α)
...
lemma PolySegStream.concatVar {seg : ℕ → List ℕ} (hseg : PolySegStream seg)
  ...
  · intro n i hi
    ...
    have hhi : i < segPrefix lenFn n (j + 1) := by
      ...
      omega
    rw [getD_flatMap_of_prefix seg lenFn 0 n (cnt n) i j (fun k => hlens _) hjlt hlo hhi]
...
`EfficientlyComputableTok.toDigit` in `Framework/Emission/Emission.lean`. -/
