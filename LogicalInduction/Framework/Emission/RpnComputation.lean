...
`Framework/Emission/RpnSentence.lean`), while list codes grow strictly along `cons`
...
`Construction/Knowledge/SourceRecognizer.lean` consumes them for its own strong recursion.
