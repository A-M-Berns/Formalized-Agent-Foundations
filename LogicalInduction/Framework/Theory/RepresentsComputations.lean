...
`representsComputations_of_peanoMinus` (`Framework/Theory/R0Instances.lean`)
...
Instances live in `Framework/Theory/R0Instances.lean`. Consumers use only
...
it.  It is realized: the committed instances (`Framework/Theory/R0Instances.lean`,
...
term `binNumeral` (`Construction/LUV/SourceCodec.lean`) is, at `O(log v)`
nodes.
