...
* `PricePass`, `FramePass` — the `dd:fuel` rendering in the RPN *symbol* model, where a
  ...
  legs and the two-leg join.  Namespace `RpnConditioning`.  It publishes no
  class-preservation capstone — the operational witnesses carry only the `def:ec`
  certificate — so what this pair supplies is the shared automaton and per-pass scalars the
  machine rendering below is a client of.
* `Transduction`, `TransductionFrame` — the same transducer in the machine model, as a client
  ...
  `conditionedTranslation_preserves_ec` /
  `eventualConditionedTranslation_preserves_ec`.  Namespace `CondStep`.

## The endpoints

* `Endpoints` — the criterion-level `lic_conditioned*` family at `def:ec`'s own quantifier,
  built from operational witnesses that discharge the machine translation certificate, and
  the same theorem made unconditional over the constructed `LIA`, with
  `exists_growing_conditioned_inductor` as its non-vacuity witness at a strictly
