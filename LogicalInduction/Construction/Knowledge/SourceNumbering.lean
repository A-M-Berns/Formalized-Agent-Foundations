...
(`tokenListNat`, `Framework/Emission/CodeSource.lean`) — the number `def:ec` charges a trader for
...
`Framework/Theory/BoundedConsistency.lean`) applies at the compiled formula unchanged.
...
(`Construction/Knowledge/SourceWindow.lean`), and the code of the formula that writing
...
`Construction/Knowledge/SourceWindow.lean`) admits only runs that
