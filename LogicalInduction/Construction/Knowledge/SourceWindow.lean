...
convention reaches both live in `Construction/Knowledge/Endpoints.lean`.

Everything here is primitive recursive save the last step, which inherits
`negSourceFormulaCode`'s `Computable`.  As in `Construction/Knowledge/SourceNumbering.lean`,
none of it is efficient and none of it needs to be: it runs *inside* a represented predicate, where
the paper asks only for recursive enumerability. -/
...
(`Construction/Knowledge/SourceRecognizer.lean`) derived in-project. -/
