...
(`Framework/Theory/RepresentsComputations.lean`), exercised in
`Construction/Knowledge/Endpoints.lean`; the bounded-halting *claim families* of `thm:pac`,
`thm:pazfc` and `thm:dontwait` therefore live there, over `paperTheoryDP`, rather than here.
-/
...
(`Code.ofSource`, `Framework/Emission/CodeSource.lean`), not from `Encodable.encode` — and run it on
...
(`Framework/Emission/RpnSentence.lean`), which also run `0`–`8`.  Note in particular that the
...
structure ComputationTheoryPresentation
  ...
  and is discharged by `ComputationDP.theoremPresentation` and by
  `Construction/Quotation/ProductDefinition.lean`.  The `thm:loops` endpoint over `paperDP` takes
  its negative literal from its own `hloops` premise rather than from this field. -/
  halting_refutes : ∀ z : ℕ,
    T ⊢ ∼(universalHaltingSchema/[↑z]) →
      ∃ k, (∼haltingClaimSentence z) ∈ DP.D k
...
separation of claim sentences in `Construction/Knowledge/Endpoints.lean` would need: the
missing piece
...
`Construction/Knowledge/Endpoints.lean` is a theorem rather than queued infrastructure.
