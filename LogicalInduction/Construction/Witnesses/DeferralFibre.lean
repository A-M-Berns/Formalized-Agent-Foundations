...
/-!
# Deferred and paired-index affine machinery

The quotation-free half of the reflection apparatus: the affine portfolios and emission
certificates that `QuotationAffine.lean` trades on.  Nothing here mentions a quotation
atom, schema or quote code, and the dependency runs one way — `QuotationAffine.lean`
imports this module.

`LUV.expectAffineSeq` is the day-indexed expectation mesh (`def:e`): day `n` carries its
own grid `n + 1`, and `expectAffineSeq_polySequence` emits it literally rather than
decoding a serialized affine object.

`ctsIndFeature` compiles the paper's `ctsind` into `EF`, and `PGenerableWeighting.mul` and
`.add` close the day-indexed emission certificate under the operations the gates need.

The bounded deferral preimage — `deferralMatchCount`, `deferralPreimage`,
`deferralImageFlag` and `deferralPreimage_spec` — recovers the source day of a deferral day
by a bounded scan, for an injective schedule.  Its consumers are the
`thm:wub`/`thm:wubaff`/`thm:wubexp` feedback chain in `FeedbackTruth.lean`, where the paper
itself asks for a strictly increasing deferral function.

`AffineCombination.PolySequence.add`, `.scaleFeature` and `blockSum` give variable-width
day-`m` affine sums whose block and offset are read off division and remainder.

`selectorFeature` is the division-free first-violator selector — `firstSuccess_sum_le_one`
is its magnitude budget and `firstSuccess_forces` its forcing step — and `PairedWeighting`
is the paired-index emission certificate (`def:ece`) a fibre gate needs in order to be a
legal day-`m` coefficient.

The `DeferralFibre` namespace assembles these into deferred coherence:
`deferred_block_price_tendsto_zero` sends `(Bs ⟨f n, n⟩).price P (f n)` to zero for every
`def:deferralfunc`, with no injectivity or monotonicity assumption on the schedule.  The
paired-index families `crossPrecisionBlocks`, `pairedExpectationBlocks` and
...
/-! ## Day-indexed expectation meshes -/
...
/-- On its own day the mesh is priced at the market expectation of the LUV. -/
...
first-violator machinery of `## Deferral-fibre gating` below, which states the obstruction
a plain price-gated sum over `f⁻¹(m)` runs into and the selector that avoids it. -/
...
This is the shape every fixed-width term-stream lookup in the affine syntax reduces to. -/
...
/-- Each first-violator weight is non-negative, the gates being `[0,1]`-valued. -/
...
/-- A product over `List.range n` *is* the `Finset.range n` product: `Finset.range n` is
`Multiset.range n`, which is the coercion of `List.range n`, so the two sides are
definitionally equal. -/
lemma list_prod_range {M : Type*} [CommMonoid M] (n : ℕ) (F : ℕ → M) :
    ((List.range n).map F).prod = ∏ j ∈ Finset.range n, F j := rfl
...
lemma list_sum_range {M : Type*} [AddCommMonoid M] (n : ℕ) (F : ℕ → M) :
    ((List.range n).map F).sum = ∑ j ∈ Finset.range n, F j := rfl
...
towers' eventual bounds.  `firstSuccess_sum_le_one` is the magnitude budget,
`firstSuccess_forces` the forcing step, `AffineCombination.blockSum` the variable-width
day-`m` combination the gates weight, and `deferred_block_price_tendsto_zero` the
conclusion: `(Bs ⟨f n, n⟩).price P (f n) → 0` for every `def:deferralfunc`, with no
injectivity or monotonicity assumption on the schedule. -/
...
/-- Clipping to `[0,1]` preserves the paired-index certificate: `clip01` is built from
`max` and multiplication by constants, which the closure lemmas above already cover. -/
lemma clip01 {A : ℕ → EF} (hA : PairedWeighting A) :
    PairedWeighting (fun z ↦ _root_.LogicalInduction.clip01 (A z)) :=
  (PairedWeighting.const 0).max
...
/-! ## Cross-precision and expectation-difference portfolios -/
...
/-- The uniform polynomial emitter for `featureConstantAffine`: no terms at all, the
feature's own segment stream as the constant.  It consumes a `PGenerableWeighting`
certificate for `H`. -/
...
/-! ## Assembling the fibre portfolio

`DeferralFibre` assembles the pieces above into a single day-`m` portfolio over the fibre
`f⁻¹(m)`: block price features and their positive and negative gaps, the gate coefficients
built from them, and the two coherence conclusions — `fibre_price_eventually_small` at one
gate width, and `deferred_block_price_tendsto_zero` over all of them. -/
...
lemma priceFeat_paired {Bs : ℕ → AffineCombination}
    (hB : AffineCombination.PolySequence Bs)
...
lemma fibre_price_eventually_small
  ...
  have hnorm2 : ((1 / (2 * C) : ℚ) : ℝ) * 2 * (C : ℝ) = 1 := by
    ...
    rw [this]; field_simp
  -- The day's whole budget: the gate weights sum to at most `1/C`, so any fibre quantity
  -- bounded by `C` is weighted into `[0,1]`.  Both the price bound and the magnitude
  -- bound are this fact at different `g`.
  have hbudget : ∀ (m : ℕ) (g : ℕ → ℝ), (∀ k, g k ≤ (C : ℝ)) →
      ∑ k ∈ Finset.range m, |(coeff (Nat.pair m k)).denote P| * g k ≤ 1 := by
    intro m g hgC
    calc ∑ k ∈ Finset.range m, |(coeff (Nat.pair m k)).denote P| * g k
        ≤ ∑ k ∈ Finset.range m, |(coeff (Nat.pair m k)).denote P| * (C : ℝ) :=
          Finset.sum_le_sum fun k _ ↦ mul_le_mul_of_nonneg_left (hgC k) (abs_nonneg _)
    ...
    { family := family
      ...
      bounded := ⟨1, zero_le_one, fun m day ↦ by
        ...
        calc |∑ k ∈ Finset.range m, (coeff (Nat.pair m k)).denote P *
          ...
          _ ≤ 1 := hbudget m (fun k ↦ |(Bs (Nat.pair m k)).price P day|)
              (fun k ↦ hbdd _ day)⟩
      magnitude_le_one := fun m ↦ by
        rw [hfamMag m]
        exact hbudget m (fun k ↦ (Bs (Nat.pair m k)).magnitude P) (fun k ↦ hmag _)
      theory_coherent := by
        intro ε hε
...
lemma crossPrecision_deferred_tendsto_zero
  ...
  have hkey := deferred_block_price_tendsto_zero (P := P) (DP := DP) hworld f hspec hB
    ...
    (hwide := fun m k hk ↦ by
      rw [crossPrecisionBlocks_terms_length]; omega)
    (C := 2) (hC := by norm_num)
    (hmag := fun z ↦ by
