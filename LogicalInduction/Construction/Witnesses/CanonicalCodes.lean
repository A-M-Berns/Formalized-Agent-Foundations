...
(`BoundedEvaluation.lean`) reads `Nat.unpair`, i.e. integer square root — the one primitive
...
* `sentenceMatches_of_botFree` — its operational form: on a `⊥`-free target the
  ...
  `RpnFreeze.lean` (the spelling-list lane); the injectivity lemma itself is consumed there
