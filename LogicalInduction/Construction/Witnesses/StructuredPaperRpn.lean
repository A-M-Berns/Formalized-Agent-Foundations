...
values — consumed by `ArithmeticSource.lean` and `SubstEmission.lean`.  `binNumeral_val`
...
(`ArithmeticSource.lean`), into which this one embeds by
...
`structuredPaperSourcePrimeBlock` (`ArithmeticSource.lean`) is the same block at the
paper's own source run and inherits contraction, emission and framing from here. -/
...
`PolyArithmeticSourceSeq` (`ArithmeticSource.lean`), which meters the source as the paper
writes it; this one is strictly finer, on `⟺` alone, because Foundation's normal form
...
(`dd:nnf`).  The paper's class is `PolyArithmeticSourceSeq` (`ArithmeticSource.lean`), into
which this one embeds by `PolyArithmeticFormulaSeq.toSource`.
...
coercions the exact product uses (`PaperExactProduct.lean`). -/
