...
**The deferred layer** is quotation-free and lives in `DeferralFibre.lean`, which this
module imports: the variable-width affine combinations, the paired-index emission
certificate `PairedWeighting` (`def:ece`), the division-free first-violator
`selectorFeature`, and `DeferralFibre.deferred_block_price_tendsto_zero`, which delivers
deferred coherence for every `def:deferralfunc` with no injectivity or monotonicity
...
`PaperMarket.lean` over the single market `liaHistory (paperDP T)`.
