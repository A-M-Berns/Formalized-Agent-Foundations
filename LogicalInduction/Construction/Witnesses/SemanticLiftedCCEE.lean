...
(`Construction/Witnesses/PaperExactCCEE.lean`), priced on the single market
...
Consumers: `AxiomAudit.lean` inventories
`lic_no_expected_net_update_conditional_exact_canonical`, `canonicalCCEEDP_computable` and
`canonicalCCEEDP_hworld`; the `thm:ccee` row of `scripts/coverage-classification.md`
records the two-rendering split.
-/
