...
`computable_perturbedQuote`; (b) `Nat.Partrec.Code.exists_code`.
Paper node: `app:ifp` -/
theorem computableMarket_liaPerturbed (DP : DeductiveProcess)
  ...
  have hrange' : ∀ n φ, 0 ≤ liaPerturbed DP r n φ ∧ liaPerturbed DP r n φ ≤ 1 := by
    ...
    · constructor
      ...
      · exact_mod_cast h1
    · exact hrange n φ
  refine ⟨hrange', perturbedQuote mq r, ?_⟩
  have hcomp := computable_perturbedQuote r (computable_of_marketCode hcode)
  have hpart : Nat.Partrec (fun z : ℕ =>
      Part.some (Encodable.encode (perturbedQuote mq r z.unpair.1 z.unpair.2))) :=
    Partrec.nat_iff.mp hcomp.partrec
  obtain ⟨codeNew, hcodeNew⟩ := Nat.Partrec.Code.exists_code.mp hpart
  refine ⟨codeNew, ?_, fun z => ?_⟩
  · intro n φ
