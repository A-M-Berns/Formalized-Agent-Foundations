...
`lic_no_expected_net_update_conditional_closed` (`Construction/Paper/Market.lean`) states
`thm:ccee` for an **arbitrary** threshold-only source family, and pays for that generality with the
...
pair codes exactly (`Construction/Quotation/ExactProduct.lean`).  This module feeds that
exact product into the same generic trading argument at `slack = 0`.  The trade is exactness for
generality, and it is the only difference: same `paperDP T`, same market, same deductive process as
...
(`Construction/Quotation/RepresentedWeight.lean`).  Its hypotheses are the paper's own, and
are justified at the declaration.

The two clients at the foot of the file are witnesses in the in-file style of
`Construction/LUV/ArithmeticSource.lean`: one varies the source over `𝗣𝗔` with deferral and
weight left hypothetical, the other discharges every binder.  They sit here rather than in
`APITests/LogicalInduction.lean` because they are non-vacuity witnesses for this endpoint's
binders, not client exercises; `PaperLUVSeq` itself is documented consumer interface
(`LogicalInduction/API.lean`, *The literal first-order layer*).
-/
...
Written in the in-file client style of `Construction/LUV/ArithmeticSource.lean`'s witnesses:
they are non-vacuity witnesses for this endpoint's binders rather than client exercises, which
is why they are here and not in `APITests/LogicalInduction.lean`. -/
