...
numbers.  `RationalQuoteCode` and `BooleanQuoteCode` (`Construction/Quotation/Packages.lean`)
are the objects carrying such a quote, code-indexed after `dd:quote-code`.  This module constructs
...
* The `thm:ccee` deferred-weight machinery — `deferredWeightQuoteCode`,
  ...
  the market, because the exact-product lane
  (`Construction/Quotation/ProductDefinition.lean`) needs the deferred-weight quote before
  its definitional extension exists.
* Everything here is market-generic (`MarketComputation P`).  The single paper-facing
  market's own quote codes and the `_closed` endpoints are in `Construction/Paper/Market.lean`.
...
`m.unpair.1` and mesh threshold `i/k` from the packed query `⟨n,⟨k,i⟩⟩`: the shared
`gcd`-reduced quotient emitter `encode_natDiv_polyFueled` for the mesh rational, and the
fixed atom shell around the selector constant.  This is the whole-value emitter for the quotation
...
that route — a product-atom schema entered stagewise by an extra deductive process — is taken
in `Construction/Quotation/ProductDefinition.lean`, at the cost of pricing over a different
inductor.
...
they discharge are stated over `paperDP`, so they live in `Construction/Paper/Market.lean`,
downstream of
...
(`Construction/Quotation/ProductDefinition.lean`) builds its definitional extension on the
literal stream
...
(`Construction/Quotation/ProductDefinition.lean`) needs this quote at a different one.
Paper node: `thm:ccee` -/
