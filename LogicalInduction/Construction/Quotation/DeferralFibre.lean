...
certificates that `Construction/Quotation/Packages.lean` trades on.  Nothing here mentions a
quotation atom, schema or quote code, and the dependency runs one way —
`Construction/Quotation/Packages.lean` imports this module.
...
`thm:wub`/`thm:wubaff`/`thm:wubexp` feedback chain in `Statistics/FeedbackTruth.lean`, where
the paper itself asks for a strictly increasing deferral function.
