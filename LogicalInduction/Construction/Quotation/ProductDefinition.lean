...
`Construction/Quotation/MarketQuoteCodes.lean` builds the `thm:ccee` left product as a
width-`n+1` mesh (`dd:mesh`, its section "The mesh product: a quoted product for an arbitrary
source"), which reflects `x · w (f n)` only to within `1/(n+1)` — the one disclosed type-`(c)`
substitution of that endpoint.  The obstruction there is the *emitter*: a strategy block has to be
produced with polynomial fuel, and the exact threshold `⌜X > r / w (f n)⌝` names a value the emitter
cannot have.
...
(`Construction/SemanticExtension/Endpoints.lean`), which buys exactness over
