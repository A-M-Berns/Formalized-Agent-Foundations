...
/-!
# Semimeasure domination over the constructed `LIA`

`thm:dus` (tex:1561) and `thm:strict` (tex:1575), made unconditional over a constructed `LIA`.
This is one of the lane-terminal endpoint modules; the corresponding files for the other
families are `Construction/Paper/Market.lean` (self-reference),
`Construction/Knowledge/Endpoints.lean` (meta-learning) and
`Construction/Conditioning/Endpoints.lean` (`thm:scon`).
...
and where `paperIndependentBitAtoms` discharges independence substantively.  `thm:strict` has
only the first lane: its single endpoint
...
`Construction/NonDogmatism/UniversalDovetailer.lean`'s explicit dovetail `M*`, and both the
...
in `Construction/NonDogmatism/UniversalDovetailer.lean`, and the independence and naming
