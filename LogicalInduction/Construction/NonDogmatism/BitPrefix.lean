...
class `BigSentenceCodes` (`Framework/Emission/WriteOut.lean`) meters the *canonical Polish run*
...
per-token-value-metered class is `RpnSentenceCodes`, in `Framework/Emission/RpnSplice.lean`; it is
...
`BigSentenceCodes` (`Framework/Emission/WriteOut.lean`), which meters symbols instead of the
pair-code value and is the interface a repair should use. -/
...
is stated with the write-out `BigSentenceCodes` (`Framework/Emission/WriteOut.lean`);
