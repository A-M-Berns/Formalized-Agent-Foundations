...
semantic-lifted `thm:ccee` lane — `Construction/SemanticExtension/{Product,Registry,Source,
Endpoints}.lean` — is built on.
...
completed world identifies a quote leaf with its quotation atom, which is what
`Construction/SemanticExtension/Product.lean` and `Construction/SemanticExtension/Registry.lean`
consume.
