...
witness that reduction consumes.  The split follows the directory invariant that nothing under
`Properties/` imports `Construction/`: the witness is built over `paperDP` and the quotation
layer, which are §5 objects, and this module imports the abstract one, so the witness cannot
be named there.  `lic_paradox_resistance_ofDiagonal` and
`lic_paradox_resistance_ofDiagonal_unconditional` are split the same way.
