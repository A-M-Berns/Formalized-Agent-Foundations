...
(membership in a list of constants) and is what `Freeze/Oracle.lean` consumes; `patterns`
...
unconditional (`parseRpn_of_legacy`, `Framework/Emission/RpnSentence.lean`).
...
beside `parseRpn_bin_inv`, and would move to `Framework/Emission/RpnSentence.lean` with it. -/
...
`c = ⌜ψ⌝` case of `parseRpnLegacy_escape'` (`Framework/Emission/RpnSentence.lean`). -/
...
* legacy to full is **unconditional** and already proved upstream
  (`parseRpn_of_legacy`, `Framework/Emission/RpnSentence.lean`): a legacy escape succeeds only on a
...
what `Freeze/Oracle.lean` consumes; `patterns` states the unconditional form and is what
