...
construction builds over it.  `paperDP` (`Construction/Paper/TheoremDP.lean`) is that process — the
...
(`Construction/SemanticExtension/Endpoints.lean`), because that fixed enlarged language is
what buys exact semantic
...
(`Construction/Quotation/ExactCCEE.lean`), is priced on `liaHistory (paperDP T)` like the rest, as
is the `thm:ccee` closed form at the disclosed `1/(n+1)` slack stated here.
...
noncomputable def paperDeferredExpectationQuoteCode (f : DeferralFunction)
  ...
  -- `Construction/Quotation/MarketQuoteCodes.lean` for the same point stated in full).
...
product in `Construction/Quotation/MarketQuoteCodes.lean`, and the slack is carried by
...
`𝗣𝗔⁻ ⪯ T` by instance (`Foundation`'s `Arithmetic/Schemata.lean`), carrying both would put a
redundant pair
