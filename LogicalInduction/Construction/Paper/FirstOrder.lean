...
`Sentence` type, and gives the boundary the executable form on Gödel codes that the fixed
all-theorems process of `Construction/Paper/TheoremDP.lean` runs on.

## The propositional boundary
...
(`Construction/Knowledge/Syntax.lean`).  Negated relations and universal quantifiers are
represented by propositional negation of their positive prime, matching the paper's prime
decomposition convention; `.nrel` and `.all` are the two negative-prime cases because
Foundation stores formulas in negation normal form (`dd:nnf`).
...
A fixed all-theorems process cannot execute Lean's structural `paperPrimeDecompose`; it needs
a program on Gödel codes.  The second half of this module defines that numeric compiler
directly against Foundation's `Semiformula.toNat` layout.  Invalid inputs receive the harmless
public sentence `⊥`, and correctness is claimed only on genuine arithmetic sentence codes
...
The tags `0`–`7` matched *inside* the compiler are Foundation's `Semiformula.toNat`
