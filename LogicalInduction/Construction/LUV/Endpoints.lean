...
(`WeightingSupportedOnDeferralImage`) is absent from the printed `thm:wubexp`
(tex:1822-1832) and appears instead, spuriously, on `thm:recurringunbiasednessexp`
(tex:1812-1820), whose statement never introduces a deferral function for it to refer to.
Recorded as `PE2` in `notes/paper-errata.md`; the correction is stated at
`FeedbackTruth.luv_wubexp_ofComputation`, which this endpoint is a composition of.

Three inputs stay assumed.  Two are the paper's own: `hdet` is determination via `Θ`
(`def:affthmval`) together with the resulting truth stream — a semantic object, not a
computable oracle, so it is given rather than derived — and `C` is the paper's operational
premise, "`thmval` computable by the next deferral deadline", at the normalized threshold
mesh the trader actually quotes.  The third is `hsupport`, the `PE2` correction above.
...
Kind `C`; assumed hypotheses: `hdet`, `C`, `hsupport`; provenance: `hAs`, `h`, `hshare`,
`hWgen`, `hWdiv`, `hstrict` (a); `C` (a), in the `dd:fuel` efficiency model.
