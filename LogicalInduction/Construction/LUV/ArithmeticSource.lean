...
* `PolyArithmeticSourceSeq` meters that run.  This is the paper's class: a family is
  ...
  (`Construction/LUV/SourceCodec.lean`) is the normal-form-metered foil; it embeds here by
...
(`Construction/LUV/SourceCodec.lean`) frames its payload with `replicate payload.length 1` and the
...
own emitted run, read as a numeral (`tokenListNat`, `Framework/Emission/CodeSource.lean`): one
...
The emitted block is `structuredLeafBlock` (`Construction/LUV/SourceCodec.lean`) at the paper's
...
lemma queryNum_polyFueled :
  ...
  · rw [if_neg hk0, queryRat]
    ...
    rw [hg1, natCast_div_num hk0, Int.natAbs_natCast]
