...
`ValuesAt`) be *derived* rather than *assumed* (`Construction/LUV/Presentation.lean`).
...
bridge in the repository: `Construction/LUV/PaperLUV.lean` declares `PaperLUV` and
`Construction/LUV/ArithmeticSource.lean` the family `PaperLUVSeq` over it (with
`Construction/LUV/SourceCodec.lean` supplying the RPN leaf codec and `ℒₒᵣ` numeral they are
emitted through) — a literal frontend for an *arbitrary* value-defining
...
between LUV values is outside all of them.
...
the everywhere-zero `k = 0` query.  The reduction facts themselves (`natCast_div_num`,
`natCast_div_den`, `encode_rat_natCast_div`) are shared with three other lanes and live in
`Framework/Emission/Computable.lean`. -/
