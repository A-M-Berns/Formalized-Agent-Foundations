...
| `M7-STRICT-SEPARATORS` | `StrictSeparatorPresentation` in `Properties/UniversalSemimeasure.lean` | `thm:strict` / App. `strict`. Disjoint recursively inseparable c.e. sets yield a c.e. theory of single-bit constraints; the appendix's computability argument shows every universal continuous semimeasure gives the stagewise class of consistent strings vanishing total mass; see [Garrabrant et al.](https://arxiv.org/abs/1609.03543). | `strict_domination_of_null_separator_class`, `lic_strict_domination_universalSemimeasure` | Assumes a c.e. constraint theory (efficient repetition + joint stagewise realizability), its per-stage consistent class, and the substantive `mass_class_tendsto_zero` theorem. It contains no market prices and assumes no strict-domination conclusion. The earlier *nested prefix family* shape of this interface is provably uninhabitable — see `no_ce_null_prefix_family` in `Construction/Witnesses/StrictSeparators.lean`. That file also constructs the separator data: `kleene_recursively_inseparable` (Kleene's pair has no computable separator, proved) and `strictSeparatorPresentationOfKleene`, which discharges every field except `mass_class_tendsto_zero` (the sole named hypothesis). The constraint theory's enumerator is built, not assumed (`separatorConstraintCE`), from computability of the atom codes — itself proved for the repo's concrete atoms (`ordinaryAtom_code_computable`). |
...
among other things, a universal lower semicomputable continuous
semimeasure and the recursively-inseparable-set mass theorem (the universal prefix machine
and Kraft's inequality are now in: `Construction/Witnesses/UniversalPrefix.lean`,
`KraftInequality.lean`). The present endpoint does
