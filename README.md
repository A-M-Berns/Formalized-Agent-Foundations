...
| Garrabrant (2021), [*Temporal Inference with Finite Factored Sets*](https://arxiv.org/abs/2109.11513) | [`FiniteFactoredSets/`](FiniteFactoredSets/README.md) | **In progress** — §2–§4 (partitions, factorizations, chimera functions, generation, history, orthogonality, time, subpartitions, conditional orthogonality and the semigraphoid axioms): 51 of the 96 in-scope nodes, at paper strength with no modeling substitutions. Scope is 96 of the paper's 98 numbered nodes, settled by ruling and recorded in that README; §5–§7 are not yet claimed. Non-vacuity is discharged by construction — four factored sets are built and the §2.5 and §3 vocabulary is computed over them. Zero `sorry`, zero `axiom`. |
