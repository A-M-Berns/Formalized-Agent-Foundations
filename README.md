...
| Garrabrant (2021), [*Temporal Inference with Finite Factored Sets*](https://arxiv.org/abs/2109.11513) | [`FiniteFactoredSets/`](FiniteFactoredSets/README.md) | **In progress** — §2–§5 (partitions, factorizations, chimera functions, generation, history, orthogonality, time, subpartitions, conditional orthogonality and the semigraphoid axioms, characteristic polynomials and their factorization, and the fundamental theorem — conditional orthogonality is conditional independence in every distribution on the factored set): 68 of the 96 in-scope nodes carried by declarations of ours (Mathlib renders two of §5.1), at paper strength with no modeling substitutions. Scope is 96 of the paper's 98 numbered nodes, settled by ruling and recorded in that README; §6–§7 are not yet claimed. Non-vacuity is discharged by construction — four factored sets are built and the §2.5 and §3 vocabulary is computed over them. Zero `sorry`, zero `axiom`. |
